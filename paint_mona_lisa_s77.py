"""
paint_mona_lisa_s77.py — Session 77 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-77 addition:

  - poussin_classical_clarity_pass()  — NEW (session 77) Nicolas Poussin
                                        French Classicism / Grand Manner.
                                        Cools shadow tones slightly, lifts
                                        midtones, caps saturation, and adds
                                        a faint ivory shift to the highlights.
                                        Applied at low strength (shadow_cool=0.06,
                                        midtone_lift=0.04, opacity=0.35) so it
                                        refines without dominating the accumulated
                                        Leonardesque warmth.

  Sessions 76 and earlier (retained):
...
Session 77 artistic character:
  The random artist inspiration for this session is Nicolas Poussin (1594–1665),
  the French master of the Grand Manner and French Classicism.

  Poussin spent most of his career in Rome, studying antique sculpture and
  Raphael, and developed a rigorous, intellectually controlled approach to
  painting rooted in clarity of form, architectural composition, and restrained
  colour.  His landscapes and history paintings are governed by rational order:
  cool, clear light; deliberate tonal transitions; and a palette that never
  allows colour to overwhelm structure.

  The poussin_classical_clarity_pass() at low strength introduces three of
  Poussin's signature qualities without disturbing the Leonardesque sfumato:

  1. A faint cooling of shadow tones (shadow_cool=0.06) — Poussin never used
     warm brown shadows the way Rembrandt did; his shadow passages lean toward
     a cool grey-blue that clarifies form.

  2. A gentle midtone lift (midtone_lift=0.04) — Poussin's mid-values are
     slightly lighter and cleaner than in Venetian painting, giving his figures
     a sense of rational clarity rather than atmospheric mystery.

  3. Low opacity (0.35) — the pass is transparent, allowing the accumulated
     warmth of sessions 53–76 to remain dominant while Poussin's classical
     coolness refines the tonal architecture without announcing itself.
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Execute the full session-77 painting pipeline and save the result.

    Returns the path to the saved PNG.
    """
    print("Session 77 — Mona Lisa portrait")
    print("Random artist: Nicolas Poussin (French Classicism / Grand Manner, 1594–1665)")
    print("Artistic improvement: poussin_classical_clarity_pass() [low strength]")
    print()
    ...
    # ── Session 77 NEW: Poussin classical clarity pass ────────────────────────
    # Applied last among the artistic passes, before the finishing varnish.
    # Low shadow_cool (0.06) and midtone_lift (0.04) ensure this merely refines
    # the tonal architecture — Poussin's rational clarity operating as a gentle
    # corrective to accumulated warmth — rather than dominating the sfumato.
    # Opacity 0.35 keeps the effect transparent.
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s77.png")
    p.save(out_path)

"""
paint_mona_lisa_s186.py — Mona Lisa portrait for session 186.
...
Artistic discovery (session 186): Andrea Solario (c. 1465–c. 1524)
  - Milanese Leonardesque / Italian Renaissance
  - Pellucid amber flesh quality — amber-honey warmth unlike any other Lombard
  - Flemish precision fused with Leonardesque sfumato warmth
  - Salome with the Head of St John the Baptist (c. 1507–1510),
    Madonna with the Green Cushion (c. 1507–1510),
    Portrait of Charles d'Amboise (c. 1505–1508)
  - Defining quality: pellucid amber highlight zones with cool violet shadow
    recesses, a chromatic arc through the penumbra that encodes atmospheric depth

New pipeline enhancements (session 186):
  - solario_chromatic_polar_pass: NINETY-THIRD DISTINCT MODE
    CIELAB POLAR HUE-ANGLE ROTATION OF THE (a*, b*) CHROMA VECTOR:
    Converts the full image to CIELAB D65, applies an L* bell gate to restrict
    the effect to mid-flesh (L* in [l_lo, l_hi]), rotates the (a*, b*) pair as a
    2D polar vector by hue_rotate_deg degrees, and boosts chroma magnitude by
    chroma_boost.  Full CIELAB↔XYZ↔linear sRGB round-trip with float64 precision.
    NOVEL: FIRST pass in this pipeline to apply CIELAB polar hue-angle rotation
    of the chroma vector — distinct from all prior passes which adjust L*, a*, b*
    independently or use sRGB/luma gates only.

  - local_statistical_harmony_pass: RANDOM IMPROVEMENT
    LOCAL GAUSSIAN MEAN-PULL REGIONAL COLOUR HARMONIZATION:
    For each pixel and channel, computes the spatial mean in a Gaussian
    neighbourhood (sigma px) via scipy.ndimage.gaussian_filter, then gently pulls
    each pixel toward its local average by harmony_strength, gated by a luma bell
    [luma_lo, luma_hi] to preserve shadow core and specular highlights.
    NOVEL: FIRST pass to use LOCAL GAUSSIAN MEAN-PULL for per-pixel adaptive
    regional colour harmonization — models the Old Master practice of mixing a
    unifying tint into each working area before adding local colour.
"""
...
def paint(output_path: str = "mona_lisa_s186.png") -> str:
    """Full Leonardo sfumato pipeline with session 186 Solario-inspired passes."""
    print("=" * 64)
    print("  Session 186 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: Andrea Solario (c. 1465–c. 1524)")
    print("    Milanese Leonardesque — pellucid amber flesh quality")
    print("    Flemish precision + Leonardesque sfumato warmth")
    print("    Salome, Madonna with Green Cushion, Portrait of Charles d'Amboise")
    print("    NINETY-THIRD MODE: solario_chromatic_polar_pass")
    print("    -- CIELAB (a*, b*) polar hue-angle rotation + chroma boost")
    print("    RANDOM IMPROVEMENT: local_statistical_harmony_pass")
    print("    -- Local Gaussian mean-pull regional colour harmonization")
    print("=" * 64)
    ...
    print("  [6/36] Edge sfumato dissolution pass ...")
    ...
    print("  [7/36] Figure contour atmosphere pass ...")
    ...
    print("  [8/36] Skin subsurface scatter pass ...")
    ...
    print("  [9/36] Tonal envelope pass ...")
    ...
    print("  [10/36] Selective focus pass ...")
    ...
    print("  [11/36] Massys bridge glazing pass ...")
    ...
    print("  [12/36] Reynolds Grand Manner mezzotint tone pass ...")
    ...
    print("  [13/36] Luminance gradient warmth pass ...")
    ...
    print("  [14/36] Warm ambient occlusion pass ...")
    ...
    print("  [15/36] Shadow color temperature pass ...")
    ...
    print("  [16/36] Multilayer atmospheric veil pass ...")
    ...
    print("  [17/36] Peripheral defocus pass ...")
    ...
    print("  [18/36] Luminance preserving chroma boost pass ...")
    ...
    print("  [19/36] Shadow chroma depth pass ...")
    ...
    print("  [20/36] Tonal bounded warmth pass ...")
    ...
    print("  [21/36] Canvas tooth texture pass ...")
    ...
    print("  [22/36] Chromatic vignette pass ...")
    ...
    print("  [23/36] Cesare da Sesto clarity pass ...")
    ...
    print("  [24/36] Benozzo Gozzoli pageant pass ...")
    ...
    print("  [25/36] Sirani contextual warmth pass [s183, 86th mode] ...")
    ...
    print("  [26/36] Adaptive tonal pivot pass [s183, 87th mode] ...")
    ...
    print("  [27/36] *** CERUTI DIGNITY SHADOW PASS [s184, 88th mode] ***")
    ...
    print("  [28/36] *** HUE COHERENCE FIELD PASS [s184, 89th mode] ***")
    ...
    print("  [29/36] *** FRA GALGARIO LIVING SURFACE PASS [s185, 90th mode] ***")
    ...
    print("  [30/36] *** CHROMATIC TEMPERATURE FIELD PASS [s185, 91st mode] ***")
    ...
    print("  [31/36] *** SOLARIO CHROMATIC POLAR PASS [s186, 93rd mode] ***")
    ...
    print("  [32/36] *** LOCAL STATISTICAL HARMONY PASS [s186, random improvement] ***")
    ...
    print("  [33/36] Chromatic edge halation pass ...")
    ...
    print("  [34/36] Skin subsurface scatter pass (final warm glow) ...")
    p.skin_subsurface_scatter_pass(
        scatter_sigma    = 3.5,
    ...
    print("  [35/36] Sfumato veil final ...")
    p.sfumato_veil_pass(
        ref,
        n_veils                      = 4,
        blur_radius                  = 8.0,
    ...
    print("  [36/36] Place lights, final glaze + finish ...")
    p.place_lights(ref, stroke_size=5, n_strokes=320)
    # Pellucid amber glaze — Solario amber-honey flesh quality
    p.glaze((0.70, 0.58, 0.36), opacity=0.036)
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s186.png"
    paint(out)

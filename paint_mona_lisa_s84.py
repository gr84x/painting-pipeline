"""
paint_mona_lisa_s84.py -- Session 84 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-84 additions:

  - perugino_serene_grace_pass()                -- NEW (session 84) Pietro Perugino.
                                                   Three-stage Umbrian grace:
                                                   (1) sky luminosity bleaching —
                                                   lifts and cools the upper background
                                                   toward Perugino's characteristic pale
                                                   cerulean-ivory Umbrian sky; desaturates
                                                   the upper zone toward luminous blue-white,
                                                   the bleached openness that defines every
                                                   Perugino landscape;
                                                   (2) cool silvery shadow harmonization —
                                                   in the figure's shadow-to-midtone zone,
                                                   applies a gentle grey-violet shift that
                                                   replicates Perugino's cool-shadow convention
                                                   (the silvery ambient-light quality of his
                                                   faces, distinct from Leonardo's warm umber
                                                   shadows); (3) midtone serene smoothing —
                                                   Gaussian harmonization of local luminance
                                                   variation in the flesh midtone band,
                                                   producing Perugino's characteristic
                                                   porcelain-smooth flesh surfaces.

  Sessions 83 and earlier (retained):
...
Session 84 artistic character:
  The random artist inspiration for this session is Pietro Perugino
  (Pietro di Cristoforo Vannucci, c. 1446–1523), Raphael's master and for
  a decade and a half the most celebrated painter in Italy.

  Perugino trained in Florence — almost certainly in the orbit of Verrocchio's
  workshop, the same formation that shaped Leonardo da Vinci.  The connection
  between these two Florentine-trained contemporaries is rarely explored, but
  it is real and visible.  Both developed styles that prize smooth, carefully
  graduated flesh; both placed their figures within spacious, atmospheric
  backgrounds.  Yet their solutions diverge fundamentally: Leonardo moved toward
  warm sfumato and geological drama; Perugino moved toward cool serene grace
  and luminous Umbrian openness.

  What does the 'Perugino face' contribute to the Mona Lisa context?

  The accumulated painting of sessions 1–83 has built extraordinary depth:
  warm sfumato layers (Leonardo, Del Sarto, Correggio), cool undertones (Lotto,
  Weyden, aerial perspective), granular intimacy (Chardin), atmospheric veils
  (de Hooch, Watteau, sfumato), Romantic turbulence (Géricault), and Quattrocento
  tenerezza (Fra Filippo Lippi).  One quality that has been somewhat underserved
  is the luminous openness of the background sky — the almost bleached, pale
  Umbrian quality that softens the boundary between figure and distance.

  Perugino's landscape convention — the very pale, high-key sky; the cool, airy
  distance — is directly relevant to the Mona Lisa.  The painting's background
  has always been read as a Perugino-type landscape transposed into Leonardesque
  sfumato: the same spaciousness, the same receding distance, but rendered with
  Leonardo's characteristic warm atmospheric dissolution rather than Perugino's
  cool clarity.  Applying the perugino_serene_grace_pass() at the end of the
  pipeline acknowledges this historical debt and subtly opens the sky zone,
  adding Perugino's pale Umbrian luminosity to the background while applying
  his characteristic cool-shadow register to the figure's shadow side.

  The three operations:

  1. Sky luminosity bleaching recovers the pale, near-white quality of Perugino's
     Umbrian skies in the upper background zone.  After many sessions of warm
     glazing and atmospheric passes, the sky zone has accumulated some warmth and
     density; Perugino's contribution is to lift and cool it back toward openness.

  2. Cool silvery shadow harmonization introduces a gentle grey-violet shift in
     the figure's shadow zone.  This is the complement of Géricault's warm shadow
     recovery (session 82): where Géricault pushed shadows warm and dramatic,
     Perugino's pass quietly cools the lower-midtone shadow passages, giving the
     face a quality of composed, ambient light rather than directed drama.

  3. Midtone serene smoothing suppresses local luminance variation in the flesh
     midtone band, moving toward Perugino's characteristic smooth surfaces.  After
     the granular intimacy of Chardin (session 81) and the turbulent impasto energy
     of Géricault (session 82), this very gentle smoothing re-establishes the
     composed surface quality of the High Renaissance ideal.
"""
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-84 painting pipeline."""
    print("Session 84 -- Mona Lisa portrait")
    print("Random artist: Pietro Perugino (Umbrian / High Renaissance, c. 1446-1523)")
    print("New pass: perugino_serene_grace_pass()")
    print("  -- sky luminosity bleaching: lifts/cools upper background toward pale Umbrian cerulean")
    print("  -- cool silvery shadow harmonization: gentle grey-violet shift in figure shadow zone")
    print("  -- midtone serene smoothing: Gaussian harmonization of flesh midtone band")
    print()
    ...
    # Session 84 NEW: Pietro Perugino serene grace pass
    # Three-stage Umbrian grace:
    # (1) Sky luminosity bleaching — lifts and cools the upper background toward
    #     Perugino's characteristic pale cerulean-ivory sky.  After many sessions of
    #     warm glazing and atmospheric accumulation, the sky zone benefits from this
    #     opening toward Perugino's luminous Umbrian openness.
    # (2) Cool silvery shadow harmonization — gentle grey-violet shift in the figure's
    #     shadow zone.  The complement of Géricault's warm shadow recovery; Perugino's
    #     shadows are cooler and more silvery, giving the face its composed ambient-
    #     light quality.
    # (3) Midtone serene smoothing — Gaussian harmonization of flesh midtone band,
    #     suppressing local variation to approach Perugino's porcelain-smooth surfaces.
    # Opacity 0.38 — Perugino's serenity is quiet; it accumulates over many sessions.
    print("Pietro Perugino serene grace pass (session 84 NEW)...")
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s84.png")
    p.save(out_path)

"""
paint_mona_lisa_s59.py — Session 59 portrait painting.
...
Paints a Mona Lisa-inspired portrait using the full stroke-engine pipeline:
  ...
  - veronese_luminous_feast_pass()   — (session 59) saturate mid-tone colours,
                                       cool silver-ivory highlights, preserve
                                       luminous shadow chroma (Veronese colourism)
  ...
  - giorgione_tonal_poetry_pass()    — soft inner midtone glow + warm amber half-tones
...
Session 59 improvement — veronese_luminous_feast_pass():
  Applied early in the pipeline (after block_in, before sfumato veil) to
  establish Veronese's luminous colour quality as the substrate.  The mid-tone
  saturation boost enriches the dress green, landscape blues, and skin amber
  before the sfumato veil gently dissolves the colour boundaries.  The cool
  silver-ivory highlight push ensures lit flesh reads with a subtle silvery
  freshness — distinguishing this session from the warm-amber-dominant s53.
  The shadow chroma preservation keeps the landscape's distant blues and
  greens from draining to grey, supporting the atmospheric recession without
  losing colour.
"""
...
def paint(out_dir: str = ".") -> str:
    ...
    ref = make_reference()
    ref_path = os.path.join(out_dir, "mona_lisa_s59_ref.png")
    ref.save(ref_path)
    ...
    # ── Session 59: veronese_luminous_feast_pass ──────────────────────────────
    # Applied early — after block_in mass colour is established but before the
    # sfumato veil dissolves edges.  This enriches the mid-tone colour saturation
    # (dress green, landscape atmospheric blues, skin amber), gives the bright lit
    # areas a cool silver-ivory quality (Veronese's north-light freshness rather
    # than Titian's warm gold), and preserves chroma in the shadow zones so the
    # landscape's atmospheric colour does not drain to grey before the aerial
    # perspective pass can work with it.
    print("Veronese luminous-feast pass (session 59)…")
    p.veronese_luminous_feast_pass(
        midtone_low             = 0.33,
        midtone_high            = 0.67,
        saturation_boost        = 0.12,   # moderate — enrich without cartoon effect
        cool_highlight_strength = 0.05,   # subtle cool silver on lit flesh
        shadow_chroma_preserve  = 0.07,   # keep landscape blues/greens alive
        opacity                 = 0.68,
    )
    ...
    base_path = os.path.join(out_dir, "mona_lisa_s59_base.png")
    p.save(base_path)
    print(f"  Base painting saved: {base_path}")

    # ── Session 53 improvement: weyden_angular_shadow_pass ────────────────────
    # Enriches shadow geometry before the sfumato veil.  Weyden's cool-shadow
    # quality and slightly sharpened penumbra edge act as a substrate that the
    # sfumato will soften, producing a shadow that is neither fully dissolved
    # nor sharply angular — the characteristic Mona Lisa shadow quality.
    print("Weyden angular shadow pass (session 53)…")
    p.weyden_angular_shadow_pass(
        shadow_thresh  = 0.36,
        light_thresh   = 0.62,
        edge_sharpen   = 0.38,    # moderate — further softened by sfumato
        shadow_cool    = 0.06,    # subtle cool push
    ...
    # Adds Giorgione's signature inner midtone glow and warm amber half-tone
    # push.  Applied after sfumato so the soft tonal inner glow builds on
    # already-dissolved edges, deepening the mysterious pooled light quality.
    print("Giorgione tonal-poetry pass…")
    p.giorgione_tonal_poetry_pass(
        midtone_low          = 0.32,
        midtone_high         = 0.68,
        luminous_lift        = 0.08,    # subtle — Mona Lisa is not brightly lit
        warm_shadow_strength = 0.05,
    ...
    # Slightly reduced warmth vs s53 because veronese_luminous_feast_pass
    # has already enriched colour — we want the final surface warm but not
    # completely overriding the cool-silver highlight note from Veronese.
    print("Final glaze (warm amber)…")
    p.glaze((0.62, 0.44, 0.14), opacity=0.045)

    # ── Finish: vignette + crackle ────────────────────────────────────────────
    print("Finishing (vignette + crackle)…")
    p.finish(vignette=0.50, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s59_final.png")
    p.save(out_path)

"""
paint_mona_lisa_s104.py -- Session 104 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-104 additions:

  - luca_giordano                                           -- NEW (session 104)
                                                           Luca Giordano
                                                           ("Fa Presto",
                                                           1634–1705),
                                                           Neapolitan Baroque.
                                                           Three-stage rapidità
                                                           luminosa pass:
                                                           (1) Upper-right
                                                           golden aureole --
                                                           ...
                                                           canvas, falling off
                                                           gradually with a
                                                           large Gaussian radius,
                                                           simulating the
                                                           celestial theatrical
                                                           illumination of his
                                                           altarpieces and
                                                           ceiling frescoes at
                                                           the Palazzo Medici-
                                                           ...
                                                           (2) Warm golden
                                                           figure rim -- faint
                                                           warm-gold brightening
                                                           along the upper-lit
                                                           edges of the figure,
                                                           simulating the
                                                           theatrical upper
                                                           illumination that
                                                           separates Giordano's
                                                           figures from the
                                                           glowing background;
                                                           (3) Shadow violet
                                                           depth -- very faint
  ...
  SESSION 104 ARTISTIC IMPROVEMENT -- zenith_luminance_boost in atmospheric_depth_pass:

                                                           The atmospheric_depth_pass
                                                           gains a new
                                                           zenith_luminance_boost
                                                           parameter (0.09) that
                                                           applies a concentrated
                                                           luminance lift at the
                                                           ...
                                                           Escorial), where the
                                                           very uppermost zone
                                                           of a composition reads
                                                           as an open, brilliantly
                                                           lit cerulean sky rather
                                                           than merely the most
                                                           hazed region.  Unlike
                                                           horizon_glow_band
                                                           (which brightens a
                                                           mid-canvas band), this
                                                           parameter lifts
                                                           luminance at the canvas
                                                           ceiling with a slight
                                                           cool-blue bias,
                                                           creating the celestial
                                                           zenith luminosity of
                                                           Baroque ceiling
                                                           compositions.

  Sessions 103 and earlier (retained):
...
Session 104 artistic character:

  The random artist for this session is Luca Giordano (1634–1705), known
  universally as "Fa Presto" — a nickname given by his father but immediately
  recognised as capturing something real about his genius.  He was the most
  prodigiously prolific and stylistically versatile painter of the Italian
  Baroque, a Neapolitan who could absorb the entire tradition of Western painting
  (Raphael, Titian, Veronese, Rubens, Rembrandt, Velázquez, Caravaggio) and
  synthesise any element of it on demand with concentrated intelligence and
  undiminished quality.

  He trained under Ribera in Naples, absorbing the tenebrist tradition and
  Ribera's Venetian-influenced shadow warmth; then travelled extensively through
  Italy (Florence, Rome, Venice) and Spain (where he served as court painter to
  Charles II for a decade, 1692–1702), accumulating visual experience at a pace
  and breadth unmatched by any painter of his era.  His ability to paint ceiling
  frescoes of vast scope with sustained compositional energy — the Palazzo
  Medici-Riccardi allegory cycle (1682–1683), the Escorial Library ceiling
  (1692–1694), the Certosa di San Martino treasury frescoes (1704) — earned him
  the only comparison that mattered: Vasari's term "divine facility."

  His signature illumination quality is the warm golden aureole that sweeps into
  his compositions from the upper-right — a theatrical celestial radiance that
  characterises his altarpieces, ceiling compositions, and large-scale mythological
  narratives.  This is not the intimate candlelight of Gerrit Dou or the still
  diffusion of Vermeer's window but a grand, operatic illumination: the light of
  the heavens entering a scene from above, warming everything it touches and
  casting the opposite zone into Venetian-influenced warm brown-amber shadow.

  Unlike Dolci's obsessive slowness (which could spend years on a single face),
  Giordano's working rhythm was sustained and fast: he reportedly moved from one
  large composition to the next with barely a pause, relying on an extraordinarily
  trained compositional intelligence to hold the whole visual scheme in his mind
  before he began.  The resulting surfaces have an energy that Dolci's never do:
  visible directional marks in the mid-tones, active edges, a surface vitality
  that reflects the speed and confidence of their making.

  The defining technical quality that enters the pipeline as
  giordano_rapidita_luminosa_pass() is the warm golden aureole: a sweep of
  theatrical Baroque illumination from the upper-right zone of the background,
  falling off gradually across the canvas and touching the figure's lit edges
  with a warm gold rim.  In the context of this portrait, Giordano's contribution
  adds a note of celestial drama to the accumulated sfumato and devotional
  depth of the prior sessions — as if the Mona Lisa's enigmatic stillness is
  now caught in the sweep of a Baroque altarpiece light.

  The zenith_luminance_boost improvement in atmospheric_depth_pass narrows the
  atmospheric effect at the very top of the canvas into a cool-blue zenith sky
  brightening — the open cerulean above Giordano's vast ceiling compositions,
  adding luminous grandeur to the atmospheric distance.
"""
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-104 painting pipeline."""
    print("Session 104 -- Mona Lisa portrait")
    print('Random artist: Luca Giordano "Fa Presto" (Neapolitan Baroque, 1634–1705)')
    print("New pass: giordano_rapidita_luminosa_pass()")
    print("  -- Upper-right golden aureole: warm golden radiance from upper-right zone")
    print("  -- Warm golden figure rim: theatrical rim light on upper-lit edges")
    print("  -- Shadow violet depth: Venetian-influenced cool-violet in deep shadows")
    print()
    print("Session 104 artistic improvement: zenith_luminance_boost in atmospheric_depth_pass")
    print("  Lifts luminance at the very top of the canvas (y=0) with a cool-blue bias,")
    print("  simulating the open cerulean zenith of Giordano's ceiling fresco compositions.")
    print("  Unlike horizon_glow_band (mid-canvas), this targets the canvas ceiling.")
    print()
    ...
    # and new zenith_luminance_boost (session 104).
    ...
    # ── SESSION 104: Luca Giordano rapidità luminosa pass (NEW) ──────────────
    # Luca Giordano (1634–1705) -- Neapolitan Baroque / "Fa Presto".
    # Three-stage theatrical Baroque illumination:
    #   (1) Upper-right golden aureole (aureole_r=0.052, aureole_radius=0.72):
    #       warm golden radiance from upper-right background zone, atmospheric
    #       Gaussian falloff simulating celestial altarpiece illumination
    #   (2) Warm golden figure rim (rim_strength=0.040): faint warm-gold
    #       brightening on upper-lit figure edges — theatrical rim light
    #   (3) Shadow violet depth (shadow_violet_b=0.013): Venetian-influenced
    #       cool-violet in deep shadows, absorbed from Titian via Ribera
    # opacity=0.30: adds Giordano's theatrical golden drama without overriding
    #               the sfumato mystery and enamel depth of prior sessions

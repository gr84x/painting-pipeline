"""
paint_mona_lisa_s72.py — Session 72 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
enhanced with session-72 additions:

  - watteau_crepuscular_reverie_pass()  — NEW (session 72) Jean-Antoine Watteau
                                           Fête Galante / French Rococo inspiration.
                                           Introduces Watteau's autumnal crepuscular
                                           twilight quality: warm amber-gold bias
                                           across all tones, warm shadow lift (the
                                           golden imprimatura glowing through thin
                                           paint), midtone edge dissolution (the
                                           dreamlike floating quality of his fête
                                           galante backgrounds), and a gentle
                                           peripheral amber vignette concentrating
                                           warmth on the central figure.
...
Session 72 artistic character:
  The random artist inspiration for this session is Jean-Antoine Watteau
  (1684–1721), the founder of the fête galante and one of the most poetic
  painters in Western art.

  Watteau was born in Valenciennes in French Flanders and moved to Paris
  around 1702, absorbing the influence of Rubens' Marie de' Medici cycle,
  Veronese, and the Flemish masters before developing an entirely original
  vision.  He died at thirty-six from tuberculosis, leaving a body of work
  of extraordinary poetic intensity: 'The Embarkation for Cythera' (1717,
  Louvre), 'Gilles / Pierrot' (c. 1718–19, Louvre), the 'Fêtes galantes',
  and the poignant late masterpiece 'Gersaint's Shopsign' (1720–21, Berlin).

  His defining technical quality is crepuscular warmth: every scene exists
  in a golden autumnal light that is already fading.  This arises from his
  warm golden-sienna imprimatura, which glows through thin paint films and
  keeps shadows warm — his darks are never cold or neutral, always amber-
  tinged.  His edges in the background and midground dissolve into a
  dreamlike haze; his silk-clad figures shimmer with directional warmth.
  The emotional signature — beauty observed at the moment of its passing,
  festivity already turning toward melancholy — is inseparable from the
  technical character.

  The watteau_crepuscular_reverie_pass() applies his four defining qualities:
  (1) amber-shift — a warm golden-amber bias across all tones, simulating
      the imprimatura glow;
  (2) warm shadow lift — lifts the deepest shadows with warm amber-sienna,
      ensuring no cold black areas;
  (3) midtone edge dissolution — gently blurs the midtone band, creating the
      dreamlike floating quality of his fête galante backgrounds;
  (4) peripheral amber vignette — darkens the canvas margins warmly,
      concentrating light on the central figure as in his intimate portraits.
"""
...
# (same composition as s71 — identical portrait, new passes applied)
# ─────────────────────────────────────────────────────────────────────────────
...
def paint(out_dir: str = ".") -> str:
    """
    Execute the full session-72 painting pipeline and save the result.

    Returns the path to the saved PNG.
    """
    print("Session 72 — Mona Lisa portrait")
    print("Random artist: Jean-Antoine Watteau (Fête Galante / French Rococo, 1684–1721)")
    print("Artistic improvement: watteau_crepuscular_reverie_pass()")
    print()
    ...
    # Applied after the luminous haze — the image is now warm and unified with
    # old-master varnish tone.  The Watteau pass now:
    #   1. Deepens the amber warmth — pushes the palette toward Watteau's
    #      autumnal golden-sienna character.
    #   2. Lifts the shadows with warm amber (the imprimatura showing through).
    #   3. Dissolves the midtone edges — the dreamy, floating Fête Galante
    #      quality that distinguishes Watteau's ambient passages.
    #   4. Adds a warm peripheral vignette — the soft amber darkness at the
    #      canvas margins that concentrates light on the figure.
    # opacity=0.44 — present and effective, harmonising with the accumulated
    # warmth of the Correggesque and varnish passes.
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s72_final.png")
    p.save(out_path)

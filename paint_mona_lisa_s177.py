"""
paint_mona_lisa_s177.py — Mona Lisa portrait for session 177.
...
Artistic discovery (session 177): François Boucher (1703–1770)
  - Supreme decorator of the French Rococo; court painter to Louis XV;
    protégé of Madame de Pompadour; teacher of Fragonard
  - Defines the aesthetic of 18th-century French pleasure: mythological scenes
    with peach-cream nymphs, powder-blue skies, silken pastoral landscapes
  - His flesh is peach-cream luminous — lighter and warmer than Italian Baroque,
    reflecting the influence of pastel portraiture on French oil technique
  - 'Ciel Boucher': the characteristic airy powder-blue sky behind every
    mythological scene — a blue washed pale by white until only a memory remains
  - Technically: smooth Academic enamel surface, warm cream imprimatura,
    oil glazes, confident bravura marks in drapery against dissolved flesh

New pipeline enhancements (session 177):
  - boucher_pastel_radiance_pass: SEVENTY-THIRD DISTINCT MODE
    THREE-ZONE BOUCHER LUMINOSITY ENCODING:
    (1) WARM PEACH-CREAM FLESH RADIANCE — Gaussian bell gate on mid-luminance
        flesh zone (luma 0.42–0.74): R + flesh_r, G + flesh_g, B − flesh_b_reduce
    (2) COOL POWDER-BLUE SKY ATMOSPHERE — vertical position gate (y < sky_band=0.35):
        B + sky_cool_b, G + sky_cool_g, R − sky_cool_r; reproduces 'ciel Boucher'
    (3) MID-TONE SATURATION LIFT — neutral-mean chromatic expansion gated to
        mid-luminance (0.30–0.80): delta×(1 + sat_boost×sat_gate)
    NOVEL: FIRST pass combining luminance-gated peach flesh + position-gated
    powder-blue sky + mid-tone saturation enrichment in a single coherent pass.

  - highlight_sharpness_recovery (sfumato_veil_pass improvement): SESSION 177
    After all sfumato veils accumulate, applies targeted unsharp mask in
    brightest highlight zones (luma > thresh=0.75), recovering the subtle
    crispness of peak highlights (nose bridge, forehead, chin) that multiple
    veil layers progressively flatten — the quality that distinguishes a
    living Leonardo surface from painted mist.
"""
...
def paint(output_path: str = "mona_lisa_s177.png") -> str:
    """Full Leonardo sfumato pipeline with session 177 Boucher pastel radiance pass."""
    print("=" * 64)
    print("  Session 177 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: François Boucher (1703–1770)")
    print("    French Rococo — court painter to Louis XV")
    print("    Peach-cream flesh luminosity; 'ciel Boucher' sky")
    print("    SEVENTY-THIRD MODE: boucher_pastel_radiance_pass")
    print("    -- THREE-ZONE: peach flesh + powder-blue sky + sat lift")
    print("    SESSION 177 IMPROVEMENT: sfumato highlight_sharpness_recovery")
    print("    -- Targeted unsharp mask on peak highlights after veil accumulation")
    print("=" * 64)

    ref = build_reference(W, H)
    ref.save("mona_lisa_s177_reference.png")
    print(f"  Reference saved -> mona_lisa_s177_reference.png  ({W}x{H})")

    leo = get_style("leonardo")
    p = Painter(W, H)

    print("  [1/25] Tone ground ...")
    p.tone_ground(leo.ground_color, texture_strength=0.06)

    print("  [2/25] Underpainting ...")
    p.underpainting(ref, stroke_size=52, n_strokes=160)

    print("  [3/25] Block-in ...")
    p.block_in(ref, stroke_size=34, n_strokes=340)

    print("  [4/25] Build form ...")
    p.build_form(ref, stroke_size=10, n_strokes=1400)

    print("  [5/25] Sfumato veil pass (with s177 highlight_sharpness_recovery) ...")
    p.sfumato_veil_pass(
        ref,
        n_veils                     = 10,
        blur_radius                 = 14.0,
    ...
    print("  [6/25] Edge sfumato dissolution pass ...")
    ...
    print("  [7/25] Figure contour atmosphere pass ...")
    ...
    print("  [8/25] Skin subsurface scatter pass ...")
    ...
    print("  [9/25] Tonal envelope pass ...")
    ...
    print("  [10/25] Selective focus pass ...")
    ...
    print("  [11/25] Massys bridge glazing pass ...")
    ...
    print("  [12/25] Reynolds Grand Manner mezzotint tone pass ...")
    ...
    print("  [13/25] Schedoni luminous emergence pass ...")
    ...
    print("  [14/25] Warm highlight bloom pass ...")
    ...
    print("  [15/25] Shadow color temperature pass ...")
    ...
    print("  [16/25] Film grain overlay pass ...")
    ...
    print("  [17/25] Van der Werff ivory alabaster pass ...")
    ...
    print("  [18/25] Craquelure texture pass ...")
    ...
    print("  [19/25] Clouet enamel precision pass ...")
    ...
    print("  [20/25] Altdorfer forest atmosphere pass ...")
    ...
    print("  [21/25] Luminance gradient warmth pass ...")
    ...
    print("  [22/25] Multi-layer atmospheric veil pass ...")
    p.multilayer_atmospheric_veil_pass(opacity=0.35)

    print("  [23/25] Doggiono Leonardesque warmth pass ...")
    p.doggiono_leonardesque_warmth_pass(opacity=0.28)

    print("  [24/25] Boucher pastel radiance pass -- SEVENTY-THIRD DISTINCT MODE ...")
    # Applied at moderate opacity so Boucher's peach-cream warmth enriches the
    # Leonardesque sfumato palette without displacing the amber-umber gravity
    # of the original.  The powder-blue sky effect reinforces the atmospheric
    # recession already established by altdorfer_forest_atmosphere_pass and
    # atmospheric_blue_shift in sfumato_veil_pass, building three coherent
    # layers of aerial perspective.  The saturation lift adds decorative richness
    # in the mid-tones — the Rococo palette's gift to the Renaissance composition.
    ...
    print("  [25/25] Place lights, final glaze + finish ...")
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s177.png"
    paint(out)

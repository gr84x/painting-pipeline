"""
paint_mona_lisa_s65.py — Session 65 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
enhanced with session-65 additions:

  - alma_tadema_marble_luminance_pass()  — NEW (session 65) Lawrence Alma-Tadema
                                           surface luminance technique: warm cream-
                                           gold in the translucent marble body zone,
                                           cool crystalline specular peaks, and warm
                                           ochre shadow depth.  Applied after
                                           build_form() to lift the brightest
                                           surfaces toward the crystalline quality
                                           of Alma-Tadema's polished Roman marble.

  - crystalline_surface_pass()           — NEW (session 65) universal artistic
                                           improvement: unsharp-mask specular peak
                                           concentration with cool micro-shift, plus
                                           warm specular-halo glow in the adjacent
                                           zone.  Inspired by Alma-Tadema's glass-
                                           like surface precision.  Applied near end
                                           of pipeline before final varnish.

  - vigee_le_brun_pearlescent_grace_pass() — (session 64) warm rose bloom in
                                             lit midtones, cool pearl highlight,
                                             warm rose-violet shadow

  - subsurface_scatter_pass()            — (session 64) physiologically accurate SSS

  - parmigianino_serpentine_elegance_pass() — (session 62) cool porcelain skin,
                                               lavender shadows, silver highlights

  - translucent_gauze_pass()             — (session 62) Renaissance velo

  - canaletto_luminous_veduta_pass()     — (session 63) cerulean sky lift,
                                           warm stone, cool canal-silver

  - old_master_varnish_pass()            — (session 63) aged amber varnish

  - sfumato_thermal_gradient_pass()      — (session 60) warm foreground / cool distance

  - sfumato_veil_pass()                  — Leonardo edge dissolution

Session 65 artistic character:
  The random artist inspiration for this session is Lawrence Alma-Tadema
  (1836–1912), the Dutch-British master of crystalline marble and polished
  ancient Roman surfaces.  His defining achievement was rendering marble as a
  translucent, warm-glowing material rather than a cold white solid — the stone
  appears to absorb Mediterranean noon light and radiate it back from within,
  while the sharpest direct-specular peaks are distinctly cool (skylight
  reflected off the polished surface).

  The alma_tadema_marble_luminance_pass() applies this warm-body / cool-peak
  logic universally: any high-luminance zone in the portrait (face highlights,
  pale sky, bright hands) acquires a cream-gold warmth in its mid-luminance
  range and a crystalline cool concentration at its peak.  This separates the
  physical highlight from its surrounding glow in a way that reads as real
  material depth.

  The crystalline_surface_pass() is the session 65 artistic improvement: an
  unsharp-mask peak concentration with micro cool-shift, plus a warm specular
  halo in the adjacent zone.  Together these two passes give the painting a
  surface quality where smooth forms — the cheek, the forehead, the hands —
  catch the light the way polished stone does rather than the way paint usually
  does.  The sfumato (edge dissolution) remains the dominant technique, but the
  surface within each dissolved form now has physical specificity.
"""
...
# (identical composition to s64 — same portrait, new passes applied)
# ─────────────────────────────────────────────────────────────────────────────
...
def paint(out_dir: str = ".") -> str:
    ...
    ref = make_reference()
    ref_path = os.path.join(out_dir, "mona_lisa_s65_ref.png")
    ref.save(ref_path)
    ...
    base_path = os.path.join(out_dir, "mona_lisa_s65_base.png")
    p.save(base_path)
    ...
    # Applied after the skin passes.  The SSS and Vigée Le Brun passes have
    # established the warm glow and rose-ivory quality in the skin midtones;
    # this pass adds Alma-Tadema's crystalline quality to the brightest zones —
    # a cream-gold warmth in the upper-midtone translucent zone, and a cool
    # crystalline peak above specular_thresh.  At opacity=0.52 the effect is
    # present but not dominant: the face should not read as marble, only as
    # a surface with physical depth to its highlights.
    print("Alma-Tadema marble luminance pass (session 65)…")
    p.alma_tadema_marble_luminance_pass(
        marble_warm_strength = 0.07,    # gentle cream-gold body warmth
        specular_cool_shift  = 0.05,    # subtle crystalline peak cooling
        specular_thresh      = 0.88,    # only the very brightest pixels
    ...
    # ── Session 65: Crystalline surface pass (artistic improvement) ───────────
    # Applied near the end of the pipeline, after artist-specific passes and
    # before the final varnish.  At this point all the colour and form decisions
    # have been made; this pass is purely about surface quality — sharpening the
    # specular peak and adding the warm halo that makes smooth surfaces read as
    # physically real.  specular_strength=0.10 is moderate; the effect should be
    # noticeable on the forehead and cheekbones but not aggressive enough to
    # counteract the sfumato softness.
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s65_final.png")
    p.save(out_path)

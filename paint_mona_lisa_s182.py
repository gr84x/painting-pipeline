"""
paint_mona_lisa_s182.py — Mona Lisa portrait for session 182.
...
Artistic discovery (session 182): Benozzo Gozzoli (1420–1497)
  - Supreme Florentine master of jewel-bright narrative pageantry
  - Student of Fra Angelico; combined devotional palette with early Renaissance naturalism
  - Procession of the Magi chapel, Medici Palace (1459–1461): supreme jewel-bright pageant
  - Camposanto frescoes, Pisa (1468–1484): fifty-nine Old Testament scenes, vast chromatic richness
  - Defining quality: ZONE-PURE JEWEL PALETTE — lapis lazuli, vermilion, malachite, gold ochre
  - Maintained chromatic zone purity: each drapery area keyed to a single vivid pigment
  - Gold leaf halos and ornamental details; most realistic landscape of mid-Quattrocento Florence

New pipeline enhancements (session 182):
  - benozzo_gozzoli_pageant_pass: EIGHTY-FOURTH DISTINCT MODE
    FLORENTINE PAGEANT PALETTE SNAPPING:
    For each pixel (gated by symmetric tent gate in luma range [luma_lo, luma_hi]),
    finds the nearest color in Benozzo's canonical jewel palette (lapis lazuli blue,
    vermilion red, malachite green, gold ochre, gesso white, near-black, flesh ivory,
    purple-violet) using Euclidean distance in RGB space, then attracts the pixel
    toward that palette color: out_rgb = rgb + (nearest_color − rgb) × snap_strength × gate.
    NOVEL: FIRST pass to use NEAREST-PALETTE SOFT-SNAP as the per-pixel correction
    mechanism — driven by palette proximity, not channel thresholds or gradient magnitude.

  - tonal_bounded_warmth_pass: EIGHTY-FIFTH DISTINCT MODE
    IMPRIMATURA GROUND RECOVERY — TONAL-BOUNDED WARM BAND:
    MID-DARK ZONE [0.18, 0.44]: symmetric tent gate injects warm ground recovery
    (R+warm_r, G+warm_g, B+warm_b), peaking at zone centre. Replicates the warm
    imprimatura glow visible between pure void and opaque lit passages.
    NEAR-VOID ZONE (luma < void_hi): slight cool push (B+void_cool_b) deepens
    the darkest shadows toward cool umber-violet for tonal separation.
    NOVEL: FIRST pass to use a SYMMETRIC TENT GATE spanning a tonal band for
    SELECTIVE IMPRIMATURA RECOVERY with opposite treatment at the near-void floor —
    creating the three-stratum luminance structure of Renaissance toned-ground painting.
"""
...
def paint(output_path: str = "mona_lisa_s182.png") -> str:
    """Full Leonardo sfumato pipeline with session 182 Fra Bartolommeo monumental passes."""
    print("=" * 64)
    print("  Session 182 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: Benozzo Gozzoli (1420–1497)")
    print("    Florentine Pageant Quattrocento — supreme jewel-bright master")
    print("    Magi Chapel: lapis/vermilion/malachite/gold zone-pure palette")
    print("    EIGHTY-FOURTH MODE: benozzo_gozzoli_pageant_pass")
    print("    -- Nearest-palette soft-snap (first palette-proximity pass)")
    print("    EIGHTY-FIFTH MODE: tonal_bounded_warmth_pass")
    print("    -- Tent-gated mid-dark imprimatura recovery + void cool separation")
    print("=" * 64)
    ...
    print("  [6/30] Edge sfumato dissolution pass ...")
    ...
    print("  [7/30] Figure contour atmosphere pass ...")
    ...
    print("  [8/30] Skin subsurface scatter pass ...")
    ...
    print("  [9/30] Tonal envelope pass ...")
    ...
    print("  [10/30] Selective focus pass ...")
    ...
    print("  [11/30] Massys bridge glazing pass ...")
    ...
    print("  [12/30] Reynolds Grand Manner mezzotint tone pass ...")
    ...
    print("  [13/30] Schedoni luminous emergence pass ...")
    ...
    print("  [14/30] Warm highlight bloom pass ...")
    ...
    print("  [15/30] Shadow color temperature pass ...")
    ...
    print("  [16/30] Film grain overlay pass ...")
    ...
    print("  [17/30] Van der Werff ivory alabaster pass ...")
    ...
    print("  [18/30] Craquelure texture pass ...")
    ...
    print("  [19/30] Clouet enamel precision pass ...")
    ...
    print("  [20/30] Altdorfer forest atmosphere pass ...")
    ...
    print("  [21/30] Luminance gradient warmth pass ...")
    ...
    print("  [22/30] Multi-layer atmospheric veil pass ...")
    p.multilayer_atmospheric_veil_pass(opacity=0.35)

    print("  [23/30] Doggiono Leonardesque warmth pass ...")
    p.doggiono_leonardesque_warmth_pass(opacity=0.28)

    print("  [24/30] Cesare da Sesto clarity pass ...")
    p.cesare_da_sesto_clarity_pass(opacity=0.24)

    print("  [25/30] Pinturicchio jewel azure pass ...")
    p.pinturicchio_jewel_azure_pass(
        azure_delta    = 0.04,
        azure_boost    = 0.022,
        azure_r_reduce = 0.008,
        gold_delta_r   = 0.05,
        gold_delta_rg  = 0.04,
        gold_boost_r   = 0.018,
        gold_boost_g   = 0.007,
    ...
    print("  [26/30] Shadow chroma depth pass ...")
    ...
    print("  [27/30] Benozzo Gozzoli pageant palette snap pass -- EIGHTY-FOURTH DISTINCT MODE ...")
    # FLORENTINE PAGEANT PALETTE SNAPPING:
    # Inspired by Benozzo Gozzoli's zone-pure chromatic discipline in the Magi
    # Chapel — the lapis lazuli blues, vermilion reds, malachite greens, and gold
    # ochres maintained as discrete jewel zones.  Applied at moderate snap strength
    # to gently pull the sfumato landscape blues toward lapis quality and the warm
    # flesh zones toward palette ivory, without overriding the Leonardesque warmth.
    ...
    print("  [28/30] Tonal bounded warmth pass -- EIGHTY-FIFTH DISTINCT MODE ...")
    # IMPRIMATURA GROUND RECOVERY:
    # Replicates the three-stratum luminance structure of Fra Bartolommeo's
    # toned-ground panel painting: cool umber-violet in the deepest void (below
    # void_hi), warm amber glow in the mid-dark band (the imprimatura showing
    # through), and unmodified warm lit passages above.  Applied at moderate
    # opacity to deepen the chiaroscuro rhythm without overwhelming the sfumato.
    ...
    print("  [29/30] Luminance-preserving chroma boost pass ...")
    ...
    print("  [30/30] Place lights, final glaze + finish ...")
    p.place_lights(ref, stroke_size=5, n_strokes=320)
    p.glaze((0.64, 0.52, 0.22), opacity=0.042)  # warm gold-amber glaze — Gozzoli pageant warmth
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s182.png"
    paint(out)

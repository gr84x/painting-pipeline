"""
paint_mona_lisa_s76.py — Session 76 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
enhanced with session-76 additions:

  - steen_warm_vitality_pass()       — NEW (session 76) Jan Steen
                                        Dutch Golden Age / Genre Comedy (1626–1679).
                                        Introduces Steen's warm amber imprimatura
                                        vitality: golden-amber highlight lift
                                        (imprimatura glowing through thin paint),
                                        rosy warm flesh flush in the face zone,
                                        and warm amber shadow depth (no cold blacks).

  Sessions 73–75 (newly incorporated into painting pipeline):
  - anguissola_intimacy_pass()       — (session 73) Sofonisba Anguissola
                                        Lombard Renaissance psychological focus.
                                        Eye/lip sharpening + peripheral softening
                                        + warm Lombard ambient drift.
  - cool_atmospheric_recession_pass() — (session 74) Aerial perspective for landscape.
                                        Progressive colour cooling + atmospheric
                                        brightness lift + desaturation with distance.
  - de_hooch_threshold_light_pass()  — (session 75) Pieter de Hooch
                                        Warm/cool threshold light contrast.
                                        Warm amber lateral light + cool exterior
                                        doorway foil.

  Previous sessions (retained):
  - watteau_crepuscular_reverie_pass()  — (session 72)
  - correggio_golden_tenderness_pass()  — (session 71)
  - luminous_haze_pass()                — (session 71)
...
Session 76 artistic character:
  The random artist inspiration for this session is Jan Steen (1626–1679),
  the Dutch Golden Age master of genre comedy and moral painting.

  Jan Steen was born in Leiden, trained under Jan van Goyen (whose daughter
  he married), and spent his career painting the festive, chaotic, morally
  charged domestic scenes for which he became famous.  His paintings are
  theatrical, crowded, and alive — 'Jan Steen households' became a Dutch
  idiom for disorderly domestic chaos.  Yet his technique was far from
  careless: he was a gifted colourist and flesh-painter who understood how
  to use a warm amber imprimatura to create luminous, vibrant flesh tones
  that read with immediate vitality.

  His defining technical quality is *warm vitality*: every figure in a
  Steen painting appears alive, flushed, and full of warmth.  This arises
  from three interlocking choices:

  1. His warm ochre-amber imprimatura glows through thin paint in the lights,
     giving highlights a golden quality distinct from Vermeer's pearl-cool
     highlights or de Hooch's more restrained amber.

  2. His flesh midtones carry a visible rosy warmth — the result of his
     confident alla prima working into wet paint with vermilion-tinged
     flesh mixtures and his preference for lively, chromatic skin tones
     rather than the controlled glazed refinement of Vermeer.

  3. His shadows are consistently warm amber-brown (never cold or neutral).
     Even in his deepest darks the warm Vandyke-brown ground shows through,
     preventing any cool-black passages and maintaining the enveloping warmth
     of his interiors.

  The steen_warm_vitality_pass() distils these three qualities into a
  composable painting pass that reintroduces tonal warmth and flesh energy
  into the accumulated, highly refined pipeline of sessions 53–75.
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Execute the full session-76 painting pipeline and save the result.

    Returns the path to the saved PNG.
    """
    print("Session 76 — Mona Lisa portrait")
    print("Random artist: Jan Steen (Dutch Golden Age / Genre Comedy, 1626–1679)")
    print("Artistic improvement: steen_warm_vitality_pass()")
    print()
    ...
    # Applied early (before figure passes) so the landscape recedes naturally
    # before flesh and form passes warm the figure zone.
    ...
    # Applied after the broad flesh passes; sharpens psychological focus on
    # the eyes and lips while softening the peripheral flesh zones.
    ...
    # Applied after the broad warmth passes; introduces a warm/cool light
    # architecture that grounds the figure in believable interior space.
    # The warm lateral shaft reads as the upper-left light source; the cool
    # doorway foil in the background reinforces spatial depth.
    # Reduced opacity (0.38) vs. the standalone demo so it harmonises with
    # the accumulated warmth rather than dominating it.
    ...
    # ── Session 76 NEW: Jan Steen warm vitality pass ──────────────────────────
    # Applied after the temperature and duality passes so it enriches the
    # already-warm flesh rather than fighting a cold baseline.  The amber
    # highlight lift reinforces the ochre imprimatura quality; the rose flush
    # adds the lively complexion Steen's genre figures always carry; the warm
    # shadow depth ensures no cold patches survive in the final image.
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s76_final.png")
    p.save(out_path)

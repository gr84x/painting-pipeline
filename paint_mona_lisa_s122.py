"""
paint_mona_lisa_s122.py -- Session 122 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–122 additions cumulatively.

Session 122 additions:
  - annibale_carracci                        -- NEW (session 122)
                                               Annibale Carracci
                                               (1560–1609), Italian (Bolognese).
                                               Co-founder of the Accademia degli
                                               Incamminati in Bologna with his
                                               brother Agostino and cousin
                                               Ludovico — the first formal
                                               teaching academy in European art.
                                               The central figure of the
                                               Bolognese reform: a systematic
                                               rejection of late Mannerist
                                               artificiality in favour of direct
                                               observation of nature.  His warm,
                                               golden flesh tones, built on a
                                               sienna imprimatura, synthesise
                                               Correggio's warmth, Raphael's
                                               clarity, and Titian's colour —
                                               and trained the generation
                                               (Reni, Domenichino, Albani) that
                                               defined the Baroque.  Famous works
                                               include the Farnese Gallery ceiling
                                               frescoes (1597–1602) and the
                                               intimate self-portrait on an easel
                                               (c. 1604).

  SESSION 122 ARTISTIC IMPROVEMENT -- annibale_carracci_tonal_reform_pass:

                                               New pass encoding Carracci's
                                               defining technical contribution:
                                               the spatially-varying directional
                                               tonal temperature field.

                                               Unlike all previous passes in this
                                               pipeline (which use only luminance
                                               *value* to drive colour shifts), this
                                               pass computes the luminance *gradient*
                                               via Sobel operators — a 2-D vector
                                               field describing the direction of
                                               greatest luminance change at each
                                               pixel.  The dot product between the
                                               normalised gradient direction and a
                                               user-specified light direction vector
                                               produces a signed temperature field
                                               in [-1, 1]:

                                               Positive (gradient → light) = lit face
                                                 → warm shift: R+, G+, B−

                                               Negative (gradient ← light) = shadow
                                                 → cool shift: R−, G+, B+

                                               The field is restricted to the
                                               penumbra luminance zone (0.22–0.72)
                                               and smoothed, creating physically
                                               motivated warm/cool temperature
                                               contrast.  Also: warm shadow glow
                                               (sienna imprimatura through glazes)
                                               and highlight ivory lift.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s122 (Annibale Carracci directional tonal temperature field -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–122 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 122 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Lorenzo Lotto chromatic vibration (120) +", flush=True)
    print("          Giovanni Boldini dual-angle swirl bravura (121) +", flush=True)
    print("          Annibale Carracci tonal reform (122 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 122: Annibale Carracci directional tonal temperature field (NEW) ─
    # The defining innovation of Bolognese naturalism: a physically motivated
    # warm-light / cool-shadow temperature field derived from the luminance gradient.
    #
    # Unlike all previous passes (which use only luminance VALUE to drive colour
    # adjustments), this pass computes the luminance GRADIENT via Sobel operators —
    # a 2-D vector that describes the direction of greatest luminance change at
    # each pixel.  The dot product with the light direction vector yields a signed
    # temperature field: +1 on lit faces (warm amber shift), -1 on shadow faces
    # (cool blue-violet shift).  Restricted to the penumbra zone and smoothed.
    #
    # Three stages:
    # 1. Directional temperature field (Sobel gradient × light direction).
    # 2. Warm shadow glow: sienna imprimatura warmth glowing through shadow glazes.
    # 3. Highlight ivory lift: naturalistic warm ivory peak (not cold blue-white).
    print("Annibale Carracci directional tonal temperature field (session 122 -- NEW)...", flush=True)
    p.annibale_carracci_tonal_reform_pass(
        light_angle_deg = 135.0,   # upper-left — traditional Old Master light direction
        warm_r          = 0.022,
        warm_g          = 0.010,
        warm_b          = 0.014,   # de-cool the warm face
        cool_r          = 0.012,   # de-warm the shadow face
        cool_g          = 0.005,
        cool_b          = 0.022,   # blue-violet atmospheric shadow
        penumbra_lo     = 0.22,
        penumbra_hi     = 0.72,
        shadow_hi       = 0.28,
        shadow_warm_r   = 0.016,
    ...
    # Final glaze — warm sienna-amber (Carracci's Bolognese ground warmth)
    print("Final glaze (warm sienna-amber for Carracci's Bolognese ground warmth)...", flush=True)
    p.glaze((0.54, 0.40, 0.20), opacity=0.013)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.52, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s122.png")
    p.save(out_path)

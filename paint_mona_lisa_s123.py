"""
paint_mona_lisa_s123.py -- Session 123 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–123 additions cumulatively.

Session 123 additions:
  - salvator_rosa                            -- NEW (session 123)
                                               Salvator Rosa
                                               (1615–1673), Italian
                                               (Neapolitan/Roman).
                                               The most deliberately
                                               unconventional personality
                                               in 17th-century European
                                               art — painter, poet,
                                               musician, and actor who
                                               refused all court patronage
                                               and built a career on
                                               studied defiance of academic
                                               convention.  His wild,
                                               storm-charged wilderness
                                               landscapes with armed
                                               banditti and brooding
                                               allegorical figures were a
                                               cult obsession for the
                                               Romantics a century later.
                                               Near-black umber grounds;
                                               gestural, turbulent
                                               brushwork; atmospheric
                                               darkness pressing in from
                                               every edge.

  SESSION 123 ARTISTIC IMPROVEMENT -- salvator_rosa_wild_bravura_pass:

                                               New pass encoding Rosa's
                                               defining technical quality:
                                               the multi-scale turbulent
                                               displacement field.

                                               All previous passes in this
                                               pipeline modify pixel COLOUR
                                               VALUES in place (additive
                                               blends, temperature shifts,
                                               luminance lifts).  This pass
                                               introduces a fundamentally
                                               different operation: spatial
                                               IMAGE WARPING via a stochastic
                                               displacement field.

                                               N_OCTAVES octaves of
                                               Gaussian-filtered random
                                               noise are summed with 1/f
                                               pink-noise weighting.
                                               Orthogonal displacement
                                               components (dx, dy) are
                                               derived from the field
                                               gradient and used to warp
                                               the canvas via
                                               scipy.ndimage.map_coordinates.

                                               At max_disp ≈ 2px the
                                               effect is subtle but
                                               pervasive: the canvas
                                               feels energetically unstable,
                                               as if the paint were pushed
                                               by invisible gestural forces.
                                               This cannot be achieved by
                                               any colour-space operation.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s123 (Salvator Rosa turbulent displacement field -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–123 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 123 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Salvator Rosa turbulent displacement (123 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 123: Salvator Rosa turbulent displacement field (NEW) ──────────
    # The defining innovation: the first pass in this pipeline to use spatial
    # image warping (displacement) rather than per-pixel colour modification.
    #
    # A multi-scale turbulence field is built from summed octaves of Gaussian-
    # filtered noise with 1/f pink-noise weighting.  Orthogonal displacement
    # components (dx, dy) are derived from the gradient of the field, scaled
    # to max_disp pixels, and used to warp the canvas via map_coordinates.
    # At max_disp ≈ 2px the effect is subtle but pervasive: the canvas feels
    # energetically unstable, animated by invisible gestural forces.
    #
    # Additional stages:
    # - Warm raw umber shadow glow: Rosa's near-black umber canvas glowing
    #   through thin transparent shadow paint.
    # - Dark atmospheric vignette: Rosa's characteristic storm-charged
    #   darkness pressing inward from all edges.
    print("Salvator Rosa turbulent displacement field (session 123 -- NEW)...", flush=True)
    p.salvator_rosa_wild_bravura_pass(
        n_octaves      = 4,
        max_disp       = 1.8,      # subtle displacement — enough for gestural energy,
        #                            below the threshold of obvious distortion
        shadow_hi      = 0.30,
        shadow_warm_r  = 0.016,    # raw umber warmth in deep shadows
        shadow_warm_g  = 0.007,
        vignette_str   = 0.10,     # gentle atmospheric darkening at borders
        vignette_sigma = 0.32,
        blur_radius    = 4.0,
        opacity        = 0.24,
    )
    ...
    # Final glaze — deep raw umber (Rosa's near-black umber canvas ground warmth)
    print("Final glaze (deep warm umber for Rosa's dark ground tonality)...", flush=True)
    p.glaze((0.38, 0.26, 0.12), opacity=0.010)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.55, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s123.png")
    p.save(out_path)

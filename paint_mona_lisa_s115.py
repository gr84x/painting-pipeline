"""
paint_mona_lisa_s115.py -- Session 115 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–115 additions cumulatively.

Session 115 additions:
  - lavinia_fontana                          -- NEW (session 115)
                                               Lavinia Fontana
                                               (1552–1614), Italian
                                               Bolognese Mannerism /
                                               Post-Tridentine Portrait
                                               Realism.
                                               First woman in Western
                                               history to establish
                                               herself as a fully
                                               professional artist;
                                               daughter of the Bolognese
                                               painter Prospero Fontana;
                                               trained in the same
                                               rigorous Bolognese
                                               workshop environment that
                                               would produce the Carracci
                                               academy within a generation.
                                               Received a papal invitation
                                               to Rome from Clement VIII;
                                               her work entered royal and
                                               papal collections across
                                               Europe.  Defining quality:
                                               rich deep crimson velvet
                                               costume — a warm, saturated
                                               darkness with recoverable
                                               crimson depth, entirely
                                               unlike Bronzino's cold
                                               black or Caravaggio's void;
                                               warm rose-ivory flesh
                                               luminosity (Bolognese
                                               tradition); amber warmth
                                               retained even in deepest
                                               shadow zones — the
                                               inhabited warmth that
                                               distinguishes Bolognese
                                               portraiture.

  SESSION 115 ARTISTIC IMPROVEMENT -- fontana_jewel_costume_pass:

                                               New pass encoding three
                                               defining Fontana qualities:
                                               (1) Highlight warm
                                               brilliance — warm rose-
                                               ivory luminosity at peak
                                               flesh (lum > 0.70), with
                                               Gaussian-blurred mask for
                                               soft, natural edge; the
                                               Bolognese warmth quality —
                                               warmer than Moroni's cool
                                               silver, warmer than
                                               Boltraffio's pearl, but
                                               less amber than Strozzi;
                                               (2) Costume crimson depth —
                                               raised-cosine enrichment in
                                               the dark costume zone
                                               [0.18, 0.46]: R + crimson_r,
                                               G + crimson_g (tiny),
                                               B - crimson_b (prevents
                                               purple drift).  Encodes the
                                               warm, saturated darkness of
                                               Fontana's deep crimson
                                               velvet — the quality that
                                               gives her costume zones
                                               material richness;
                                               (3) Shadow amber warmth —
                                               warm amber retention in
                                               deepest shadow zone
                                               (lum < 0.22): R + amber_r,
                                               G + amber_g.  Bolognese
                                               painters never go cold in
                                               the shadows; even near-
                                               black void retains
                                               recoverable amber warmth.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s115 (Lavinia Fontana jewel costume -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–115 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 115 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Lavinia Fontana jewel costume (115 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 115: Lavinia Fontana jewel costume pass (NEW) ─────────────────
    # "Bolognese warmth" — Fontana's costume richness and warm flesh luminosity.
    # Applied as a refinement layer after the accumulated Baroque and Italian passes:
    #
    # (1) Highlight warm brilliance — Fontana's rose-ivory flesh warmth at peak
    #     highlights (hi_lo=0.70).  After Furini's ivory bloom and Cagnacci's rose
    #     highlights, Fontana adds a characteristically Bolognese quality: a warm,
    #     present ivory that is less cool than Moroni's silver, less pearled than
    #     Boltraffio, warmer than Furini's near-neutral ivory — a specific Bolognese
    #     warmth that reads as lived-in and natural rather than idealised.
    #
    # (2) Costume crimson depth — raised-cosine enrichment in the dark costume zone
    #     [0.18, 0.46].  After the accumulated passes, the dark zones have a warm
    #     amber-brown quality (Strozzi, Guercino, Jordaens).  Fontana's crimson_r
    #     (0.018) and crimson_b reduction (-0.006) introduces the deep crimson
    #     velvet richness that is her most distinctive signature: the dark zones
    #     gain a warm, saturated depth, as if glazed with vermilion and lake over
    #     warm umber — fundamentally different from the flat black of Bronzino or
    #     the void of Caravaggio.
    #
    # (3) Shadow amber warmth — amber_r (0.012), amber_g (0.005) in zones below
    #     lum=0.22.  Bolognese painters never surrender warmth in the shadows; the
    #     near-black void retains a recoverable amber that makes portraits feel
    #     inhabited.  After Furini's slight shadow cool (shadow_r=0.010), Fontana's
    #     warmth works as a partial counter-correction in the deepest zones, where
    #     her Bolognese tradition reclaims warmth from Furini's Florentine cool.
    print("Lavinia Fontana jewel costume pass (session 115 -- NEW)...", flush=True)
    ...
    # Final glaze — warm amber-rose reflecting Bolognese warmth tradition
    print("Final glaze (warm amber-rose for Bolognese warmth)...", flush=True)
    p.glaze((0.62, 0.46, 0.20), opacity=0.026)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.52, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s115.png")
    p.save(out_path)

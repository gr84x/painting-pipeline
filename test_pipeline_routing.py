...
# ── Session 218: Oskar Kokoschka — kokoschka_anxious_portrait_pass ───────────

def test_s218_kokoschka_pass_changes_pixels():
    """Session 218: kokoschka_anxious_portrait_pass must change canvas pixels."""
    import numpy as _np

    def _make_painter():
        p = _make_small_painter(64, 64)
        rng = _np.random.RandomState(218)
        buf = _np.frombuffer(
            p.canvas.surface.get_data(), dtype=_np.uint8).reshape((64, 64, 4)).copy()
        buf[:, :, :3] = rng.randint(50, 180, (64, 64, 3), dtype=_np.uint8)
        buf[:, :, 3]  = 255
    ...
    before = _np.frombuffer(
        p.canvas.surface.get_data(), dtype=_np.uint8).reshape((64, 64, 4)).copy()
    p.kokoschka_anxious_portrait_pass(opacity=0.80)
    after = _np.frombuffer(
        p.canvas.surface.get_data(), dtype=_np.uint8).reshape((64, 64, 4)).copy()
    assert not _np.array_equal(before, after), (
        "kokoschka_anxious_portrait_pass must change canvas pixels")


def test_s218_kokoschka_pass_zero_opacity_no_change():
    """Session 218: kokoschka_anxious_portrait_pass at opacity=0.0 must not change pixels."""
    import numpy as _np

    def _make_painter():
        p = _make_small_painter(64, 64)
        rng = _np.random.RandomState(2180)
        buf = _np.frombuffer(
            p.canvas.surface.get_data(), dtype=_np.uint8).reshape((64, 64, 4)).copy()
        buf[:, :, :3] = rng.randint(20, 200, (64, 64, 3), dtype=_np.uint8)
        buf[:, :, 3]  = 255
    ...
    before = _np.frombuffer(
        p.canvas.surface.get_data(), dtype=_np.uint8).reshape((64, 64, 4)).copy()
    p.kokoschka_anxious_portrait_pass(opacity=0.0)
    after = _np.frombuffer(
        p.canvas.surface.get_data(), dtype=_np.uint8).reshape((64, 64, 4)).copy()
    assert _np.array_equal(before, after), (
        "kokoschka_anxious_portrait_pass at opacity=0.0 must not change any pixels")


def test_s218_kokoschka_in_catalog():
    """Session 218: oskar_kokoschka must appear in CATALOG with correct movement."""
    from art_catalog import CATALOG, get_style
    assert "oskar_kokoschka" in CATALOG, "oskar_kokoschka missing from CATALOG"
    s = get_style("oskar_kokoschka")
    assert "Expressionism" in s.movement, (
        f"oskar_kokoschka movement unexpected: {s.movement!r}")

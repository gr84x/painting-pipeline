"""
paint_mona_lisa_s92.py -- Session 92 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-92 additions:

  - antonello_pellucid_flesh_pass()                   -- NEW (session 92) Antonello da Messina.
                                                         Three-stage Sicilian Renaissance technique:
                                                         (1) Flemish shadow cooling -- in mid-shadow
                                                         transitions (lum 0.18–0.46), a slight blue-
                                                         green subsurface tint is applied (G+, B+,
                                                         R-), replicating the Flemish translucent-
                                                         skin quality that Antonello brought from
                                                         Jan van Eyck to Italy; this is the 'pellucid'
                                                         quality: you sense light passing *through*
                                                         the paint, not merely reflecting from its
                                                         surface; (2) warm highlight clarification --
                                                         in lit zones (lum > 0.74), a push toward
                                                         Naples yellow-ivory (R+, G+, B-) reinforces
                                                         Antonello's warm, small, enamel-crisp
                                                         specular highlights; (3) pellucid edge band
                                                         -- in the light-shadow transition zone
                                                         (lum 0.44–0.58), a subtle contrast push
                                                         sharpens the tonal gradient without hard-
                                                         ening the edge, creating Antonello's
                                                         characteristic 'found-edge' -- the boundary
                                                         reads as resolved and precise but not
                                                         mechanical, unlike the hard Italian contour
                                                         or Leonardo's atmospheric sfumato.

  - Improved subsurface_scatter_pass()                -- SESSION 92 ARTISTIC IMPROVEMENT.
    with shadow_pellucidity=0.05                        Shadow pellucidity: Antonello and the
                                                         Flemish masters worked on white-gesso
                                                         panel grounds with multiple transparent
                                                         oil-glaze layers.  Even in the deepest
                                                         shadows, the white ground exerts a faint
                                                         luminous presence through the paint film --
                                                         the paint is never completely opaque.
                                                         The new shadow_pellucidity parameter in
                                                         subsurface_scatter_pass() applies a very
                                                         faint warm-neutral lift to the absolute
                                                         darkest pixels (lum < 0.18), simulating
                                                         this Flemish gesso-ground glow.  Unlike
                                                         the existing shadow_cool parameter (which
                                                         cools all darks below 0.28), shadow_
                                                         pellucidity targets only the innermost
                                                         darks and lifts them with a warm-neutral
                                                         rather than cold tint -- the shadows
                                                         remain dark but are no longer dead.
                                                         Applied at 0.05: barely perceptible,
                                                         but cumulative with the transparent-
                                                         glazing quality built up by twenty-plus
                                                         prior passes.

  Sessions 91 and earlier (retained):
...
Session 92 artistic character:
  The random artist for this session is Antonello da Messina (c. 1430–1479),
  the Sicilian painter who occupies a singular pivot-point in the history of
  European portraiture.  Born in Messina, trained in Naples under Colantonio
  (who had direct access to the Flemish oil technique through Alfonso V of
  Aragon's collection), Antonello brought Jan van Eyck's oil-glazing method
  to Italy decades before the technique became standard there.  When he
  arrived in Venice in 1475–76 he transformed Italian portraiture: the frontal
  gaze that Flemish painters had mastered but Italian artists had avoided as
  too confrontational became, under Antonello, not a challenge but an
  invitation — the sitter meets the viewer across centuries with calm, direct
  clarity.

  The 'pellucid' quality of Antonello's flesh — the sense that you are
  looking through transparent layers of paint at something with genuine
  interior luminosity — is achieved through two sources working together:
  the Flemish practice of building colour in stacked transparent oil glazes
  on a white-gesso ground (so light passes through the layers rather than
  reflecting from an opaque surface), and the Italian tradition of warm
  sienna-to-ivory flesh tones in the lit zones.  The distinctive coolness
  in his mid-shadow transitions — a slight blue-green tint that reads as
  translucent skin — is the Flemish subsurface quality in Italian dress.

  In the context of this Mona Lisa portrait, Antonello's contribution is
  pellucid flesh depth.  By session 91, the painting has accumulated warm
  golden glazing from Caillebotte, Correggio, Andrea del Sarto, and the
  varnish pass — all appropriate for Leonardo's warm chiaroscuro.  But the
  flesh shadows risk becoming opaque and flat: warm, correctly coloured,
  but lacking the sense of interior light that Leonardo's own multiple-
  glaze technique would have built up.  The antonello_pellucid_flesh_pass()
  restores that translucent depth in the shadow transitions, and the
  shadow_pellucidity improvement in subsurface_scatter_pass() ensures that
  even the deepest darks retain the faint presence of the white ground
  glowing through the paint.

  These are quiet additions — not transformative like a new colour pass,
  but cumulative in the way that Antonello's own technique was cumulative:
  each transparent layer adding almost nothing on its own, but the sum
  producing the luminosity that has made his portraits unforgettable for
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-92 painting pipeline."""
    print("Session 92 -- Mona Lisa portrait")
    print("Random artist: Antonello da Messina (Sicilian Renaissance / Flemish-Italian Bridge, c. 1430-1479)")
    print("New pass: antonello_pellucid_flesh_pass()")
    print("  -- Flemish shadow cooling: blue-green subsurface tint in mid-shadow transitions")
    print("  -- warm highlight clarification: Naples yellow-ivory push in lit zones")
    print("  -- pellucid edge band: found-edge contrast in the light-shadow transition zone")
    print()
    print("Session 92 artistic improvement: shadow_pellucidity in subsurface_scatter_pass()")
    print("  Flemish gesso-ground luminosity: a faint warm-neutral lift in absolute darks (lum < 0.18)")
    print("  simulates the way multiple transparent oil-glaze layers never fully occlude the white")
    print("  gesso ground -- even the deepest shadows retain a faint inner luminosity.")
    print()
    ...
    # Antonello/Flemish gesso-ground glow: faint warm-neutral lift in absolute darks (lum < 0.18)
    ...
    # ── SESSION 92: Antonello da Messina pellucid flesh pass (NEW) ────────────
    # Antonello da Messina (c. 1430-1479) -- Sicilian Renaissance / Flemish-Italian Bridge.
    # Three operations: Flemish shadow cooling (blue-green subsurface tint in mid-shadows),
    # warm highlight clarification (Naples yellow-ivory push in lit zones), and pellucid
    # edge band (found-edge contrast sharpening in the light-shadow transition zone).
    # Applied at opacity=0.38 -- a moderate contribution that adds translucent depth
    # to the flesh without conflicting with the warm golden glazing chain already built.
    print("Antonello da Messina pellucid flesh pass (session 92 -- NEW)...")
    p.antonello_pellucid_flesh_pass(
        shadow_cool_lo      = 0.18,   # lower bound of shadow cooling zone
        shadow_cool_hi      = 0.46,   # upper bound of shadow cooling zone
        shadow_green_lift   = 0.018,  # G+ in shadow transitions (Flemish cool-green)
        shadow_blue_lift    = 0.028,  # B+ in shadow transitions (Flemish subsurface)
        shadow_red_damp     = 0.010,  # R- in shadow transitions
        highlight_lo        = 0.74,   # threshold for warm highlight zone
        highlight_warm_r    = 0.022,  # Naples yellow-ivory R boost
        highlight_warm_g    = 0.012,  # Naples yellow-ivory G boost
        highlight_cool_b    = 0.008,  # slight B reduction (warm ivory, not cold)
        penumbra_lo         = 0.44,   # lower lum of pellucid edge band
        penumbra_hi         = 0.58,   # upper lum of pellucid edge band
        penumbra_contrast   = 0.06,   # found-edge contrast push
        blur_radius         = 5.0,
        opacity             = 0.38,
    )
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s92.png")
    p.save(out_path)

"""
paint_mona_lisa_s80.py -- Session 80 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-80 addition:

  - andrea_del_sarto_golden_sfumato_pass()  -- NEW (session 80) Andrea del Sarto
                                               Florentine High Renaissance.
                                               The 'faultless painter': warm golden
                                               midtone warmth (amber-ivory push in the
                                               midtone flesh band), sfumato edge
                                               feathering at form boundaries (gradient-
                                               detected, Gaussian-blended), and colour
                                               harmony pull (desaturation toward the warm
                                               Florentine centre to prevent chromatic
                                               outliers from preceding passes).
                                               Applied at opacity 0.46 -- present but
                                               never intrusive; del Sarto's 'faultless'
                                               quality is accumulative subtlety.

  Sessions 79 and earlier (retained):
...
Session 80 artistic character:
  The random artist inspiration for this session is Andrea del Sarto (1486–1530),
  called 'il pittore sanza errori' — the faultless painter — by Giorgio Vasari.

  Del Sarto occupied a singular position in Florentine High Renaissance painting:
  he synthesised the two dominant traditions of his moment — Fra Bartolommeo's
  monumental architectural clarity and Leonardo da Vinci's atmospheric sfumato —
  into a style that Vasari considered technically superior to both.  Where
  Leonardo's sfumato dissolved forms into a cool, mysterious smoke, del Sarto's
  version retained the underlying solidity of volume — a warm, golden haze rather
  than Leonardo's grey-blue dissolve.

  His flesh tones are among the most admired in the Italian tradition.  The
  Portrait of a Young Man (c. 1517) demonstrates the characteristic quality:
  warm ivory highlight deepening through golden midtone to warm umber shadow, with
  no visible seam between the tonal registers, no cold-light / warm-shadow break,
  and no decorative flourish — just the inevitable progression of warm, living skin
  rendered with absolute control.  Vasari's phrase 'without error' means precisely
  this: not the absence of risk but the absence of visible effort.

  The andrea_del_sarto_golden_sfumato_pass() introduces three of his defining
  qualities:

  1. Golden midtone warmth (gold_r_boost=0.05, gold_g_boost=0.028) -- skin pixels
     in the warm midtone band [0.45–0.78] receive an R↑ and G↑ push in a warm
     amber ratio (R:G ≈ 1.8:1).  The tent mask concentrates the effect at the
     true midtone rather than highlights or shadows, preserving the luminance
     structure built by preceding sessions while warming the temperature register.

  2. Sfumato edge feathering (sfumato_blur=5.5, edge_grad) -- luminance gradient
     magnitude detects form boundaries that have become too hard across the
     accumulated passes.  These transition zones receive a weighted Gaussian
     softening (blend_w = edge_weight × opacity × 0.55), pulling hard edges toward
     del Sarto's Leonardo-adjacent atmospheric quality.  The gradient weighting
     ensures only actual edges are softened; smooth interior modelling is preserved.

  3. Colour harmony pull (harmony_sat_cap=0.85, harmony_pull=0.055) -- pixels
     whose saturation exceeds the cap are mildly desaturated toward the warm
     Florentine centre (del Sarto's ivory-gold midtone, R=0.82/G=0.72/B=0.55).
     This prevents chromatic outliers from the Lotto cool-anxiety and Poussin
     cool-clarity passes from fighting the warm Florentine register.

  Opacity 0.46 -- del Sarto's effect is never dramatic.  The 'faultless' quality
  comes from what the viewer does not notice: transitions that feel inevitable,
  warmth that feels innate to the subject rather than applied by the painter.
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Execute the full session-80 painting pipeline and save the result.

    Returns the path to the saved PNG.
    """
    print("Session 80 -- Mona Lisa portrait")
    print("Random artist: Andrea del Sarto (Florentine High Renaissance, c. 1486-1530)")
    print("Artistic improvement: andrea_del_sarto_golden_sfumato_pass()")
    print("  -- golden midtone warmth + sfumato edge feathering + colour harmony pull")
    print()
    ...
    # Session 80 NEW: Andrea del Sarto golden sfumato pass
    # Applied after the Lotto cool anxiety and before the finishing varnish.
    # The golden midtone warmth (R↑ and G↑ in the warm midtone flesh band)
    # re-asserts the Florentine amber-ivory after Lotto's cool undertone,
    # without cancelling the psychological tension -- just warming the mid-register.
    # The sfumato edge feathering dissolves the hard edges that accumulate across
    # sessions, pulling form boundaries toward del Sarto's Leonardo-adjacent
    # atmospheric transitions.  The colour harmony pull desaturates chromatic
    # outliers from the Lotto and Poussin passes toward the warm Florentine centre.
    # Opacity 0.46 -- del Sarto's 'faultless' quality comes from accumulative
    # subtlety; the result should feel inevitable rather than applied.
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s80.png")
    p.save(out_path)

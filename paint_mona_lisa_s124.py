"""
paint_mona_lisa_s124.py -- Session 124 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–124 additions cumulatively.

Session 124 additions:
  - massimo_stanzione                        -- NEW (session 124)
                                               Massimo Stanzione
                                               (c. 1585–1656), Italian
                                               (Neapolitan).
                                               "Il Cavalier Calabrese" —
                                               the dominant master of
                                               Neapolitan Baroque painting
                                               between Caravaggio's harsh
                                               tenebrism and Guido Reni's
                                               luminous classicism.
                                               Sometimes called "the
                                               Neapolitan Guido Reni":
                                               warm golden-ivory flesh,
                                               cool violet shadow penumbra,
                                               Bolognese academic smoothness
                                               applied to Neapolitan warmth.

  SESSION 124 ARTISTIC IMPROVEMENT -- stanzione_noble_repose_pass:

                                               New pass encoding Stanzione's
                                               defining technical quality:
                                               the Laplacian pyramid
                                               multi-scale clarity pass.

                                               All previous passes in this
                                               pipeline operate in one of
                                               two modes: colour-space
                                               transforms (additive RGB
                                               blends, temperature shifts,
                                               luminance lifts) or
                                               spatial-domain transforms
                                               (image warping via
                                               displacement fields, s123).

                                               This pass introduces a third,
                                               entirely new mode:
                                               FREQUENCY-BAND DECOMPOSITION
                                               via a stationary Laplacian
                                               pyramid.

                                               The canvas is decomposed into
                                               four additive frequency bands:
                                               L0 = orig - Gauss(σ=2)  (fine detail)
                                               L1 = Gauss(σ=2) - Gauss(σ=4) (mid-freq)
                                               L2 = Gauss(σ=4) - Gauss(σ=8) (coarse)
                                               L3 = Gauss(σ=8) (global base)

                                               L1 is selectively boosted to
                                               enhance mid-frequency facial
                                               structure; L0 is gently
                                               suppressed to reduce fine
                                               noise accumulation.
                                               Reconstruction: L0' + L1' + L2 + L3.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s124 (Massimo Stanzione Laplacian pyramid clarity -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–124 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 124 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Massimo Stanzione Laplacian pyramid clarity (124 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 124: Massimo Stanzione Laplacian pyramid clarity (NEW) ─────────
    # The defining innovation: the first pass in this pipeline to use frequency-band
    # decomposition via a stationary Laplacian pyramid.
    #
    # The canvas is decomposed into four additive frequency bands:
    #   L0 = orig − Gauss(σ=2)        finest detail (~2 px scale)
    #   L1 = Gauss(σ=2) − Gauss(σ=4) mid-frequency facial structure (~4 px)
    #   L2 = Gauss(σ=4) − Gauss(σ=8) coarse tonal planes (~8 px)
    #   L3 = Gauss(σ=8)               global tonal base
    #
    # L1 is selectively boosted to enhance mid-frequency structural clarity
    # (facial planes, cheekbone volumes, chin form).  L0 is gently suppressed
    # to reduce fine noise that has accumulated across 22 previous painting passes.
    # Reconstruction: L0' + L1' + L2 + L3.
    #
    # Additional colour stages:
    # - Warm ivory highlight lift: Stanzione's Reni-derived golden ivory lights.
    # - Cool violet shadow penumbra: Caravaggist depth softened by classicist reflected light.
    print("Massimo Stanzione Laplacian pyramid clarity (session 124 -- NEW)...", flush=True)
    p.stanzione_noble_repose_pass(
        pyramid_levels  = 4,
        mid_freq_boost  = 0.10,    # gentle boost of mid-frequency facial structure
        fine_suppress   = 0.08,    # gentle suppression of finest noise accumulation
        hi_lo           = 0.70,
        ivory_r         = 0.014,   # warm ivory in highlights (Reni-derived)
        ivory_g         = 0.008,
        penumbra_lo     = 0.24,
        penumbra_hi     = 0.58,
        violet_b        = 0.018,   # cool violet in shadow penumbra (Caravaggist depth)
        violet_r        = 0.009,
        blur_radius     = 4.0,
        opacity         = 0.30,
    )
    ...
    # Final glaze — warm amber (Stanzione's Bolognese-derived warm amber ground)
    print("Final glaze (warm amber for Stanzione's Bolognese ground tonality)...", flush=True)
    p.glaze((0.44, 0.32, 0.16), opacity=0.008)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.55, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s124.png")
    p.save(out_path)

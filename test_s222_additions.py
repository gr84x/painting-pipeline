"""
test_s222_additions.py — Session 222 tests for schjerfbeck_sparse_portrait_pass,
...
# ──────────────────────────────────────────────────────────────────────────────
# schjerfbeck_sparse_portrait_pass — session 222 artist pass (133rd mode)
# ──────────────────────────────────────────────────────────────────────────────

def test_s222_schjerfbeck_pass_exists():
    """Session 222: Painter must have schjerfbeck_sparse_portrait_pass method."""
    from stroke_engine import Painter
...
def test_s222_schjerfbeck_pass_no_error():
    """Session 222: schjerfbeck_sparse_portrait_pass runs without error on a toned canvas."""
...
def test_s222_schjerfbeck_pass_changes_canvas():
    """Session 222: schjerfbeck_sparse_portrait_pass must modify a non-uniform canvas."""
    p   = _make_small_painter(64, 64)
    ref = _solid_reference(64, 64)
...
def test_s222_schjerfbeck_pass_zero_opacity_no_change():
    """Session 222: schjerfbeck_sparse_portrait_pass at opacity=0.0 must not change pixels."""
    p   = _make_small_painter(64, 64)
    ref = _solid_reference(64, 64)
...
def test_s222_schjerfbeck_cool_wash_cools_bright_pixels():
    """Session 222: cool_shift must raise blue channel in bright regions."""
    p = _make_small_painter(64, 64)
...
def test_s222_helene_schjerfbeck_in_catalog():
    """Session 222: helene_schjerfbeck must appear in CATALOG with correct properties."""
    ...
    assert "133" in s.inspiration or "THIRTY-THIRD" in s.inspiration, (
        "helene_schjerfbeck inspiration must reference 133rd mode")


# ──────────────────────────────────────────────────────────────────────────────
# halation_glow_pass — session 222 artistic improvement
# ──────────────────────────────────────────────────────────────────────────────

def test_s222_halation_glow_pass_exists():
    """Session 222: Painter must have halation_glow_pass method."""
    from stroke_engine import Painter
...
def test_s222_halation_glow_pass_no_error():
    """Session 222: halation_glow_pass runs without error on a toned canvas."""
    p   = _make_small_painter(64, 64)
    ref = _solid_reference(64, 64)
...
def test_s222_halation_glow_pass_does_not_darken():
    """Session 222: halation_glow_pass must not darken any pixel (only adds bloom)."""
    p   = _make_small_painter(64, 64)
    ref = _solid_reference(64, 64)
    p.tone_ground((0.60, 0.55, 0.40), texture_strength=0.05)
    ...
    # At opacity=1.0 the composite is: orig*(0) + glow_result*(1) = glow_result.
    # Since glow only adds bloom >= 0, the result must be >= original everywhere.
    # Use channels 0-2 (BGR); channel 3 is alpha, always 255.
...
def test_s222_halation_glow_pass_zero_opacity_no_change():
    """Session 222: halation_glow_pass at opacity=0.0 must not change pixels."""
    p   = _make_small_painter(64, 64)
    ref = _solid_reference(64, 64)

"""
paint_mona_lisa_s183.py — Mona Lisa portrait for session 183.
...
Artistic discovery (session 183): Elisabetta Sirani (1638–1665)
  - Supreme Bolognese Feminine Baroque master; first woman to run a painting academy
  - Student tradition of Guido Reni; golden Reniesque warmth with rosy personal inflection
  - Judith with the Head of Holofernes (1658), Porcia Wounding Her Thigh (1664)
  - Defining quality: CONTEXTUAL WARM FIELD — golden Bolognese ground warmth permeates
    every zone, arising from warm imprimatura and Reniesque glazing tradition
  - Rosy-luminous flesh: warmer midtones than Reni, lighter highlights, soft Bolognese sfumato

New pipeline enhancements (session 183):
  - sirani_contextual_warmth_pass: EIGHTY-SIXTH DISTINCT MODE
    SPATIAL NEIGHBOURHOOD FIELD WARMTH AMPLIFICATION:
    Large-sigma Gaussian blur captures scene-scale neighbourhood colour field.
    Pixels in warm-dominant colour field zones (field_r > field_b + threshold)
    receive gentle rosy warm lift (R+ G+ B-), amplifying the contextual Bolognese
    imprimatura warmth. NOVEL: FIRST pass to use SPATIAL NEIGHBOURHOOD FIELD
    WARMTH (large-sigma blur R-B balance) as the primary gate — not the pixel's
    own luma, saturation, hue, gradient magnitude, or channel dominance, but the
    CONTEXTUAL WARM-COOL BALANCE of the surrounding colour field at scene scale.

  - adaptive_tonal_pivot_pass: EIGHTY-SEVENTH DISTINCT MODE
    SELF-LOCATING SIGMOID TONAL S-CURVE:
    Computes the MEDIAN luma of the current canvas as the adaptive pivot point.
    Applies sigmoid S-curve centred on that empirical pivot, amplifying tonal
    contrast symmetrically above and below it. NOVEL: FIRST pass to use ADAPTIVE
    S-CURVE centred on the EMPIRICALLY COMPUTED MEDIAN — self-locating at the
    image's own tonal midpoint regardless of what prior passes have done.
"""
...
def paint(output_path: str = "mona_lisa_s183.png") -> str:
    """Full Leonardo sfumato pipeline with session 183 Sirani-inspired passes."""
    print("=" * 64)
    print("  Session 183 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: Elisabetta Sirani (1638–1665)")
    print("    Bolognese Feminine Baroque — golden Reniesque warmth, rosy flesh")
    print("    Judith, Porcia, Timoclea: composed female heroism in warm Bolognese light")
    print("    EIGHTY-SIXTH MODE: sirani_contextual_warmth_pass")
    print("    -- Spatial neighbourhood field warmth (first context-field gate pass)")
    print("    EIGHTY-SEVENTH MODE: adaptive_tonal_pivot_pass")
    print("    -- Self-locating sigmoid S-curve centred on empirical canvas median")
    print("=" * 64)
    ...
    print("  [13/30] Luminance gradient warmth pass ...")
    ...
    print("  [14/30] Warm ambient occlusion pass ...")
    ...
    print("  [16/30] Multilayer atmospheric veil pass ...")
    ...
    print("  [17/30] Peripheral defocus pass ...")
    ...
    print("  [18/30] Luminance preserving chroma boost pass ...")
    ...
    print("  [19/30] Shadow chroma depth pass ...")
    ...
    print("  [20/30] Tonal bounded warmth pass [s182] ...")
    ...
    print("  [21/30] Canvas tooth texture pass ...")
    ...
    print("  [22/30] Chromatic vignette pass ...")
    ...
    print("  [23/30] Cesare da Sesto clarity pass ...")
    ...
    print("  [24/30] Benozzo Gozzoli pageant pass [s182] ...")
    ...
    print("  [25/30] *** SIRANI CONTEXTUAL WARMTH PASS [s183, 86th mode] ***")
    ...
    print("  [26/30] *** ADAPTIVE TONAL PIVOT PASS [s183, 87th mode] ***")
    ...
    print("  [27/30] Chromatic edge halation pass ...")
    ...
    print("  [28/30] Skin subsurface scatter pass (final warm glow) ...")
    p.skin_subsurface_scatter_pass(
        scatter_sigma    = 3.5,
    ...
    print("  [29/30] Sfumato veil final ...")
    p.sfumato_veil_pass(
        ref,
        n_veils                      = 4,
        blur_radius                  = 8.0,
    ...
    p.glaze((0.72, 0.62, 0.42), opacity=0.040)  # warm golden glaze — Sirani Reniesque gold
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s183.png"
    paint(out)

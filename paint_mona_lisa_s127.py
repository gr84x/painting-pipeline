...
then applies sessions 102–127 additions cumulatively.

Session 127 additions:
  - simone_cantarini                         -- NEW (session 127)
                                               Simone Cantarini
                                               (1612–1648), Italian,
                                               called 'il Pesarese'.
                                               Born in Oropezza near
                                               Pesaro; trained under
                                               Guido Reni in Bologna
                                               c. 1635–37; absorbed
                                               Reni's silver-ivory
                                               palette while developing
                                               a personal 'pearl-fog'
                                               penumbra through
                                               multi-layer smalt and
                                               lead-white glazing.
                                               His flesh tones are the
                                               most delicately iridescent
                                               in the Bolognese school.

  SESSION 127 ARTISTIC IMPROVEMENT -- cantarini_pearl_fog_pass:

                                               New pass encoding
                                               Cantarini's defining
                                               technical practice: the
                                               multi-layer spectral glaze
                                               that creates the
                                               'pearl-fog' penumbra.
                                               ...
                                               s126 Bartolommeo — edge-
                                                 map-driven selective
                                                 modulation (Sobel)

                                               This pass introduces the
                                               fifth distinct mode:
                                               SPECTRAL CHANNEL-SELECTIVE
                                               DIFFUSION — applying
                                               wavelength-dependent
                                               Gaussian blur to each
                                               RGB channel independently.

                                               σ_R = 1.2  (red travels
                                                 deepest before scatter)
                                               σ_G = 2.0  (mid-wavelength)
                                               σ_B = 3.5  (blue scatters
                                                 most at glaze surface)

                                               In the penumbra zone the
                                               blurred channels blend with
                                               the originals, creating a
                                               soft blue iridescence over
                                               warm undertones — exactly
                                               Cantarini's pearl-fog.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s126 (Fra Bartolommeo Sobel-gradient velo shadow),
         s127 (Simone Cantarini spectral channel-selective diffusion -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–127 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 127 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Fra Bartolommeo Sobel-gradient velo shadow (126) +", flush=True)
    print("          Simone Cantarini spectral channel-selective diffusion (127 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── Session 126: Fra Bartolommeo Sobel-gradient velo shadow ──────────────
    print("Fra Bartolommeo Sobel-gradient velo shadow (session 126)...", flush=True)
    p.fra_bartolommeo_velo_shadow_pass(
        ...
        mid_lo           = 0.25,
        mid_hi           = 0.70,
        saturation_boost = 0.04,
        blur_radius      = 4.0,
        opacity          = 0.30,
    )

    # ── SESSION 127: Simone Cantarini spectral channel-selective diffusion ─────
    # The defining innovation: the fifth processing mode in this pipeline.
    # All previous blur operations apply identical sigma to all three RGB channels.
    # Cantarini's multi-layer smalt/lead-white glazing creates a physically layered
    # medium where different wavelengths scatter at different depths:
    #   λ_blue ≈ 450 nm: scatters most (Rayleigh: σ ∝ λ⁻⁴)
    #   λ_red  ≈ 650 nm: scatters least (penetrates deeper before reflecting)
    # The pass models this as per-channel Gaussian blur with σ_B > σ_G > σ_R,
    # applied selectively in the penumbra zone.  The result: blue channel diffuses
    # softly across form boundaries while red stays crisp — creating the 'pearl-fog'
    # iridescence of Cantarini's finest flesh passages.
    print("Simone Cantarini spectral channel-selective diffusion (session 127 -- NEW)...", flush=True)
    p.cantarini_pearl_fog_pass(
        penumbra_lo      = 0.32,     # penumbra zone lower bound
        penumbra_hi      = 0.72,     # penumbra zone upper bound
        sigma_r          = 1.2,      # red channel blur: minimal scatter
        sigma_g          = 2.0,      # green channel blur: moderate scatter
        sigma_b          = 3.5,      # blue channel blur: maximum scatter (pearl-fog)
        scatter_strength = 0.55,     # blend weight of blurred vs original in penumbra
        rose_hi_lo       = 0.70,     # rose carnation warmth threshold
        rose_r           = 0.012,    # gentle rose-pink R lift in highlights
        rose_b_damp      = 0.005,    # slight B damp to keep carnation warm
        ivory_lo         = 0.80,     # ivory specular lift threshold
        ivory_r          = 0.010,    # ivory R lift — prevents coolness from greying
        ivory_g          = 0.007,    # ivory G lift
        blur_radius      = 4.0,
        opacity          = 0.32,
    )
    ...
    # Final glaze — warm ivory (Cantarini's pale Bolognese ground)
    print("Final glaze (warm ivory for Cantarini's Bolognese silver ground)...", flush=True)
    p.glaze((0.88, 0.82, 0.70), opacity=0.006)

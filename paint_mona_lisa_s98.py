"""
paint_mona_lisa_s98.py -- Session 98 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-98 additions:

  - federico_barocci                                        -- NEW (session 98)
                                                           Federico Barocci
                                                           (c. 1535-1612),
                                                           Urbino / Umbrian
                                                           Mannerism.
                                                           Three-stage Umbrian
                                                           petal-flush technique:
                                                           (1) Petal flush --
                                                           warm rose-pink bloom
                                                           in penumbra zone
                                                           [lum 0.42-0.62],
                                                           ...
                                                           (2) Bianca ground
                                                           luminosity -- warm
                                                           creamy lift in upper
                                                           mid-tones [lum
                                                           0.58-0.82], simulating
                                                           the pale warm ground
                                                           reading through thin
                                                           transparent glazes;
                                                           (3) Warm perimeter
                                                           dissolution -- figure
                                                           edges dissolve into
  ...
  SESSION 98 ARTISTIC IMPROVEMENT -- penumbra as first-class bloom target:
                                                           The barocci_petal_flush_pass()
                                                           introduces a new tonal
                                                           concept to the pipeline:
                                                           the penumbra bloom.
                                                           Prior sessions have
                                                           treated highlights
                                                           (session 83) and
                                                           shadows (sessions 93,
                                                           95) as zones of special
                                                           attention, and session
                                                           96 animated the broader
                                                           mid-tone zone with
                                                           amber warmth.  Barocci
                                                           reveals that the upper
                                                           portion of the mid-tone
                                                           range -- the penumbra
                                                           zone where light first
                                                           withdraws from the
                                                           surface -- is where the
                                                           painting's most intimate
                                                           warmth lives.  His
                                                           characteristic rose-pink
                                                           petal flush (lum 0.42-
                                                           0.62) is distinct from
                                                           Leyster's amber mid-tone
                                                           animation (lum 0.35-
                                                           0.60) in that it targets
                                                           a warmer rose-pink
                                                           chromaticity and a
                                                           slightly higher
                                                           luminance band -- the
                                                           zone of the half-lit
                                                           cheek just below the
                                                           specular peak.  This
                                                           is the anatomical zone
                                                           that feels most alive
                                                           in Barocci's paintings.

  Sessions 97 and earlier (retained):
  - luini_leonardesque_glow_pass()                         -- (session 97) Bernardino Luini
  - sfumato_veil_pass (highlight_ivory_lift=0.06)          -- (session 97 improved)
...
Session 98 artistic character:
  The random artist for this session is Federico Barocci (c. 1535-1612), the
  supreme colorist of Umbrian Mannerism and the critical bridge between the
  late-Renaissance tradition and the fully realised Baroque.  Born in Urbino
  in the Marche -- a city whose court had shaped the ideals of Renaissance
  art and culture through Castiglione's Courtier -- Barocci worked almost
  exclusively for the courts and churches of central Italy.  He rejected
  prestigious commissions from Rome (including from Philip II of Spain and
  the Emperor Maximilian II) that might have altered his singularly personal
  development.  His work was admired by Rubens, who owned several Barocci
  drawings and absorbed his warm-flesh technique into the Flemish Baroque
  tradition; by Annibale Carracci, who recognised in him the colouristic
  ambition the Bolognese reform would systematise; and by Giovanni Pietro
  Bellori, whose Vite dei pittori gives the most sustained account of his
  technique.

  Barocci's most distinctive technical achievement was his use of chalk
  pastel studies -- 'pasteletti' -- as full colour preparatory tools.  He
  was among the first Italian artists to employ pastels not merely for
  drawing but for exploring the exact colour relationships that would appear
  in the finished oil.  This practice fed directly into his oil technique:
  each finished surface was built from multiple warm-cool glaze layers over a
  ...
  the optical depth of oil glazing.  Technical analysis of his works reveals
  an unusually complex layer structure: warm amber-ochre imprimatura, then
  coloured underpainting in warm rose and cool blue, then multiple transparent
  glazes in alternating temperature, producing a surface that reads as
  emanating light rather than reflecting it.
  ...
  lum 0.42-0.62 in the pipeline's luminance scheme).  Where Leonardo
  dissolved this boundary with grey sfumato, Rembrandt charged it with
  psychological weight, and Leyster (session 96) animated it with amber
  candlelight, Barocci filled this zone with a warm rose-pink luminosity
  that is uniquely and unmistakably his own.  This 'petal flush' -- soft,
  warm, slightly rosy -- gives his flesh surfaces a quality closer to living
  skin than paint: the cheek turning away from the main light carries a warm
  rose breath, as if the body's internal warmth rises through the surface
  from within.

  In the context of this Mona Lisa portrait, Barocci's contribution is
  precisely placed.  The Mona Lisa's most discussed quality is the ambiguity
  of her expression -- and the expression lives in the penumbra zone of the
  face.  The corners of the mouth, the slight forward projection of the
  cheekbones, the soft shadow below the lower lip: these are all penumbra
  passages, the half-lit zones that carry the face's emotional temperature.
  By adding Barocci's warm rose petal flush to these zones -- while retaining
  the sfumato dissolution from sessions 82/89/97 and the mid-tone amber
  animation from Leyster (session 96) -- the Barocci pass gives the face a
  warm, breathing quality that enriches the enigma rather than resolving it.
  The subject's expression becomes not merely ambiguous but alive: the
  penumbra zones glow with rose warmth, as if the inward life of the figure
  is pressing gently through the surface.
"""
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-98 painting pipeline."""
    print("Session 98 -- Mona Lisa portrait")
    print("Random artist: Federico Barocci (Umbrian Mannerism / proto-Baroque, c. 1535-1612)")
    print("New pass: barocci_petal_flush_pass()")
    print("  -- Petal flush: warm rose-pink bloom in penumbra zone [lum 0.42-0.62]")
    print("  -- Bianca ground luminosity: warm creamy lift in upper mid-tones [lum 0.58-0.82]")
    print("  -- Warm perimeter dissolution: figure edges dissolve into warm ambient haze")
    print()
    print("Session 98 artistic improvement: penumbra as first-class bloom target")
    print("  The penumbra zone [lum 0.42-0.62] -- where Barocci's most intimate rose")
    print("  warmth lives -- now receives a dedicated petal flush pass, distinct from")
    print("  Leyster's amber mid-tone animation (lum 0.35-0.60) in its rose-pink")
    print("  chromaticity and upper-mid-tone focus.")
    print()
    ...
    # ── SESSION 98: Federico Barocci petal flush pass (NEW) ─────────────────
    # Federico Barocci (c. 1535-1612) -- Umbrian Mannerism / proto-Baroque.
    # Three operations: petal flush (warm rose-pink bloom in penumbra zone
    # [lum 0.42-0.62] -- the signature warmth of Barocci's flesh passages),
    # bianca ground luminosity (warm creamy lift in upper mid-tones [lum
    # 0.58-0.82] simulating the pale warm 'bianca' ground showing through
    # thin glazes), and warm perimeter dissolution (figure edges dissolve
    # into warm ambient haze -- Barocci's warm-colourised sfumato).
    print("Federico Barocci petal flush pass (session 98 -- NEW)...")
    p.barocci_petal_flush_pass(
        penumbra_lo             = 0.42,    # lower lum bound of petal flush zone
        penumbra_hi             = 0.62,    # upper lum bound of petal flush zone
        rose_r                  = 0.022,   # R boost in penumbra (warm rose)
        rose_g                  = 0.008,   # G boost in penumbra (slight tint)
        rose_b                  = 0.010,   # B reduction in penumbra (dampen cool)
        bianca_lo               = 0.58,    # lower lum bound of bianca luminosity zone
        bianca_hi               = 0.82,    # upper lum bound of bianca luminosity zone
        bianca_r                = 0.018,   # R lift in bianca zone (warm ground show-through)
        bianca_g                = 0.012,   # G lift in bianca zone
        bianca_b                = 0.006,   # B lift in bianca zone (warm, not cool)
        edge_dissolve_sigma     = 2.2,     # Gaussian sigma for warm perimeter dissolution
        edge_dissolve_radius    = 0.72,    # relative radius beyond which dissolution begins
        edge_dissolve_strength  = 0.35,    # blend fraction toward dissolved in periphery
        blur_radius             = 4.0,
        opacity                 = 0.38,
    )
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s98.png")
    p.save(out_path)

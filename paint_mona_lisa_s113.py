"""
paint_mona_lisa_s113.py -- Session 113 portrait painting.
...
Session 113 additions:
  - guido_cagnacci                           -- NEW (session 113)
                                               Guido Cagnacci
                                               (1601–1663), Italian
                                               Emilian Baroque /
                                               Caravaggesque Idealism.
                                               Born in Santarcangelo
                                               di Romagna; trained in
                                               Bologna in the orbit
                                               of Guido Reni before
                                               moving to Rome, Venice,
                                               and finally Vienna as
                                               court painter to
                                               Emperor Leopold I
                                               (c. 1658–1663).
                                               His defining quality:
                                               a dreamlike rose-flesh
                                               luminescence unlike any
                                               other Baroque master —
                                               warm and pinkish in the
                                               mid-tones, as if lit
                                               from within rather than
                                               from without.

  SESSION 112 ARTISTIC IMPROVEMENT -- cagnacci_rose_flesh_pass:
                                               ...
                                               highlight zone
                                               (lum > hi_lo ≈ 0.70),
                                               applies a pinkish-rose
                                               warmth (R + rose_r_lift,
                                               B - rose_b_damp) — warmer
                                               than Boltraffio's pearl
                                               or Orazio's silver, but
                                               a specific rose quality
                                               distinct from the amber
                                               warmth of Guercino or
                                               Rembrandt;
                                               (2) Peach mid-tone glow
                                               — in the mid-tone band,
                                               the primary Cagnacci
                                               effect: a rose-peach
                                               shift (R + peach_r,
                                               G + peach_g, B - peach_b)
                                               via raised-cosine window,
                                               encoding the warm
                                               pinkish-peach in the
                                               mid-tone flesh zone
                                               that defines his
                                               Emilian signature;
                                               ...
                                               — in shadow zones, a
                                               rose-amber recovery
                                               that prevents Orazio's
                                               cool correction and
                                               Sassoferrato's blue
                                               from making the darks
                                               too cold — Cagnacci's
                                               shadows stay warm.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s113 (Guido Cagnacci rose flesh -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–113 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 113 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Guido Cagnacci rose flesh (113 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 113: Guido Cagnacci rose flesh pass (NEW) ─────────────────────
    # "Rose flesh" — Cagnacci's dreamlike Emilian Baroque luminescence.
    # Applied as a warm counter-balance to Orazio's cool silver correction and
    # Sassoferrato's ultramarine purity:
    #
    # (1) Rose highlight warmth — where Orazio's silver daylight pass coolled
    #     the upper highlights (R - silver_r_damp, B + silver_b_lift), Cagnacci's
    #     rose_r_lift (0.010) adds a gentle pinkish warmth back at the highest
    #     flesh tones — specifically the forehead and cheekbone peaks.  The net
    #     effect after both passes is a cool-but-rosy quality at the highlights:
    #     not the cold silver of Orazio alone, not the warm ivory of pre-s111,
    #     but a specific luminous rose-ivory that characterises Cagnacci's mature
    #     style.  Set conservatively (0.010 R lift, 0.005 B damp) to preserve the
    #     Orazio silver quality while adding the Cagnacci rose warmth.
    #
    # (2) Peach mid-tone glow — the primary Cagnacci effect, applied in the mid-
    #     tone zone [0.34, 0.70].  After the Strozzi amber impasto and Guercino
    #     amber warmth passes, the mid-tones carry warm amber-ochre quality.
    #     Cagnacci's rose is different: more pink, less ochre.  The peach_r (0.016)
    #     and peach_g (0.006) shifts nudge the mid-tone warmth from amber-ochre
    #     toward rose-peach, while peach_b (0.008) removes a trace of accumulated
    #     Sassoferrato blue from the mid-tone zone — net effect is a warmer, more
    #     pinkish mid-tone that reads as Cagnacci's characteristic inner glow.
    #
    # (3) Warm shadow recovery — after Orazio's cool shadow correction
    #     (cool_r_damp, cool_b_lift in shadows) and Sassoferrato's ultramarine
    #     shadow enrichment, the darkest zones are cooler and bluer than Cagnacci's
    #     shadows typically were.  A gentle warm_r (0.014) and warm_g (0.006)
    #     recovery in shadow zones restores rose-amber warmth in the darks, so
    #     that the overall temperature balance is: highlights = cool-rosy silver,
    #     mid-tones = warm rose-peach, shadows = warm amber-rose.
    print("Guido Cagnacci rose flesh pass (session 113 -- NEW)...", flush=True)
    ...
    # Final glaze — warm rose-balance reflecting Cagnacci's luminous warmth
    print("Final glaze (rose-warm balance for Cagnacci rose flesh)...", flush=True)
    p.glaze((0.62, 0.46, 0.18), opacity=0.030)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.50, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s113.png")
    p.save(out_path)

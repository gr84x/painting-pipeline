"""
paint_mona_lisa_s108.py -- Session 108 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–108 additions cumulatively.

Session 108 additions:
  - moroni                                -- NEW (session 108)
                                             Giovanni Battista Moroni
                                             (c. 1520–1579), Bergamasque
                                             Portrait Realism / Lombard
                                             Renaissance.
                                             The defining master of direct
                                             psychological portraiture in
                                             the Italian Cinquecento.
                                             Trained under Moretto da
                                             Brescia; spent his career in
                                             Bergamo painting merchants,
                                             clergy, and guild masters
                                             with an uncanny sense of
                                             physical presence that would
                                             not be matched until
                                             Velázquez.
                                             His canonical work, Il Sarto
                                             (The Tailor, c. 1570,
                                             National Gallery), captures
                                             his essence: direct gaze,
                                             precise textile rendering,
                                             and cool north-daylight on
                                             warm-earthed flesh.

  SESSION 108 ARTISTIC IMPROVEMENT -- moroni_silver_presence_pass:

                                             New pass encoding three
                                             defining Moroni qualities:
                                             (1) Cool silver highlight —
                                             shifts uppermost flesh lights
                                             toward cool north-daylight
                                             silver (B lift > G lift,
                                             tiny R reduction) without
                                             approaching the icy cold of
                                             Bronzino's enamel;
                                             (2) Warm shadow recovery —
                                             adds earthy amber-ochre warmth
                                             in the shadow zone (R lift,
                                             smaller G lift, tiny B
                                             reduction) so shadows remain
                                             grounded and warm, never cold
                                             or grey;
                                             (3) Mid-tone presence
                                             enhancement — mild gamma
                                             separation in the mid-tone
                                             zone slightly darkens the
                                             lower mid-tones and lightens
                                             the upper, producing the
                                             quiet tonal precision that
                                             makes Moroni's sitters seem
                                             physically present rather
                                             than painted.

  Also fixes: SPANISH_NEAPOLITAN_BAROQUE (Ribera, session 106) now has
  a proper stroke_params entry instead of falling back to Period.NONE.
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–108 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 108 warm-start from mona_lisa_s101.png", flush=True)
    ...
    # ── SESSION 108: Giovanni Battista Moroni silver presence pass (NEW) ──────
    # "Silver presence" — the Bergamasque master of direct psychological realism.
    # Three-stage effect complementing the warm-amber Leonardesque foundation:
    #
    # (1) Cool silver highlight — shifts the uppermost flesh lights from the
    #     warm-ivory tone left by Dolci/Guercino/Boltraffio toward a cooler,
    #     more neutral silver-daylight quality.  Applied at hi_lo=0.70 so only
    #     the very peak highlights are affected.  The Boltraffio pass (s107)
    #     already adds cool-silver pearl; Moroni's version is warmer and more
    #     naturalistically grounded — slightly less blue, slightly more green.
    #
    # (2) Warm shadow recovery — in the deepest shadow zone (lum < 0.35),
    #     reinforces amber-ochre warmth.  The Ribera pass (s106) already adds
    #     shadow grain texture; Moroni's version adds colour warmth in the same
    #     zone, giving the shadows a rich, earthy depth rather than cold void.
    #
    # (3) Mid-tone presence enhancement — the key Moroni quality: mild gamma
    #     separation in the mid-tone zone [0.35, 0.70] that slightly strengthens
    #     tonal plane separation without sfumato dissolution or visible sharpening.
    #     Applied at moderate opacity=0.34 to preserve the accumulated sfumato
    #     depth of the prior sessions while adding Moroni's direct presence.
    print("Moroni silver presence pass (session 108 -- NEW)...", flush=True)
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s108.png")
    p.save(out_path)

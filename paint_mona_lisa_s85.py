"""
paint_mona_lisa_s85.py -- Session 85 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-85 additions:

  - signorelli_sculptural_vigour_pass()         -- NEW (session 85) Luca Signorelli.
                                                   Three-stage sculptural vigour:
                                                   (1) contour clarification — unsharp-
                                                   mask edge sharpening at the scale of
                                                   muscle boundaries and drapery folds,
                                                   replicating Signorelli's characteristic
                                                   firm painted contour that gives his
                                                   figures their bas-relief sculptural
                                                   quality; (2) shadow depth modelling —
                                                   push mid-dark pixels toward warm umber,
                                                   deepening the sculptural shadow relief
                                                   that distinguishes Signorelli's modelling
                                                   from Perugino's softer ambient-light
                                                   approach; (3) chromatic accent lift —
                                                   gentle saturation boost in already-
                                                   saturated mid-tone regions (drapery,
                                                   landscape), replicating the jewel-like
                                                   colour punctuation that energises
                                                   Signorelli's warm-earth compositions.

  Sessions 84 and earlier (retained):
...
Session 85 artistic character:
  The random artist inspiration for this session is Luca Signorelli
  (c. 1445/50–1523), the Umbrian master whose muscular, clear-contour
  figure style exercised the most direct documented influence on
  Michelangelo and through him on the entire tradition of High Renaissance
  and Mannerist figure painting.

  Signorelli stands in productive tension with the accumulated sfumato
  layers of sessions 1–84.  Where Leonardo dissolved every boundary into
  atmospheric unity, and where Perugino (session 84) further softened the
  sky zone toward pale Umbrian openness, Signorelli insisted on the
  structural clarity of individual forms — the firm painted contour that
  describes a muscle plane or a drapery fold as if it were carved.

  What does Signorelli contribute to the Mona Lisa context?

  The previous 84 sessions have built extraordinary atmospheric depth:
  warm sfumato veils, cool aerial recession, Romantic turbulence, Fra
  Filippo Lippi's Quattrocento tenerezza, Perugino's Umbrian sky-
  luminosity bleaching.  One quality that remains underexplored is the
  sculptural clarity of individual forms beneath all that atmospheric
  dissolution.  Signorelli's three-stage pass recovers this quality without
  undoing the sfumato heritage:

  1. Contour clarification operates at a fine spatial scale (USM sigma
     ~1.2px), targeting mid-luminance edge transitions where the boundary
     between one form and the next is real and anatomically significant.
     In the context of the accumulated composition, this pass quietly
     recovers the structural edges of the face, the hands, and the drapery
     folds that 84 sessions of blending have softened toward atmospheric
     dissolution.  The effect is subtle — the sfumato softening remains
     dominant — but gives the figure a slightly more sculptural, present
     quality against its geological background.

  2. Shadow depth modelling warms the mid-dark passages toward umber-
     sienna.  After Perugino's cool silvery shadow harmonization (session
     84) and Géricault's warm shadow recovery (session 82), this pass
     adds a further warm depth to the shadow passages that reads as
     Signorelli's characteristic chiaroscuro — more dramatic and directed
     than Perugino's ambient-light gentleness, less turbulent than
     Géricault's Romantic void.  The result is a shadow register that
     reads as both warm and deep — sculptural rather than atmospheric.

  3. Chromatic accent lift boosts the saturation of already-vivid mid-
     tone zones: the blue-grey of the distant geological landscape, the
     muted olive-green of the sparse vegetation, any residual colour in
     the drapery accents.  In the context of the predominantly warm-earth
     tonality of the accumulated composition, this gentle saturation
     enrichment in the already-coloured zones gives the background
     landscape a slightly more vivid, more present quality — the blue of
     the far distance reads more clearly as blue — which in turn makes the
     figure's warm skin tones advance more emphatically from the cool
     distance behind.  This is Signorelli's characteristic jewel-punctuation
     against the warm earth ground.
"""
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-85 painting pipeline."""
    print("Session 85 -- Mona Lisa portrait")
    print("Random artist: Luca Signorelli (Umbrian Renaissance, c. 1445/50–1523)")
    print("New pass: signorelli_sculptural_vigour_pass()")
    print("  -- contour clarification: USM sharpening of muscle/drapery boundaries")
    print("  -- shadow depth modelling: warm umber push in mid-dark shadow passages")
    print("  -- chromatic accent lift: saturation boost in vivid landscape/drapery zones")
    print()
    ...
    print("Pietro Perugino serene grace pass (session 84)...")
    p.perugino_serene_grace_pass(
        ...
        midtone_hi       = 0.76,
        smooth_sigma     = 3.5,
        smooth_strength  = 0.16,
        blur_radius      = 7.0,
        opacity          = 0.38,
    )

    # Session 85 NEW: Luca Signorelli sculptural vigour pass
    # Three-stage sculptural vigour:
    # (1) Contour clarification — USM sharpening at the scale of muscle boundaries
    #     and drapery folds.  After 84 sessions of sfumato accumulation, the forms
    #     benefit from a gentle recovery of structural edges that reads as Signorelli's
    #     characteristic firm painted contour — not a digital sharpen but a clarification
    #     of the underlying anatomical planes already present in the composition.
    # (2) Shadow depth modelling — warm umber push in mid-dark passages.  After
    #     Perugino's cool silvery shadow harmonization and Géricault's turbulent warm
    #     recovery, Signorelli adds a further sculptural warmth to the shadow zone:
    #     deeper, warmer, more directed than Perugino's ambient-light gentleness.
    # (3) Chromatic accent lift — saturation boost in vivid mid-tone regions.  The
    #     cool blue-grey of the distant geological landscape, the olive-green of the
    #     sparse vegetation: these zones receive a gentle saturation enrichment that
    #     gives the background its Signorelli jewel-punctuation quality and makes the
    #     warm skin tones of the figure advance more emphatically.
    # Opacity 0.40 — Signorelli's vigour is direct but restrained in portrait context.
    print("Luca Signorelli sculptural vigour pass (session 85 NEW)...")
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s85.png")
    p.save(out_path)

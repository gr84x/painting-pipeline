"""
paint_mona_lisa_s118.py -- Session 118 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–118 additions cumulatively.

Session 118 additions:
  - savoldo                                  -- NEW (session 118)
                                               Giovanni Gerolamo Savoldo
                                               (c. 1480/85–1548), Italian
                                               (Brescian).
                                               Brescian Renaissance painter
                                               working in Venice — a bridge
                                               between the Lombard tradition
                                               and the Venetian school.
                                               Savoldo pioneered a distinctive
                                               nocturnal light quality without
                                               precedent in the Renaissance:
                                               a phosphorescent silver-grey
                                               shimmer concentrated at the
                                               transitional boundary between
                                               light and shadow.  Visible
                                               most fully in Saint Mary
                                               Magdalen Approaching the
                                               Sepulchre (National Gallery,
                                               London): drapery lit by
                                               moonlight reads as luminous
                                               silver-grey at fold crests
                                               while shadowed valleys retain
                                               warm amber depth; the
                                               transition carries a cool
                                               phosphorescent shimmer unlike
                                               anything else in Western art
                                               of the period.  Known for
                                               Portrait of a Knight, Tobias
                                               and the Angel, Adoration of
                                               the Shepherds, and Saint
                                               Matthew and the Angel.

  SESSION 118 ARTISTIC IMPROVEMENT -- savoldo_silver_veil_pass:

                                               New pass encoding three
                                               defining Savoldo qualities:
                                               (1) Silver moonveil —
                                               Gaussian-peaked weight
                                               centred at the penumbra
                                               midpoint ([0.28, 0.62]).
                                               R - silver_r, B + silver_b.
                                               The Gaussian window (σ = 0.20
                                               of the range) is the session
                                               118 improvement: a more
                                               physically accurate model of
                                               scattered moonlight at the
                                               penumbra boundary than the
                                               sin window (session 116) or
                                               sin²-window (session 117).
                                               A Gaussian naturally models
                                               the bell-shaped distribution
                                               of scattered radiance at a
                                               surface tangent to the
                                               illumination direction;
                                               (2) Warm ivory highlight —
                                               warm silver-ivory lift at
                                               peak flesh (lum > 0.70):
                                               R + ivory_r, G + ivory_g.
                                               Not cold — the Venetian-
                                               Lombard ground moderates the
                                               silver with Brescian warmth;
                                               (3) Warm shadow retention —
                                               raised-cosine warm recovery
                                               in shadow (lum < 0.32):
                                               R + warm_r, G + warm_g.
                                               Savoldo's nocturnal darks
                                               never become arctic cold —
                                               Brescian earth warmth
                                               persists even at their
                                               deepest passages.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s118 (Giovanni Gerolamo Savoldo silver moonveil -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–118 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 118 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Giovanni Gerolamo Savoldo silver moonveil (118 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 118: Giovanni Gerolamo Savoldo silver moonveil pass (NEW) ──────
    # The silver moonveil pass encodes Savoldo's three defining qualities:
    #
    # (1) Silver moonveil — a Gaussian-peaked weight centred at the penumbra
    #     midpoint [0.28, 0.62].  The Gaussian window (σ = 0.20 of the range)
    #     is the session 118 artistic improvement: a more physically accurate
    #     model of scattered moonlight than the sin window (session 116) or
    #     sin²-window (session 117).  A Gaussian models the bell-shaped
    #     distribution of scattered radiance at a surface whose normal is tangent
    #     to the illumination direction — exactly the condition at the penumbra
    #     boundary.  R - silver_r (0.010), B + silver_b (0.022).
    #
    # (2) Warm ivory highlight — warm silver-ivory lift at peak flesh (lum > 0.70).
    #     R + ivory_r (0.012), G + ivory_g (0.007).  Savoldo's highlights are
    #     warm, not cold — the Venetian-Lombard imprimatura moderates the silver
    #     with Brescian earth warmth.
    #
    # (3) Warm shadow retention — raised-cosine warm recovery below lum = 0.32.
    #     R + warm_r (0.012), G + warm_g (0.005).  Savoldo's nocturnal shadows
    #     never become arctic cold — Brescian earth warmth persists even at the
    #     deepest passages.
    print("Giovanni Gerolamo Savoldo silver moonveil pass (session 118 -- NEW)...", flush=True)
    ...
    # Final glaze — warm silver-amber reflecting Savoldo's Brescian-Venetian warmth
    print("Final glaze (warm silver-amber for Brescian-Venetian ground warmth)...", flush=True)
    p.glaze((0.58, 0.46, 0.22), opacity=0.020)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.52, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s118.png")
    p.save(out_path)

"""
paint_mona_lisa_s166.py — Mona Lisa portrait for session 166.
...
Artistic discovery (session 166): Adriaen van der Werff (1659–1722)
  - Dutch Classical Late Baroque fijnschilder (fine-painter)
  - Pupil of Eglon van der Neer; supreme master of the polished enamel surface
  - His skin is the polar opposite of Schalcken's warm candlelight: cool
    alabaster-ivory highlights, cool violet-grey shadows, no warm amber
  - Characteristic: porcelain smoothness, pearl-blue highlights, north-window
    diffuse light — every brushstroke dissolved into seamless glaze

New pipeline enhancements (session 166):
  - van_der_werff_ivory_alabaster_pass: FIFTY-FIFTH DISTINCT MODE
    COOL IVORY ALABASTER SKIN TINTING WITH PEARL-BLUE HIGHLIGHT LIFT:
    (1) Skin zone detection (RGB range + r>g+margin, g>=b)
    (2) Cool ivory lift in skin highlights: B+, R− at hi_gate
    (3) Cool shadow depth in skin zone: B+, R− at shad_gate
    — FIRST pass to apply COOL tint to detected skin zone; all prior skin
    passes apply warm tints (Massys, Schalcken, Gentileschi, Reynolds)
  - craquelure_texture_pass: FIFTY-SIXTH DISTINCT MODE
    DIFFERENCE-OF-GAUSSIANS SYNTHETIC CRAQUELURE CRACK NETWORK:
    (1) Uniform noise generation
    (2) DoG on noise: blur_fine − blur_coarse → normalised ridge map
    (3) Threshold → sparse crack mask
    (4) Multiplicative crack darkening (hue-preserving)
    — FIRST pass to use DoG on a noise source (not the image) to generate
    synthetic topographic ridge structure
"""
...
def paint(output_path: str = "mona_lisa_s166.png") -> str:
    """Full Leonardo sfumato pipeline with session 166 Van der Werff passes."""
    print("=" * 64)
    print("  Session 166 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: Adriaen van der Werff (1659–1722)")
    print("    FIFTY-FIFTH MODE: van_der_werff_ivory_alabaster_pass")
    print("    — cool ivory skin tinting, first cool skin-detection pass")
    print("    FIFTY-SIXTH MODE: craquelure_texture_pass")
    print("    -- DoG on noise map -> synthetic age crack network")
    print("=" * 64)

    ref = build_reference(W, H)
    ref.save("mona_lisa_s166_reference.png")
    print(f"  Reference saved -> mona_lisa_s166_reference.png  ({W}x{H})")

    leo = get_style("leonardo")
    p = Painter(W, H)

    print("  [1/20] Tone ground ...")
    p.tone_ground(leo.ground_color, texture_strength=0.06)

    print("  [2/20] Underpainting ...")
    p.underpainting(ref, stroke_size=52, n_strokes=160)

    print("  [3/20] Block-in ...")
    p.block_in(ref, stroke_size=34, n_strokes=340)

    print("  [4/20] Build form ...")
    p.build_form(ref, stroke_size=10, n_strokes=1400)

    print("  [5/20] Sfumato veil pass ...")
    p.sfumato_veil_pass(
        ref,
        n_veils       = 10,
        blur_radius   = 14.0,
    ...
    print("  [6/20] Edge sfumato dissolution pass ...")
    ...
    print("  [7/20] Skin subsurface scatter pass ...")
    ...
    print("  [8/20] Tonal envelope pass ...")
    ...
    print("  [9/20] Selective focus pass ...")
    ...
    print("  [10/20] Massys bridge glazing pass ...")
    ...
    print("  [11/20] Reynolds Grand Manner mezzotint tone pass ...")
    ...
    print("  [12/20] Schedoni luminous emergence pass ...")
    ...
    print("  [13/20] Warm highlight bloom pass ...")
    ...
    print("  [14/20] Shadow color temperature pass ...")
    ...
    print("  [15/20] Film grain overlay pass ...")
    ...
    print("  [16/20] Van der Werff ivory alabaster pass — FIFTY-FIFTH DISTINCT MODE ...")
    # Applied at moderate opacity: adds cool pearl quality to skin highlights
    # and cool violet depth to skin shadows without overwhelming the sfumato warmth.
    # The pearl-blue lift in highlights gives the skin a luminous porcelain quality.
    ...
    print("  [17/20] Craquelure texture pass — FIFTY-SIXTH DISTINCT MODE ...")
    # Adds fine age cracks to suggest the passage of centuries.
    # Fine cracks (sigma_fine=1.2, sigma_coarse=5.0): delicate network,
    # not the heavy cracking of severely aged paintings.
    ...
    print("  [18/20] Place lights ...")
    p.place_lights(ref, stroke_size=5, n_strokes=320)

    print("  [19/20] Final glaze ...")
    p.glaze((0.70, 0.50, 0.20), opacity=0.055)
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s166.png"
    paint(out)

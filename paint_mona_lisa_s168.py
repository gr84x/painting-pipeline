"""
paint_mona_lisa_s168.py — Mona Lisa portrait for session 168.
...
Artistic discovery (session 168): Joachim Wtewael (1566–1638)
  - Last great Dutch Mannerist; Utrecht master who worked on copper
  - His copper support gives paintings distance-invariant jewel saturation
  - Carmine, cadmium orange, ultramarine, emerald — colours at maximum chroma
  - No atmospheric haze: background as vivid as foreground, like stained glass
  - Heavy Mannerist linearity: crisp contours, no sfumato, explicit forms
  - Famous works: 'Perseus and Andromeda' (1611), 'The Kitchen Piece' (c. 1605)

New pipeline enhancement (session 168):
  - wtewael_copper_jewel_pass: FIFTY-EIGHTH DISTINCT MODE
    PER-PIXEL CHROMA AMPLIFICATION VIA OPPONENT-SPACE DISTANCE SCALING:
    (1) Neutral mean: n = (r+g+b)/3 per pixel
    (2) Chroma decomposition: delta = (r-n, g-n, b-n) — opponent-space vector
    (3) Bell-curve mid-gate: peak in mid-bright zone (luma 0.25–0.78)
    (4) Chroma amplification: out = n + delta × (1 + saturation_boost × gate)
    (5) Copper shadow warmth: warm copper-red bleed in deep shadow zone
    — FIRST and ONLY pass to use per-pixel neutral mean decomposition followed
    by chroma distance scaling in opponent-colour space; all prior colour
    passes work on absolute channel values, not on deviation from the pixel's
    own neutral point.
"""
...
def paint(output_path: str = "mona_lisa_s168.png") -> str:
    """Full Leonardo sfumato pipeline with session 168 Wtewael copper jewel pass."""
    print("=" * 64)
    print("  Session 168 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: Joachim Wtewael (1566–1638)")
    print("    Dutch Mannerist — oil on copper jewel saturation")
    print("    FIFTY-EIGHTH MODE: wtewael_copper_jewel_pass")
    print("    — per-pixel chroma amplification via opponent-space distance")
    print("=" * 64)

    ref = build_reference(W, H)
    ref.save("mona_lisa_s168_reference.png")
    print(f"  Reference saved -> mona_lisa_s168_reference.png  ({W}x{H})")
    ...
    print("  [16/20] Van der Werff ivory alabaster pass ...")
    ...
    print("  [17/20] Craquelure texture pass ...")
    ...
    print("  [18/20] Clouet enamel precision pass ...")
    ...
    print("  [19/20] Wtewael copper jewel pass — FIFTY-EIGHTH DISTINCT MODE ...")
    # Wtewael's copper support gives distance-invariant jewel saturation.
    # The pass amplifies chroma in mid-bright zones via per-pixel neutral mean
    # decomposition, and injects copper-red warmth in the deepest shadows —
    # replicating the warm metal ground bleeding through thin paint layers.
    # Applied at moderate opacity so Leonardo's sfumato is not overwhelmed:
    # the jewel quality enriches the colour field without hardening the edges.
    ...
    print("  [20/20] Place lights, final glaze + finish ...")
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s168.png"
    paint(out)

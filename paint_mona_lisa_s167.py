"""
paint_mona_lisa_s167.py — Mona Lisa portrait for session 167.
...
Artistic discovery (session 167): François Clouet (c. 1510–1572)
  - French Renaissance court portraitist; valet de chambre to four French kings
  - Synthesised Flemish enamel precision with Italian idealized form
  - His skin is cool, pale ivory-rose — fair, courtly, north-window lit
  - No dramatic chiaroscuro; cool even diffuse light; precise crisp outlines
  - Antithesis of sfumato: contours are findable, surface is polished
  - Famous works: 'Pierre Quthe' (1562), 'Lady in Her Bath' (c. 1571)

New pipeline enhancement (session 167):
  - clouet_enamel_precision_pass: FIFTY-SEVENTH DISTINCT MODE
    LUMINANCE-PRESERVING COOL ENAMEL GLAZE:
    (1) Luma gate: apply tint progressively in lighter zones only
    (2) Chroma shift: lerp each pixel toward cool ivory target colour
    (3) Luma measurement: compute new luma of tinted output
    (4) Luminance restoration: DIVIDE back by ratio to restore original luma
    — FIRST and ONLY pass to perform luminance-normalizing division after
    a colour tint, making the chroma shift 'chrominance-only' in the strict
    colour-science sense; all prior tinting passes alter luma as a side-effect
"""
...
def paint(output_path: str = "mona_lisa_s167.png") -> str:
    """Full Leonardo sfumato pipeline with session 167 Clouet enamel pass."""
    print("=" * 64)
    print("  Session 167 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: François Clouet (c. 1510–1572)")
    print("    French Renaissance court portraitist — cool enamel precision")
    print("    FIFTY-SEVENTH MODE: clouet_enamel_precision_pass")
    print("    — luminance-preserving cool glaze (first luma-normalising pass)")
    print("=" * 64)

    ref = build_reference(W, H)
    ref.save("mona_lisa_s167_reference.png")
    print(f"  Reference saved -> mona_lisa_s167_reference.png  ({W}x{H})")
    ...
    print("  [18/20] Clouet enamel precision pass — FIFTY-SEVENTH DISTINCT MODE ...")
    # Clouet's defining quality: cool, pearl-ivory north-window light with
    # precisely preserved tonal structure underneath.  The cool_b value (0.94)
    # pushes highlights toward blue-pearl; the luminance normalisation ensures
    # the tonal drawing stays intact — exactly how Clouet's thin cool glazes
    # functioned over his careful underpainting.
    ...
    print("  [19/20] Place lights ...")
    p.place_lights(ref, stroke_size=5, n_strokes=320)

    print("  [20/20] Final glaze + finish ...")
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s167.png"
    paint(out)

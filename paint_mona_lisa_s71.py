"""
paint_mona_lisa_s71.py — Session 71 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
enhanced with session-71 additions:

  - correggio_golden_tenderness_pass()  — NEW (session 71) Antonio Correggio
                                           Parma Renaissance inspiration.
                                           Warms the midtones toward amber-gold;
                                           resists cool shadow drift with warm
                                           umber; adds a radial warm glow centred
                                           on the face — the Correggesque quality
                                           of figures suffused with honeyed light.

  - luminous_haze_pass()               — NEW (session 71) artistic improvement.
                                           Simulates the warm unifying glow of
                                           aged old-master varnish combined with
                                           the tonal softening of diffused interior
                                           light: shadow lift, contrast damping,
                                           warm amber-honey overlay, and a gentle
                                           pre-composite softening blur.

  Previous sessions (retained):
  - guido_reni_angelic_grace_pass()    — (session 70)
  - highlight_bloom_pass()             — (session 70)
...
Session 71 artistic character:
  The random artist inspiration for this session is Antonio Allegri da
  Correggio (1489–1534), the supreme master of golden, tender luminosity
  in the Italian Renaissance.

  Correggio worked in Parma, largely independent of the Roman and Florentine
  academic traditions.  This isolation allowed him to develop a rendering of
  light and flesh without parallel in his era — a proto-Baroque warmth and
  softness that would not be seen again until Rubens and Reni, both of whom
  studied him deeply.  His defining quality is 'Correggesque' tenderness: the
  sense that every figure is bathed in a warm amber-gold illumination that
  emanates from within the form rather than falling on its surface.

  Unlike Leonardo's cool, smoky sfumato or Raphael's clear rational idealisation,
  Correggio's approach is entirely governed by warmth.  His shadows are always
  amber-brown — never grey or violet — and his highlights are warm ivory rather
  than cool pearl.  The Assumption of the Virgin in Parma Cathedral (1526–30),
  painted lying on his back on scaffolding, is the first great Baroque illusionistic
  ceiling: a spiralling vortex of foreshortened figures ascending through layers
  of warm golden light toward heaven.  His Jupiter and Io (c. 1531) takes the
  warm-flesh ideal to its fullest expression: Io's body, emerging from a dark
  amber cloud, glows with a honeyed translucency that no other Renaissance painter
  achieved.

  The correggio_golden_tenderness_pass() applies his three defining operations:
  (1) golden midtone lift — shifts the midtone band toward amber-gold, warming
      the overall skin without burning it;
  (2) warm shadow resistance — prevents the shadow passages from cooling toward
      violet-grey (Reni's territory) or neutral brown, keeping them warm umber;
  (3) radial face glow — a soft warm radiance centred on the face, simulating
      the concentration of Correggesque light on the primary subject.

  The session 71 artistic improvement is luminous_haze_pass() — an encoding of
  two sources of golden unity in Correggio's paintings:
  (a) aged natural-resin varnish, which yellows over centuries to give the whole
      canvas an amber honey tone; and
  (b) diffused interior light, which compresses the tonal scale by lifting
      shadows and softening highlights, unifying the palette into a single
      luminous whole.  The pass applies shadow lift, contrast damping, warm amber
      overlay, and a gentle pre-composite softening blur.
"""
...
# (same composition as s70 — identical portrait, new passes applied)
# ─────────────────────────────────────────────────────────────────────────────
...
def paint(out_dir: str = ".") -> str:
    """
    Execute the full session-71 painting pipeline and save the result.

    Returns the path to the saved PNG.
    """
    print("Session 71 — Mona Lisa portrait")
    print("Random artist: Antonio Correggio (Parma Renaissance, 1489–1534)")
    print("Artistic improvement: luminous_haze_pass()")
    print()
    ...
    # Applied after the warm skin passes — at this point the skin is warm and
    # glowing.  Correggio's pass now:
    #   1. Lifts the midtone band toward amber-gold — warmer and more honeyed
    #      than the ivory-warm produced by Vigée Le Brun and subsurface scatter.
    #   2. Resists cool shadow drift — adds amber warmth to shadow zones,
    #      keeping all passages warm and preventing any violet-grey lean.
    #   3. Adds a radial warm face glow — the Correggesque concentration of
    #      warm light on the primary subject, as in Jupiter and Io.
    # opacity=0.46 — balanced to warm without overwhelming the sfumato.
    ...
    # Applied near the end — after the specular and bloom passes are complete.
    # At this point the image has crisp highlights and a clear tonal structure;
    # the luminous haze pass now:
    #   1. Adds a warm amber-honey overlay (the aged-varnish effect).
    #   2. Lifts the deepest shadows slightly (ambient scatter).
    #   3. Damps the overall contrast fractionally (interior diffuse light).
    #   4. Applies a very gentle pre-composite softening (aged-glaze scatter).
    # opacity=0.50 — present and effective, but not overwhelming the sfumato.
    print("Luminous haze pass (session 71)…")
    p.luminous_haze_pass(
        haze_warmth    = 0.038,
        haze_opacity   = 0.10,
        haze_color     = (0.88, 0.76, 0.50),   # warm honey amber — Correggesque varnish
        soften_radius  = 2.5,
        contrast_damp  = 0.055,
        shadow_lift    = 0.016,
        opacity        = 0.50,
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s71_final.png")
    p.save(out_path)

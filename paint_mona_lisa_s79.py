"""
paint_mona_lisa_s79.py -- Session 79 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-79 addition:

  - lotto_chromatic_anxiety_pass()     -- NEW (session 79) Lorenzo Lotto
                                          Venetian Psychological portraiture.
                                          Introduces the cool chromatic anxiety
                                          beneath Venetian warmth: cool B-lift
                                          in flesh midtones (Lotto's grey-cool
                                          undertone), eye-region contrast
                                          intensification with cool shadow shift
                                          (the psychologically alert Lotto gaze),
                                          and background green-blue dissonance
                                          (his characteristic cool muted-green
                                          backgrounds that prevent Venetian ease).
                                          Applied at opacity 0.46 -- subtle but
                                          accumulative, as Lotto's effect always is.

  Sessions 78 and earlier (retained):
...
Session 79 artistic character:
  The random artist inspiration for this session is Lorenzo Lotto (c. 1480-1556),
  the psychologically restless master of Venetian Renaissance portraiture.

  Lotto trained in the same tradition as Titian and Giorgione but was
  temperamentally and chromatically apart from both.  Where Titian unified
  his canvases with warm amber glazes and sensual, dissolved surfaces, Lotto
  introduced a pervasive cool chromatic anxiety: a grey-blue undertone beneath
  the warm flesh, a muted cool-green in backgrounds, a slight formal restlessness
  that prevents his portraits from settling into comfortable Venetian grandeur.

  Modern critics describe his work as proto-psychological -- his sitters seem
  caught at a moment of interior tension, their composure slightly effortful.
  The Portrait of a Young Man with a Book (c. 1526), the Andrea Odoni (1527),
  and the Triple Portrait of a Goldsmith (c. 1530) all share this quality:
  the sitter gazes out, but something in the cool half-light suggests they
  would rather not be seen.

  The lotto_chromatic_anxiety_pass() introduces three of his defining qualities:

  1. Cool flesh midtone injection (flesh_mid_lo=0.40, cool_b_boost=0.07,
     cool_r_reduce=0.04) -- skin pixels in the warm-ochre midtone band receive
     a gentle B-up, R-down push.  The face still reads as warm, but the cool
     undertone makes it psychologically distinct from a Titian portrait of the
     same sitter.  Applied to the flesh midtone band (lum in [0.40, 0.75]).

  2. Eye-region chromatic intensification (eye contrast=0.10, eye_cool_b=0.04)
     -- a small elliptical zone around each eye receives a local contrast boost
     and a cool-blue shadow shift in the darker half of the eye zone.  This
     heightens the psychologically alert quality Lotto's portraits are known for.
     Eye positions adjusted for the three-quarter pose: left eye at (0.468, 0.198),
     right eye at (0.562, 0.198).

  3. Background color dissonance (bg_green_lift=0.045, bg_blue_lift=0.028) --
     non-skin background midtone pixels receive a G-up and B-up push (the cool
     muted-green and grey-blue that characterise Lotto's backgrounds).  This
     prevents the landscape from settling into warm Venetian comfort.

  Opacity 0.46 -- Lotto's effect is never dramatic but accumulative: by the
  end of the canvas, the sum of small cool deviations produces a portrait that
  reads as psychologically distinct from a Titian of the same sitter.
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Execute the full session-79 painting pipeline and save the result.

    Returns the path to the saved PNG.
    """
    print("Session 79 -- Mona Lisa portrait")
    print("Random artist: Lorenzo Lotto (Venetian Psychological, c. 1480-1556)")
    print("Artistic improvement: lotto_chromatic_anxiety_pass() [subtle cool undertone]")
    print()
    ...
    # Session 79 NEW: Lotto chromatic anxiety pass
    # Applied after the Rigaud drapery enrichment and before the finishing varnish.
    # The cool flesh midtone injection (B-up, R-down in warm midtone band) introduces
    # Lotto's grey-cool undertone beneath the accumulated Venetian warmth of the
    # preceding sessions.  The eye-region intensification sharpens the gaze toward
    # Lotto's psychological alertness.  The background dissonance pushes landscape
    # midtones toward the cool muted-green that prevents Lotto's canvases from
    # settling into the comfortable grandeur of his Venetian contemporaries.
    # Opacity 0.46 -- subtle but cumulatively present over the full canvas.
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s79.png")
    p.save(out_path)

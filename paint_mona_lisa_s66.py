"""
paint_mona_lisa_s66.py — Session 66 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
enhanced with session-66 additions:

  - patinir_weltlandschaft_pass()     — NEW (session 66) Joachim Patinir
                                        Weltlandschaft three-zone atmospheric
                                        recession: warm ochre foreground →
                                        olive-green middle distance → cool
                                        blue-grey far distance.  Applied to the
                                        background landscape zones of the
                                        portrait — exactly the kind of
                                        'world landscape' Patinir pioneered and
                                        that Leonardo employed behind the Mona
                                        Lisa.

  - warm_cool_form_duality_pass()     — NEW (session 66) artistic improvement.
                                        Applies the universal warm-light /
                                        cool-shadow temperature duality to every
                                        pixel: lit surfaces pushed warmer (R↑
                                        B↓), shadow surfaces pushed cooler (R↓
                                        B↑).  This reinforces three-dimensional
                                        form throughout the entire painting
                                        without overriding any artist-specific
                                        colour decisions.
...
Session 66 artistic character:
  The random artist inspiration for this session is Joachim Patinir
  (c. 1480–1524), the Flemish pioneer of 'world landscape' painting.

  Patinir is the direct ancestor of the Mona Lisa's background.  Leonardo's
  dreamlike geological wilderness — the winding path on the left, the river
  in the middle distance, the pale dissolving far distance — is almost exactly
  the structure Patinir codified as Weltlandschaft.  Whether Leonardo and
  Patinir were aware of each other is uncertain (they were exact contemporaries),
  but the compositional idea is the same: a figure in the foreground seated
  against an impossibly vast, imaginary landscape viewed from a high, impossible
  vantage point.

  The patinir_weltlandschaft_pass() applies Patinir's three-zone colour logic
  to the portrait background: the lower landscape (rocky crags, winding path)
  is pushed toward warm sandy ochre; the middle zone (river, vegetation) toward
  muted olive-green; and the sky/far-distance toward pale cool blue-grey.
  Applied at opacity=0.52 after the Canaletto veduta pass, this deepens the
  spatial recession of the background while harmonising with the existing
  aerial perspective treatment.

  The session 66 artistic improvement is warm_cool_form_duality_pass() — the
  most universal principle in oil-painting technique.  Virtually every great
  traditional painter from Titian to Sargent applied warm-light / cool-shadow
  temperature contrast to enforce the sensation of rounded, three-dimensional
  form.  On the Mona Lisa portrait this means: the forehead and cheekbone
  highlights acquire a very slightly warmer (golden) quality; the shadow side
  of the face and deep dress shadows acquire a slightly cooler (blue-shifted)
  quality.  Applied at moderate opacity (0.52) after all artist-specific passes
  and before the final varnish, it acts as a tonal unifier that makes every
  form in the painting feel physically substantial.
"""
...
# (same composition as s65 — identical portrait, new passes applied)
# ─────────────────────────────────────────────────────────────────────────────
...
def paint(out_dir: str = ".") -> str:
    ...
    ref = make_reference()
    ref_path = os.path.join(out_dir, "mona_lisa_s66_ref.png")
    ref.save(ref_path)
    ...
    base_path = os.path.join(out_dir, "mona_lisa_s66_base.png")
    p.save(base_path)
    ...
    # Applied immediately after the Canaletto veduta pass to deepen the three-
    # zone colour recession of the background landscape.  horizon_near=0.52
    # (below the figure's shoulder line) keeps the warm push from entering the
    # figure zone; horizon_far=0.68 covers the mid-distance vegetation band.
    # opacity=0.52 is moderate — the Patinir effect should enrich the landscape
    # without competing with the sfumato and aerial haze that Leonardo applied
    # to dissolve the background.
    ...
    # ── Session 66: Warm/cool form duality pass (artistic improvement) ────────
    # Applied after all artist-specific colour passes and the crystalline
    # surface pass but before the final varnish.  At this stage all the form
    # and colour decisions are locked in; this pass acts as a tonal unifier
    # that makes every form in the painting feel physically substantial.
    # midtone=0.48 (slightly below 0.5) keeps the warm push from encroaching
    # on the shadow side.  opacity=0.52 is conservative — the effect should
    # be felt, not seen.
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s66_final.png")
    p.save(out_path)

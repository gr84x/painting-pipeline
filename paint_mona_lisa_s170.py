"""
paint_mona_lisa_s170.py — Mona Lisa portrait for session 170.
...
Artistic discovery (session 170): Carel Fabritius (1622–1654)
  - Dutch Golden Age painter, born Midden-Beemster; studied under Rembrandt
    in Amsterdam c.1641–1643; later settled in Delft
  - Died in the catastrophic Delft Thunderclap gunpowder explosion, 12 October
    1654 — only a handful of works survive (c.12 authenticated paintings)
  - Widely regarded as the crucial artistic bridge between Rembrandt and Vermeer
  - His great technical innovation: inverted tenebrism — use of a PALE, near-
    white or light grey ground instead of Rembrandt's dark umber imprimatura
  - This creates the defining Delft effect: figures sit dark-against-light rather
    than light-against-dark; the background GLOWS; the pale ground radiates
    ambient luminosity through thin paint layers
  - Most famous surviving work: 'The Goldfinch' (1654) — a chained goldfinch
    perched against a bleached, warm-ivory plaster wall that is brighter than the
    bird itself; the ultimate inversion of Rembrandtesque tenebrism
  - His palette: restrained, chalky, ambient; greys, ivory whites, warm umbers,
    soft olive greens — no saturated primaries; no dramatic candlelight
  - Brushwork visible but controlled; not Rembrandt's heavy impasto, not
    Vermeer's invisible surface; an honest, confident middle ground
  - 'A View in Delft' (c.1652) — extraordinary trompe-l'oeil with forced
    perspective; painted on a curved panel for a specific viewing angle

New pipeline enhancement (session 170):
  - fabritius_pale_ground_pass: SIXTY-FIRST DISTINCT MODE
    INVERTED TENEBRISM — LUMINANCE-RAISING SHADOW LIFT:
    (1) Dark zone gate: dark_gate = clip((dark_hi − luma)/dark_hi, 0,1)^power
        Peaks at luma=0; falls to 0 at luma=dark_hi
    (2) Pale ground lift: out_c = c0 + (pale_c − c0) × dark_gate × lift_strength
        Interpolates dark pixels toward pale warm-ivory ground color
    — FIRST and ONLY pass to deliberately BRIGHTEN dark zones toward a
    lighter target.  All prior shadow-zone passes (tenebrism, candlelight,
    void deepening, shadow compression) DARKEN; this one BRIGHTENS.

  - luminance_gradient_warmth_pass: SIXTY-SECOND DISTINCT MODE
    PENUMBRAL ZONE WARM TINTING VIA LUMINANCE GRADIENT MAGNITUDE:
    (1) Pre-blur luma: luma_smooth = gaussian(luma, sigma=grad_sigma)
    (2) Sobel gradient magnitude: dx, dy → grad_mag = sqrt(dx²+dy²)
        Normalise: grad_norm = grad_mag / max(grad_mag)
    (3) Penumbral gate: pen_gate = grad_norm^grad_gamma
        Peaks at sharp light/shadow transitions; near-zero in flat zones
    (4) Warm amber injection: out_r += warm_r × pen_gate × luma_gate
        out_g += warm_g × pen_gate × luma_gate
    — FIRST and ONLY pass to gate chromatic effect on SPATIAL GRADIENT
    MAGNITUDE of luminance (rate of change), not luminance level itself.
"""
...
def paint(output_path: str = "mona_lisa_s170.png") -> str:
    """Full Leonardo sfumato pipeline with session 170 Fabritius pale-ground passes."""
    print("=" * 64)
    print("  Session 170 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: Carel Fabritius (1622–1654)")
    print("    Delft School — inverted tenebrism; pale warm ground")
    print("    SIXTY-FIRST MODE: fabritius_pale_ground_pass")
    print("    — FIRST pass to LIFT dark zones toward lighter target")
    print("    SIXTY-SECOND MODE: luminance_gradient_warmth_pass")
    print("    — warm amber tint gated on luminance gradient magnitude")
    print("=" * 64)

    ref = build_reference(W, H)
    ref.save("mona_lisa_s170_reference.png")
    print(f"  Reference saved -> mona_lisa_s170_reference.png  ({W}x{H})")

    leo = get_style("leonardo")
    p = Painter(W, H)

    print("  [1/23] Tone ground ...")
    p.tone_ground(leo.ground_color, texture_strength=0.06)

    print("  [2/23] Underpainting ...")
    p.underpainting(ref, stroke_size=52, n_strokes=160)

    print("  [3/23] Block-in ...")
    p.block_in(ref, stroke_size=34, n_strokes=340)

    print("  [4/23] Build form ...")
    p.build_form(ref, stroke_size=10, n_strokes=1400)

    print("  [5/23] Sfumato veil pass ...")
    p.sfumato_veil_pass(
        ref,
        n_veils       = 10,
        blur_radius   = 14.0,
    ...
    print("  [6/23] Edge sfumato dissolution pass ...")
    ...
    print("  [7/23] Skin subsurface scatter pass ...")
    ...
    print("  [8/23] Tonal envelope pass ...")
    ...
    print("  [9/23] Selective focus pass ...")
    ...
    print("  [10/23] Massys bridge glazing pass ...")
    ...
    print("  [11/23] Reynolds Grand Manner mezzotint tone pass ...")
    ...
    print("  [12/23] Schedoni luminous emergence pass ...")
    ...
    print("  [13/23] Warm highlight bloom pass ...")
    ...
    print("  [14/23] Shadow color temperature pass ...")
    ...
    print("  [15/23] Film grain overlay pass ...")
    ...
    print("  [16/23] Van der Werff ivory alabaster pass ...")
    ...
    print("  [17/23] Craquelure texture pass ...")
    ...
    print("  [18/23] Clouet enamel precision pass ...")
    ...
    print("  [19/23] Wtewael copper jewel pass ...")
    ...
    print("  [20/23] Altdorfer forest atmosphere pass ...")
    ...
    print("  [21/23] Fabritius pale ground pass — SIXTY-FIRST DISTINCT MODE ...")
    # Fabritius's inverted tenebrism: lift shadow zones toward a pale warm ivory
    # ground rather than deepening them toward void.  Applied conservatively so
    # Leonardo's rich chiaroscuro is not erased — only the very darkest zones
    # (dress, background depth, shadowed landscape) are gently lifted toward
    # the luminous pale ground.  The result subtly brightens the dreamlike
    # landscape background while the flesh tones remain unaffected (they are
    # above dark_hi).
    ...
    print("  [22/23] Luminance gradient warmth pass — SIXTY-SECOND DISTINCT MODE ...")
    # Penumbral warm tinting: inject subtle amber at the transitional zones
    # between light and shadow, gated on the spatial gradient magnitude of
    # luminance.  This adds warmth specifically at tonal boundaries — the
    # penumbral fringe around the figure, the edge of the veil, the boundary
    # of the landscape — without affecting flat bright or flat dark areas.
    # Applied at restrained strength to complement rather than overpower the
    # existing sfumato edge dissolution.
    ...
    print("  [23/23] Place lights, final glaze + finish ...")
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s170.png"
    paint(out)

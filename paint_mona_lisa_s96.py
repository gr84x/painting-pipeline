...
with session-96 additions:
  ...
  - hals_alla_prima_vivacity_pass()                        -- NEW (session 96) Frans Hals.
                                                             Three-stage Dutch alla prima
                                                             technique: (1) Directional
                                                             tache energy -- the pipeline's
                                                             first pass to work with the
                                                             luminance gradient direction
                                                             field; sine modulation orthogonal
                                                             to the gradient direction adds
                                                             rhythmic luminance variation
                                                             that reads as confident,
                                                             contour-following brushmarks
                                                             (taches); (2) Warm imprimatura
                                                             vivacity -- in mid-luminance
                                                             flesh zones, a gentle R lift
                                                             simulates the amber-brown ground
                                                             showing through thin alla prima
                                                             paint; (3) Focal psychological
                                                             crispness -- subtle unsharp mask
                                                             in the face region heightens the
                                                             direct psychological presence and
                                                             eye contact that defines Hals.

  SESSION 96 ARTISTIC IMPROVEMENT -- directional tache energy:
                                                             The hals_alla_prima_vivacity_pass()
                                                             introduces a genuinely new spatial
                                                             concept to the pipeline: working
                                                             with the luminance gradient
                                                             direction field rather than
                                                             luminance magnitude thresholds
                                                             or spatial positions.  Prior
                                                             sessions have accumulated
                                                             luminance-threshold effects
                                                             (cool shadow veil, warm mid-tone
                                                             lifts), spatial-position effects
                                                             (face centre masks, de Hooch
                                                             threshold light), and edge-
                                                             detection effects (Fabritius
                                                             contre-jour bloom).  The Hals
                                                             pass adds a direction-field
                                                             concept: gradient direction at
                                                             each pixel tells us which way
                                                             the form is 'going', and the
                                                             orthogonal direction is the
                                                             implied brushstroke direction.
                                                             A sine modulation along this
                                                             orthogonal direction creates
                                                             rhythmic variation that reads as
                                                             confident, form-following mark-
                                                             making -- the visual signature
                                                             of Hals's tache surface -- without
                                                             adding literal brushstrokes.
...
Session 96 artistic character:
  The random artist for this session is Frans Hals (1582/3–1666), the great
  Haarlem master of alla prima portraiture and the first painter to fully
  embrace the visible, directional brushmark as an expressive tool in its own
  right.  Where the accumulated sessions 80–95 have enriched the portrait with
  sfumato, glazing, contre-jour, and fijnschilder fineness, Hals contributes
  the opposite impulse: spontaneity, directness, and the mark showing.

  Hals's most famous work, The Laughing Cavalier (1624, Wallace Collection),
  demonstrates his method with exemplary clarity.  The subject is not actually
  laughing — his expression is more of a smirk, a contained amusement — but
  the painting feels alive in a way that 16th-century portraiture never
  managed.  This vitality comes from the taches: each element of the
  embroidered sleeve is resolved in three or four confident brushstrokes of
  ochre, cream, and gold, placed wet-on-wet and never overworked.  Viewed up
  close, the surface is a sequence of assured marks; at normal viewing distance
  it reads as the most vivid portraiture of its century.

  Hals's late works — the two paired group portraits of the Regents and
  Regentesses of the Old Men's Almshouse (c. 1664, Frans Hals Museum,
  Haarlem) — push his alla prima technique to its limit.  Painted when he was
  over eighty, they are almost abstract in their directness: the figures are
  built from very few marks, mostly black and grey with sudden cream highlights.
  Van Gogh, standing before them in 1882, counted 27 different blacks and was
  overwhelmed.  Manet cited Hals as the primary influence on his own direct,
  spontaneous handling.

  In the context of the Mona Lisa portrait by session 96, the cumulative
  effect of 95 sessions of glazing, sfumato, and atmospheric refinement has
  created a richly layered surface.  Hals's contribution — the
  hals_alla_prima_vivacity_pass() — introduces a counter-pressure: where
  previous sessions have smoothed and blended, Hals re-animates the surface
  with directional energy.  The tache modulation adds a very subtle rhythmic
  variation to the mid-tones that reads as life, spontaneity, and the sense
  that the paint was applied with conviction rather than caution.

  The directional gradient field concept is structurally new in the pipeline.
  Previous spatial passes have operated on:
    - Luminance magnitude (shadow/highlight thresholds)
    - Spatial position (face centre masks, de Hooch threshold light)
    - Edge detection (Fabritius contre-jour bloom, edge_lost_and_found)
  The Hals pass operates on luminance gradient direction — the angle at which
  the form is oriented — and modulates along the orthogonal brushstroke axis.
  This is a genuinely new category of spatial operation.
"""
...
def paint(out_dir: str = ".") -> str:
    ...
    print("Random artist: Frans Hals (Dutch Golden Age / Bravura Portrait, 1582/3-1666)")
    print("New pass: hals_alla_prima_vivacity_pass()")
    print("  -- Directional tache energy: sine modulation orthogonal to luminance gradient")
    print("  -- Warm mid-tone vivacity: imprimatura R lift in alla prima flesh zones")
    print("  -- Focal psychological crispness: unsharp mask in face region")
    print()
    print("Session 96 artistic improvement: directional gradient field (tache energy)")
    print("  First pipeline pass to work with luminance gradient direction rather than")
    print("  magnitude thresholds or spatial positions -- a genuinely new spatial concept.")
    print()
    ...
    # Cool atmospheric recession pass
    print("Cool atmospheric recession pass (session 74, improved s87)...")
    ...
    # Parmigianino serpentine elegance pass
    ...
    # Vigee Le Brun pearlescent grace pass
    ...
    print("Subsurface scatter pass (session 65, improved s92)...")
    ...
    # Alma-Tadema marble luminance pass
    ...
    # David neoclassical clarity pass
    ...
    # Correggio golden tenderness pass
    ...
    # Luminous haze pass
    print("Luminous haze pass (session 71, improved s88)...")
    ...
    # Poussin classical clarity pass
    ...
    # Andrea del Sarto golden sfumato pass
    ...
    # Chardin granular intimacy pass
    ...
    # Gericault romantic turbulence pass
    ...
    # Sfumato veil pass (s82 improved s89)
    ...
    # Fra Filippo Lippi tenerezza pass
    ...
    # Signorelli sculptural vigour pass
    ...
    # Spilliaert vertiginous void pass
    ...
    # Antonello da Messina pellucid flesh pass
    ...
    # Hugo van der Goes expressive depth pass
    ...
    # ── SESSION 96: Frans Hals alla prima vivacity pass (NEW) ─────────────────
    # Frans Hals (1582/3–1666) — Dutch Golden Age bravura portrait master.
    # Three operations: directional tache energy (gradient-direction field modulation),
    # warm imprimatura vivacity in mid-tones, and focal psychological crispness.
    print("Frans Hals alla prima vivacity pass (session 96 -- NEW)...")
    p.hals_alla_prima_vivacity_pass(
        face_cx          = 0.515,   # face centre X
        face_cy          = 0.215,   # face centre Y
        face_rx          = 0.18,    # face ellipse half-width
        face_ry          = 0.22,    # face ellipse half-height
        tache_strength   = 0.022,   # directional luminance modulation amplitude
        tache_sigma      = 2.4,     # gradient smoothing sigma
        tache_frequency  = 14.0,    # brushstroke rhythm frequency
        vivacity_lo      = 0.35,    # vivacity zone lower lum bound
        vivacity_hi      = 0.78,    # vivacity zone upper lum bound
        warm_r_lift      = 0.018,   # imprimatura warmth lift in mid-tones
        shadow_thresh    = 0.30,    # shadow warm lift threshold
        shadow_warm_r    = 0.012,   # warm ground reading through thin paint
        crisp_sigma      = 1.0,     # focal unsharp mask sigma
        crisp_amount     = 0.14,    # focal unsharp mask amplitude
        blur_radius      = 4.0,
        opacity          = 0.35,
    )

    # Edge lost-and-found pass (s91 improved)

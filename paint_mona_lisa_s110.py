"""
paint_mona_lisa_s110.py -- Session 110 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–110 additions cumulatively.

Session 110 additions:
  - sassoferrato                             -- NEW (session 110)
                                               Giovanni Battista Salvi
                                               da Sassoferrato
                                               (1609–1685), Roman Baroque
                                               / Devotional Classicism.
                                               Born in the small Marche
                                               town of Sassoferrato, he
                                               worked in Rome throughout
                                               the seventeenth century
                                               producing devotional
                                               Madonnas and saints whose
                                               chromatic purity stands
                                               entirely apart from the
                                               Baroque mainstream.
                                               His ultramarine blue is
                                               the most saturated in
                                               Western painting — pure
                                               lapislazuli built through
                                               multiple glaze layers over
                                               warm ochre imprimatura so
                                               that the colour glows from
                                               within.  His flesh tones
                                               have a cool pearl quality
                                               at the peak highlights
                                               suggesting light through
                                               translucent skin rather
                                               than reflected from
                                               opaque pigment.
                                               His "Virgin in Prayer"
                                               (c. 1640–1650, National
                                               Gallery, London) is the
                                               canonical work: the most
                                               purely ultramarine blue
                                               in oil painting, set
                                               against ivory-warm flesh
                                               with cool pearl highlights.

  SESSION 110 ARTISTIC IMPROVEMENT -- sassoferrato_pure_devotion_pass:

                                               New pass encoding three
                                               defining Sassoferrato
                                               qualities:
                                               (1) Ultramarine purity —
                                               in blue-dominant zones
                                               (b0 > r0 + ultra_thresh,
                                               b0 > g0 + ultra_thresh),
                                               lifts blue and damps red
                                               contamination toward pure
                                               lapislazuli glow;
                                               (2) Porcelain skin glow —
                                               in upper highlight zone
                                               (lum > pearl_lo ≈ 0.76),
                                               applies cool pearl shift
                                               (R - pearl_r_damp,
                                               B + pearl_b_lift) giving
                                               the impression of light
                                               through translucent skin;
                                               (3) Ultramarine shadow
                                               depth — in shadow zones
                                               already blue-dominant,
                                               deepens toward lapislazuli
                                               darkness with violet-blue
                                               enrichment.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s110 (Sassoferrato pure devotion -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–110 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 110 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Sassoferrato pure devotion (110 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 110: Sassoferrato pure devotion pass (NEW) ───────────────────
    # "Pure devotion" — Sassoferrato's Roman Baroque devotional classicism.
    # Three-stage effect complementing the accumulated warm passes:
    #
    # (1) Ultramarine purity — the cool blue-grey of the background sky and
    #     the dark blue-green dress contain blue-dominant passages; this pass
    #     enriches those zones toward pure lapislazuli glow (B + ultra_b_lift,
    #     R - ultra_r_damp).  Applied at ultra_thresh=0.04 to catch genuine
    #     blue zones without affecting the warm flesh or amber passages.
    #
    # (2) Porcelain skin glow — the accumulated warm layers (Strozzi amber,
    #     Dolci enamel) have built deep warmth in the flesh; Sassoferrato's
    #     cool pearl shift at the peak highlights (lum > 0.76) adds a final
    #     translucent quality — light through skin, not just from skin.
    #     Applied very gently (pearl_r_damp=0.007, pearl_b_lift=0.010) to
    #     refine without disturbing the warm balance.
    #
    # (3) Ultramarine shadow depth — in the shadow zones already blue-dominant
    #     (the atmospheric sky, the dark dress, the background haze), deepen
    #     toward lapislazuli darkness: B + shadow_b_lift=0.016,
    #     R - shadow_r_damp=0.008.  This clarifies the characteristic
    #     Sassoferrato quality: even the darkest passages of blue retain a
    #     chromatic richness, not descending into neutral grey or black.
    print("Sassoferrato pure devotion pass (session 110 -- NEW)...", flush=True)
    ...
    # Final warm amber glaze — maintaining Strozzi's warmth with Sassoferrato's cool refinement
    print("Final glaze (warm amber, balanced for Strozzi warmth and Sassoferrato purity)...", flush=True)
    p.glaze((0.60, 0.44, 0.14), opacity=0.036)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.50, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s110.png")
    p.save(out_path)

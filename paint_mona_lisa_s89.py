"""
paint_mona_lisa_s89.py -- Session 89 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-89 additions:

  - spilliaert_vertiginous_void_pass()               -- NEW (session 89) Léon Spilliaert.
                                                       Three-stage tonal architecture:
                                                       (1) shadow compression —
                                                       push dark pixels toward near-black
                                                       with a cold blue-grey ink cast,
                                                       removing residual oil warmth from
                                                       the darkest voids; (2) pale
                                                       isolation — lift bright areas
                                                       toward cool ivory-grey, isolating
                                                       the figure as a luminous island
                                                       against the dark field; (3)
                                                       peripheral vignette deepening —
                                                       darken the canvas perimeter toward
                                                       near-absolute black, reinforcing
                                                       vertiginous geometric depth.

  - sfumato_veil_pass (chroma_gate=0.42)             -- SESSION 89 ARTISTIC IMPROVEMENT.
                                                       Chroma-gated sfumato: high-chroma
                                                       (saturated) areas of the reference
                                                       receive proportionally less sfumato
                                                       blending.  Leonardo applied sfumato
                                                       most heavily to near-neutral
                                                       transitions (mouth corners, temples,
                                                       shadow edges) rather than to the
                                                       chromatic accents (warm dress
                                                       fabric, rose lips, landscape
                                                       greens).  At chroma_gate=0.0 the
                                                       behaviour is identical to all prior
                                                       sessions; at chroma_gate=0.42 the
                                                       sfumato is gated back by up to 42%
                                                       in the most saturated zones while
                                                       running at full strength across the
                                                       grey-amber transitions.

  Sessions 88 and earlier (retained):
...
Session 89 artistic character:
  The random artist for this session is Léon Spilliaert (1881–1946), the
  Belgian Symbolist whose work stands apart from every contemporary movement
  — too withdrawn for Expressionism, too geometric for Symbolism, too dark
  for Impressionism, too personal for any school.

  Spilliaert worked on the seafront at Ostend, often at night, almost always
  alone.  He suffered from a chronic stomach condition that made normal social
  life difficult and kept him awake through the small hours.  His insomnia
  is the single most important biographical fact about his art: the paintings
  he made between 1900 and 1912 are lit not by day but by the particular
  quality of artificial light against total darkness — the sharp, acid yellow
  of a street lamp against a void of cold blue-black sea.  He worked in
  Indian ink, watercolour, and pastel, often combined in the same work,
  building his near-monochromatic surfaces from transparent washes of cold
  grey over the deep near-black ink scaffolding.

  Two visual ideas define Spilliaert's work with unusual consistency:

  1. The near-monochrome tonal field.  His palette is almost entirely
     composed of values rather than colours.  Near-black, cold dark grey,
     warm mid-grey, pale ivory, near-white — with a single chromatic accent
     (acid yellow, electric blue, muted mauve) inserted as an event rather
     than as description.  The effect is of a world that has been drained of
     colour except at one carefully chosen point.

  2. Vertiginous geometric depth.  Where other painters constructed depth
     through aerial perspective or tonal graduation, Spilliaert used the
     perspective conventions of architecture: corridors, staircases, seawall
     railings, promenade tiles.  His most celebrated image, 'Dizziness'
     (1908), is a spiral staircase descending into a void so absolute that
     the receding forms lose all scale reference.  The viewer's sense of
     depth collapses — it is simultaneously a staircase and an abyss.

  The connection between Spilliaert's visual world and the accumulated
  portrait is precise.  After eighteen sessions, the composition has
  extraordinary tonal depth: Géricault's shadow drama, Redon's velvet void
  enrichment, Whistler's cool tonal harmony.  What Spilliaert adds is tonal
  compression — a further push of the existing darks toward near-absolute
  black, with a cold blue-grey undertone that distinguishes Spilliaert's
  ink black from the warm oil-paint darkness accumulated across the previous
  sessions.

  Three operations address this:

  1. Shadow compression pushes the portrait's dark zones — already warmed
     by Géricault's shadow_warm_recovery and Redon's velvet void — toward
     the characteristic cold near-black of Spilliaert's Indian ink ground.
     Red and green channels are slightly damped; blue is lifted.  The effect
     is not coldness but austerity: the warmth remains in the midtones while
     the deepest darks become precisely, beautifully dark.

  2. Pale isolation lifts the bright passages (the face highlights, the
     gauze luminosity, the hand surfaces) toward a cool ivory-grey — the
     pale forms now emerge more insistently from the dark field, the
     characteristic 'figure against void' quality of Spilliaert's
     self-portraits.

  3. Peripheral vignette deepening extends the vignette already present
     in the finish() call, darkening the canvas perimeter further toward
     near-black and drawing the eye more forcefully toward the focal zone.

  The session 89 artistic improvement — chroma_gate in sfumato_veil_pass()
  — corrects a subtle imprecision in all prior sessions: the sfumato veil
  was applied with uniform opacity across the composition, treating a
  saturated passage of warm dress fabric the same as a grey-amber shadow
  transition.  But Leonardo's sfumato is selective.  Multispectral analysis
  of the Mona Lisa (Louvre 2004) confirms that the highest-saturation areas
  (the brown-green dress, the warm veil over the hair) show the fewest glaze
  layers and sharpest edges; the lowest-saturation areas (the mouth corners,
  the eye corners, the transition from face to hair) show the most layers
  and the most extreme blurring.  The chroma_gate parameter now applies this
  selectivity automatically: at gate=0.42, the sfumato opacity is reduced
  by up to 42% in the most saturated zones while running at full strength
  across the neutral grey-amber transitions.
"""
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-89 painting pipeline."""
    print("Session 89 -- Mona Lisa portrait")
    print("Random artist: Léon Spilliaert (Belgian Symbolism, 1881–1946)")
    print("New pass: spilliaert_vertiginous_void_pass()")
    print("  -- shadow compression: push darks toward cold near-black ink void")
    print("  -- pale isolation: lift bright areas toward cool ivory-grey island")
    print("  -- peripheral vignette deepening: reinforce vertiginous geometric depth")
    print()
    print("Session 89 artistic improvement: chroma_gate in sfumato_veil_pass()")
    print("  Leonardo applied sfumato most heavily to near-neutral transitions,")
    print("  not to saturated chromatic accents.  chroma_gate=0.42 gates the")
    print("  sfumato opacity back by up to 42% in high-chroma zones, preserving")
    print("  chromatic identity while still dissolving grey-amber transitions.")
    print()
    ...
    # chroma_gate=0.42: Leonardo applied sfumato most heavily to near-neutral
    # transitions, not to saturated chromatic accents.  Sfumato is gated back
    # by up to 42% in high-chroma zones, preserving chromatic identity while
    # still dissolving the grey-amber transitions.
    ...
    p.sfumato_veil_pass(
        ...
        chroma_gate          = 0.42,   # session 89: gate sfumato away from saturated zones
    )
    ...
    # ── SESSION 89: Spilliaert vertiginous void pass (NEW) ────────────────────
    # Léon Spilliaert (1881–1946) — Belgian Symbolist.
    # Three operations: shadow compression (cold ink black), pale isolation,
    # peripheral vignette deepening.
    print("Spilliaert vertiginous void pass (session 89 -- NEW)...")
    p.spilliaert_vertiginous_void_pass(
        void_thresh       = 0.30,    # luminance ceiling for shadow compression
        void_cool_b       = 0.028,   # cold blue-grey ink cast in deep voids
        void_damp_r       = 0.014,   # remove residual oil-paint warmth from darks
        void_damp_g       = 0.008,   # Spilliaert's ink black is blue, not olive
        pale_thresh       = 0.74,    # luminance floor for pale isolation
        pale_grey_lift    = 0.016,   # lift pale areas toward cool ivory-grey
        pale_cool_b       = 0.010,   # ink-on-paper white is cooler than oil white
        vignette_strength = 0.16,    # peripheral near-black deepening
        vignette_radius   = 0.58,    # begin vignette well inside the composition
        blur_radius       = 9.0,
        opacity           = 0.38,
    )
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s89.png")
    p.save(out_path)

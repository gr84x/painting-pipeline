"""
paint_mona_lisa_s63.py — Session 63 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
enhanced with session-63 additions:

  - canaletto_luminous_veduta_pass()   — NEW (session 63) three-register Venetian
                                         atmospheric clarity: cerulean sky lift,
                                         warm stone push, cool canal-silver pull.
                                         Applied to the background landscape to
                                         give it Canaletto's luminous, crystalline
                                         depth — a counterpoint to the sfumato
                                         softness of the figure.

  - old_master_varnish_pass()          — NEW (session 63) aged amber varnish
                                         simulation: warm amber tint, edge
                                         oxidation darkening, highlight desaturation.
                                         Applied very lightly (opacity=0.28) as the
                                         final tonal integration step — the sense
                                         of three centuries of museum varnish that
                                         unifies figure and landscape into a single
                                         golden key.
...
Session 63 artistic character:
  Canaletto's influence is felt in the landscape behind the figure.  Where prior
  sessions rendered the background as an undifferentiated sfumato haze, session 63
  introduces the Venetian vedutismo reading: the sky zone above the horizon gains a
  saturated cerulean lift, the rocky crags and paths in the mid-distance are warmed
  toward honey-stone, and the river/water zone is pulled toward cool silver-blue.
  This creates a more luminous, more architecturally present background — as if the
  figure sits before an actual geological landscape rather than a purely atmospheric
  one.  The old_master_varnish_pass() then fuses figure and landscape into a single
  warm amber key, undoing any chromatic separation the canaletto pass may have
  introduced, and adding the sense of aged varnish that is inseparable from the
  experience of the Mona Lisa in the Louvre.
"""
...
# (identical composition to s62 — same portrait, new passes applied)
# ─────────────────────────────────────────────────────────────────────────────
...
def paint(out_dir: str = ".") -> str:
    ...
    ref = make_reference()
    ref_path = os.path.join(out_dir, "mona_lisa_s63_ref.png")
    ref.save(ref_path)
    ...
    base_path = os.path.join(out_dir, "mona_lisa_s63_base.png")
    p.save(base_path)
    print(f"  Base painting saved: {base_path}")

    # ── Session 63: Canaletto luminous veduta pass ────────────────────────────
    # Applied to the background landscape zone — the session 63 key departure
    # from prior sessions.  Canaletto's influence is felt in the background:
    # the sky gains a saturated cerulean lift (the clearest sky tone in the
    # Western landscape tradition), the rocky crags and paths in the mid-distance
    # are warmed toward honey-stone, and the river zone is pulled toward cool
    # silver-blue.  sky_lift=0.14 is kept modest — this is sfumato country, not
    # the crisp Venetian noon; we want Canaletto's chromatic clarity applied at
    # sfumato scale, not his architectural precision.  The pass is deliberately
    # restrained relative to its default so that the background reads as more
    # luminous without losing its dreamlike geological quality.
    print("Canaletto luminous veduta pass (session 63)…")
    p.canaletto_luminous_veduta_pass(
        sky_lift   = 0.14,    # modest — sfumato country, not Venetian noon
        stone_warm = 0.10,    # warm the rocky crags toward honey-stone
        water_cool = 0.10,    # cool the river toward canal silver
        sky_band   = 0.50,    # sky zone covers top half of the canvas
        opacity    = 0.58,
    )
    ...
    p.parmigianino_serpentine_elegance_pass(
        porcelain_strength = 0.11,
        lavender_shadow    = 0.09,
        silver_highlight   = 0.07,
        opacity            = 0.62,
    )
    ...
    # Applied last — the tonal unifier.  After canaletto_luminous_veduta_pass()
    # has introduced chromatic distinctions between sky, stone, and water in the
    # background, old_master_varnish_pass() draws the whole image back together
    # under a warm amber key — the same varnish patina that the Louvre Mona Lisa
    # carries.  amber_strength=0.12 is deliberately light (the default is 0.18);
    # we want the sense of aged varnish rather than a yellowed-distorted picture.
    # The edge darkening (0.10) reinforces the natural vignette of the composition.
    print("Old-master varnish pass (session 63)…")
    p.old_master_varnish_pass(
        amber_strength  = 0.12,   # light amber wash — museum patina
        edge_darken     = 0.10,   # gentle edge oxidation
        highlight_desat = 0.06,   # ivory highlights, not pure white
        opacity         = 0.28,   # subtle: present but not dominant
    )
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s63_final.png")
    p.save(out_path)

"""
paint_mona_lisa_s160.py — Mona Lisa portrait for session 160.
...
Pipeline enhancements (session 159–160):
  - skin_subsurface_scatter_pass: warm-red translucency in flesh zones
  - edge_sfumato_dissolution_pass: gradient-edge softening
  - moro_regal_presence_pass: tonal polarity amplification (very light touch)
"""
...
def paint(output_path: str = "mona_lisa_s160.png") -> str:
    """Full Leonardo sfumato pipeline with session 159–160 passes."""
    print("=" * 64)
    print("  Session 160 — Mona Lisa sfumato portrait")
    print("=" * 64)

    ref = build_reference(W, H)
    ref.save("mona_lisa_s160_reference.png")
    print(f"  Reference saved -> mona_lisa_s160_reference.png  ({W}×{H})")
    ...
    print("  [6/13] Edge sfumato dissolution pass (session 159) ...")
    ...
    print("  [7/13] Skin subsurface scatter pass (session 159) ...")
    ...
    print("  [8/13] Tonal envelope pass ...")
    ...
    print("  [9/13] Selective focus pass ...")
    ...
    print("  [10/13] Moro regal presence pass (subtle — tonal polish) ...")
    ...
    print("  [11/13] Place lights ...")
    p.place_lights(ref, stroke_size=5, n_strokes=320)

    print("  [12/13] Final glaze ...")
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s160.png"
    paint(out)

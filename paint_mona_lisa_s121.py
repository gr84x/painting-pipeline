"""
paint_mona_lisa_s121.py -- Session 121 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–121 additions cumulatively.

Session 121 additions:
  - boldini                                  -- NEW (session 121)
                                               Giovanni Boldini
                                               (1842–1931), Italian (Ferrarese,
                                               later Parisian).
                                               The 'Master of Swirl' — supreme
                                               virtuoso of Belle Époque
                                               portraiture in Paris.  Boldini
                                               combined the tonal grandeur of
                                               Old Master dark grounds with a
                                               new gestural freedom: diagonal
                                               swirling brushstrokes at two
                                               competing angles create a visual
                                               vibration in which the surface
                                               itself appears to be in motion.
                                               Only the face and hands emerge
                                               as resolved zones of luminous
                                               warmth; everything else dissolves
                                               into directional energy.  Famous
                                               sitters include the Marchesa
                                               Luisa Casati (1908), Madame
                                               Charles Max (1896), and the
                                               Duchess of Marlborough (1906).

  SESSION 121 ARTISTIC IMPROVEMENT -- boldini_swirl_bravura_pass:

                                               New pass encoding Boldini's
                                               defining visual signature: the
                                               dual-angle diagonal swirl field.

                                               Unlike Batoni's single-angle
                                               silk sheen (session 119), two
                                               anisotropic streak fields are
                                               computed at competing angles
                                               (~45° primary, ~-28° secondary).
                                               Each field is built via rotate
                                               → anisotropic Gaussian (σ_along=
                                               14px >> σ_across=1.5px) →
                                               rotate-back.  Combined at
                                               primary + secondary×0.50, the
                                               fields produce warm ivory
                                               brightness along two crossing
                                               axes — the visual complexity of
                                               superimposed directional
                                               brushstrokes.

                                               Also: warm flesh luminosity lift
                                               (R+0.015, G+0.008, B-0.005 in
                                               the 0.50–0.85 luminance zone)
                                               and background darkening
                                               (×0.93 + faint warm undertone)
                                               reinforcing the chestnut-umber
                                               deep ground from which Boldini's
                                               figures emerge luminously.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s121 (Giovanni Boldini dual-angle swirl bravura -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–121 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 121 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Giovanni Boldini dual-angle swirl bravura (121 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 121: Giovanni Boldini dual-angle swirl bravura pass (NEW) ─────
    # The defining technique of Belle Époque portraiture's greatest virtuoso.
    #
    # Two competing diagonal stroke directions (~45° primary, ~-28° secondary)
    # create the characteristic Boldini 'swirl': a visual vibration in which
    # the surface itself appears to be in motion.  Each streak field is built
    # via rotate → anisotropic Gaussian (σ_along=14px >> σ_across=1.5px) →
    # rotate-back, then combined at primary + secondary×0.50.
    #
    # Three stages:
    # 1. Dual swirl: warm ivory brightness modulation along crossing axes.
    # 2. Warm flesh lift: R+0.015, G+0.008, B-0.005 in the 0.50–0.85 zone.
    # 3. Background darkening: ×0.93 + faint warm undertone — the near-black
    #    chestnut ground from which Boldini's luminous figures emerge.
    print("Giovanni Boldini dual-angle swirl bravura pass (session 121 -- NEW)...", flush=True)
    ...
    # Final glaze — warm chestnut unifier (Boldini's ground warmth breathing through)
    print("Final glaze (warm chestnut for Boldini ground warmth)...", flush=True)
    p.glaze((0.50, 0.36, 0.16), opacity=0.014)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.52, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s121.png")
    p.save(out_path)

"""
paint_mona_lisa_s117.py -- Session 117 portrait painting.

Warm-start from mona_lisa_s101.png (the latest available base canvas),
then applies sessions 102–117 additions cumulatively.

Session 117 additions:
  - perugino                                 -- NEW (session 117)
                                               Pietro Perugino
                                               (c. 1446–1523), Italian
                                               Umbrian Renaissance /
                                               Proto-Classical.
                                               Teacher of Raphael and
                                               founder of the Umbrian
                                               school — a tradition of
                                               serene, harmonious
                                               painting defined by
                                               luminous ground warmth:
                                               the warm ochre
                                               imprimatura glowing
                                               softly through thin
                                               glazes, suffusing every
                                               passage with gentle
                                               inner warmth.  Open
                                               luminous landscapes,
                                               soft atmospheric
                                               ...
                                               that directly inspired
                                               his pupil Raphael.
                                               Famous for The Delivery
                                               of the Keys (Sistine
                                               Chapel), Apollo and
                                               Marsyas, and Portrait
                                               of Francesco delle
                                               Opere.

  SESSION 117 ARTISTIC IMPROVEMENT -- perugino_serene_grace_pass:
                                               ...
                                               window (broader and
                                               more gradual than
                                               Solario's sin window)
                                               encodes the diffuse,
                                               pervasive quality of
                                               Perugino's warmth —
                                               ambient ground field
                                               rather than targeted
                                               penumbra pulse.  This
                                               is the session 117
                                               improvement: sin²-
                                               windowed ground warmth,
                                               a more physically
                                               grounded model of
                                               imprimatura luminosity
                                               than the sin-windowed
                                               penumbra arc (s116);
                                               (2) Ivory highlight
                                               serenity — warm ivory-
                                               gold lift at peak flesh
                                               (lum > 0.70): R +
                                               ivory_r, G + ivory_g,
                                               B - ivory_b.  Warm,
                                               unassuming, harmonious
                                               — the serene Umbrian
                                               afternoon quality;
                                               (3) Warm amber shadow
                                               — raised-cosine amber
                                               ...
                                               (lum < 0.35): R +
                                               warm_r, G + warm_g.
                                               Unlike Solario's cool
                                               violet Venetian
                                               shadows, Perugino's
                                               darks retain warm
                                               amber-sienna Umbrian
                                               earth quality.
...
Applies: s102 (Tissot), s103 (Dolci), s104 (Giordano), s105 (Guercino),
         ...
         s117 (Pietro Perugino Umbrian serenity -- NEW)
"""
...
def paint(out_dir: str = ".") -> str:
    """
    Apply sessions 102–117 passes on top of the session-101 base canvas.

    Returns the output PNG path.
    """
    print("=" * 68, flush=True)
    print("Session 117 warm-start from mona_lisa_s101.png", flush=True)
    ...
    print("          Pietro Perugino Umbrian serenity (117 -- NEW)", flush=True)
    print("=" * 68, flush=True)
    print(flush=True)
    ...
    # ── SESSION 117: Pietro Perugino Umbrian serenity pass (NEW) ──────────────
    # The Umbrian serenity pass encodes Perugino's three defining qualities:
    #
    # (1) Luminous ground warmth — sin²-windowed warm ambient lift across the
    #     mid-tone zone [0.30, 0.75].  ground_r (0.013) and ground_g (0.006)
    #     model the warm ochre imprimatura glowing through thin glazes.  The
    #     sin²(π·t) window is broader and more gradual than the sin(π·t) of
    #     Solario's arc_window — encoding a more diffuse, ambient warmth field
    #     (the ground) rather than a targeted penumbra warmth pulse.
    #
    # (2) Ivory highlight serenity — warm ivory-gold at peak flesh (hi_lo=0.70).
    #     ivory_r (0.010) and ivory_g (0.006) lift the highlight to creamy
    #     ivory-gold — warmer than Boltraffio's pearl, less amber than Solario,
    #     less silvery than Moroni: the harmonious Umbrian afternoon quality.
    #
    # (3) Warm amber shadow — raised-cosine recovery below lum=0.35.
    #     warm_r (0.013) and warm_g (0.006) ensure the shadow zone never goes
    #     cold or violet (unlike Solario's Venetian shadows) — the warm amber-
    #     sienna quality of the Umbrian earth even in the darkest passages.
    print("Pietro Perugino Umbrian serenity pass (session 117 -- NEW)...", flush=True)
    ...
    # Final glaze — warm amber-gold reflecting Umbrian luminous ground warmth
    print("Final glaze (warm amber-gold for Umbrian luminous ground warmth)...", flush=True)
    p.glaze((0.60, 0.46, 0.20), opacity=0.022)

    # Vignette and crackle
    print("Finishing (vignette + crackle)...", flush=True)
    p.finish(vignette=0.52, crackle=True)

    out_path = os.path.join(out_dir, "mona_lisa_s117.png")
    p.save(out_path)

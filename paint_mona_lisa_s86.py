"""
paint_mona_lisa_s86.py -- Session 86 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-86 additions:

  - carriera_pastel_glow_pass()                 -- NEW (session 86) Rosalba Carriera.
                                                   Three-stage pearlescent glow:
                                                   (1) pearlescent skin brightening —
                                                   warm ivory-pink lift in the upper
                                                   mid-tone zone (luminance 0.55–0.85),
                                                   replicating the pale luminous bloom
                                                   of pastel pigment on warm vellum;
                                                   (2) highlight bloom diffusion —
                                                   Gaussian spread of the brightest
                                                   pixels into feathery powder halos,
                                                   Carriera's signature in contrast to
                                                   Vermeer's crisp pearl dots; (3)
                                                   background vignette cooling — shift
                                                   the background zone toward cool
                                                   lavender-grey, making the warm
                                                   luminous face advance.

  Sessions 85 and earlier (retained):
...
Session 86 artistic character:
  The random artist inspiration for this session is Rosalba Carriera
  (1673–1757), the Venetian Rococo pastellist who pioneered portrait on
  ivory, became the most celebrated portrait artist in Europe during the
  early eighteenth century, and introduced pastel portraiture to France
  on her triumphant 1720 Paris visit.

  Carriera stands in a fascinating relationship to the accumulated
  sessions before her.  Sessions 1–85 have built extraordinary depth
  through oil-painting techniques: sfumato dissolution (Leonardo),
  sculptural contour (Signorelli), turbulent chiaroscuro (Géricault),
  granular intimacy (Chardin), golden warmth (Correggio, del Sarto).
  Carriera's pastel medium offers something orthogonal to all of this:
  not depth through glazing or drama through chiaroscuro, but a
  luminous, powdery surface quality where the face appears self-
  luminous — lit from within rather than illuminated from without.

  What does Carriera contribute to the Mona Lisa context?

  The figure described in the prompt wears a translucent gauze wrap and
  a semi-transparent veil — textiles that in Carriera's world would be
  rendered with exquisite, feathery lightness.  The face has faint
  eyebrows and heavy-lidded eyes — a physiognomy that suits Carriera's
  softening technique, where features dissolve at their edges rather
  than asserting themselves with the sculptural firmness Signorelli
  (session 85) emphasised.

  The three-stage pass:

  1. Pearlescent skin brightening lifts the upper mid-tone zone toward
     ivory-pink warmth, approximating the warm-pink overstrokes on
     cool-lavender underlayers that define real pastel flesh.  After
     Signorelli's warm umber shadow enrichment (session 85), this pass
     brightens the lit zone without disturbing the shadow depth that
     the accumulated sessions have established.

  2. Highlight bloom diffusion spreads the brightest skin highlights
     into feathery halos rather than Vermeer's crisp pearl dots.  This
     is the most visually distinctive Carriera contribution: in the
     context of the accumulated sfumato layers, it gives the face a
     slightly dreamy, softly radiant quality — the skin appears to glow
     very gently from within, which is the pastel effect that made
     Carriera the most sought-after portraitist in Europe.

  3. Background vignette cooling pushes the already atmospheric
     background zone toward Carriera's characteristic cool lavender-
     grey, making the warm face advance with slightly more force.  The
     Patinir-style geological landscape and the atmospheric depth built
     by sessions 74 and beyond are subtly cooled and de-warmed at the
     periphery — the effect is to make the figure feel slightly more
     present and luminous against the dreamlike distance behind her.
"""
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-86 painting pipeline."""
    print("Session 86 -- Mona Lisa portrait")
    print("Random artist: Rosalba Carriera (Venetian Rococo, 1673–1757)")
    print("New pass: carriera_pastel_glow_pass()")
    print("  -- pearlescent skin brightening: ivory-pink lift in upper mid-tones")
    print("  -- highlight bloom diffusion: Gaussian spread of brightest pixels")
    print("  -- background vignette cooling: cool lavender shift to make face advance")
    print()
    ...
    # Session 85: Luca Signorelli sculptural vigour pass
    print("Luca Signorelli sculptural vigour pass (session 85)...")
    ...
    # Session 86 NEW: Rosalba Carriera pastel glow pass
    # Three-stage pearlescent glow inspired by Venetian Rococo pastel portraiture:
    # (1) Pearlescent skin brightening — warm ivory-pink lift in the upper mid-tone zone
    #     (luminance 0.55–0.85), replicating the luminous bloom of pastel on warm vellum.
    #     After Signorelli's warm umber shadow enrichment, this pass brightens the lit
    #     zone and adds a delicate lavender-pink undertone that distinguishes Carriera's
    #     pastel flesh from the golden warmth of oil-painting traditions.
    # (2) Highlight bloom diffusion — Gaussian spread of the brightest skin pixels into
    #     feathery halos.  Where Vermeer's pearls are crisp and Rembrandt's impasto
    #     highlights are physical ridges, Carriera's lights are powdery, diffused halos:
    #     the face appears to glow softly from within, the signature effect of dry pastel
    #     pigment scattering light through its own dust.
    # (3) Background vignette cooling — the background zone outside the figure mask
    #     receives a gentle shift toward cool lavender-grey, Carriera's characteristic
    #     neutral background tone that makes the warm luminous face advance.
    # Opacity 0.34 — Carriera's effect is cumulative and delicate; heavier application
    # would compromise the sfumato depth and shadow warmth built up by prior sessions.
    print("Rosalba Carriera pastel glow pass (session 86 NEW)...")
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s86.png")
    p.save(out_path)

"""
paint_mona_lisa_s180.py — Mona Lisa portrait for session 180.
...
Artistic discovery (session 180): Cesare da Sesto (c. 1477–1523)
  - Milanese-Roman Bridge Renaissance — the supreme synthesist of two great traditions
  - Trained in Leonardo's Milan circle (sfumato warmth, warm ivory flesh, glazed depth)
  - Traveled to Rome c. 1508–1510; absorbed Raphael's cool classical form clarity
  - Defining characteristic: warm Leonardesque flesh + cool Raphaelesque 'air between forms'
  - His 'Salome' (Vienna / Madrid, c. 1510–1520) — quintessential bridge synthesis
  - Palette: warm Milanese ivory, cool sky-blue highlights, Lombard green landscape

New pipeline enhancements (session 180):
  - cesare_da_sesto_clarity_pass: EIGHTIETH DISTINCT MODE
    MILANESE-ROMAN CLARITY BRIDGE:
    Uses Sobel gradient magnitude on luma to detect two simultaneously active zones:
    (1) LOW-GRADIENT SMOOTH ZONES: warm Leonardesque injection (R+warm_r, G+warm_g) —
        sfumato warmth flows through flesh, sky, and all smooth tonal areas.
    (2) MEDIUM-GRADIENT FORM BOUNDARIES: cool Raphaelesque 'air' injection via bell
        gate on gradient space (B+cool_b, R-cool_r) — luminous cool separation at
        the boundary between adjacent forms giving classical spatial legibility.
    NOVEL: FIRST pass to use GRADIENT MAGNITUDE as a BIMODAL gate applying OPPOSITE
    chromatic treatments (warm in smooth / cool at boundaries). All prior gradient
    passes apply monotonic treatment; this splits warm/cool by gradient level.

  - canvas_tooth_texture_pass: EIGHTY-FIRST DISTINCT MODE
    CANVAS WEAVE TOOTH SIMULATION:
    Crossed sine wave canvas weave pattern at canvas scale (tooth_freq cycles/unit);
    cavity_mask = clip(1 - weave/tooth_thresh) → high in weave troughs;
    midtone bell gate [tooth_lo, tooth_hi] — tooth most visible in midtones
    (paint bridges canvas peaks in highlights, but reveals tooth in midtones);
    blend toward ground colour in cavity zones.
    NOVEL: FIRST pass to use DETERMINISTIC PERIODIC STRUCTURE (crossed sine waves)
    to simulate canvas weave tooth — unlike film_grain (mode 54) which uses
    stochastic Gaussian noise, this uses periodic canvas-scale structure with
    midtone luma gating, giving the surface tactile woven quality of oil-on-canvas.
"""
...
def paint(output_path: str = "mona_lisa_s180.png") -> str:
    """Full Leonardo sfumato pipeline with session 180 Cesare da Sesto passes."""
    print("=" * 64)
    print("  Session 180 — Mona Lisa sfumato portrait")
    print("  Artistic discovery: Cesare da Sesto (c. 1477–1523)")
    print("    Milanese-Roman Bridge Renaissance")
    print("    Leonardo's sfumato warmth + Raphael's cool classical clarity")
    print("    'Salome' (Vienna / Madrid) — supreme bridge synthesis")
    print("    EIGHTIETH MODE: cesare_da_sesto_clarity_pass")
    print("    -- Gradient-zoned bimodal warm/cool bridge")
    print("    EIGHTY-FIRST MODE: canvas_tooth_texture_pass")
    print("    -- Canvas weave tooth simulation (periodic structure)")
    print("=" * 64)
    ...
    print("  [24/30] Ter Borch satin sheen pass ...")
    ...
    print("  [25/30] Shadow chroma depth pass ...")
    ...
    print("  [26/30] Cesare da Sesto clarity pass -- EIGHTIETH DISTINCT MODE ...")
    # MILANESE-ROMAN CLARITY BRIDGE: Cesare's synthesis of Leonardo warmth in smooth
    # zones and Raphael's cool 'air between forms' at medium-gradient boundaries.
    # Applied after the satin/chroma passes so it reads on a warm, tonally-rich surface.
    # The bimodal warm/cool gradient response gives the portrait simultaneous depth
    # and spatial legibility — the quality that distinguishes Cesare's hybrid manner.
    ...
    print("  [27/30] Canvas tooth texture pass -- EIGHTY-FIRST DISTINCT MODE ...")
    # CANVAS WEAVE TOOTH SIMULATION: crossed sine waves at canvas scale reveal the
    # linen weave ground in midtone zones. Applied at moderate opacity so the
    # effect reads as a subtle tactile quality rather than a dominant texture —
    # Cesare worked on panel and canvas; the tooth adds period-appropriate surface
    # materiality consistent with early-16th-century Italian practice.
    ...
    print("  [28/30] Luminance-preserving chroma boost pass ...")
    ...
    print("  [29/30] Brouwer tavern glow pass (warm mid-tone coherence) ...")
...
if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "mona_lisa_s180.png"
    paint(out)

"""
paint_mona_lisa_s90.py -- Session 90 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-90 additions:

  - hodler_parallelism_pass()                        -- NEW (session 90) Ferdinand Hodler.
                                                       Three-stage Parallelism technique:
                                                       (1) tonal band simplification --
                                                       reduce luminance to discrete bands
                                                       via smooth staircase, collapsing
                                                       mid-tones toward dominant plane
                                                       values; (2) contour darkening --
                                                       Sobel-detected edges are darkened
                                                       to strengthen the bold outline
                                                       separating Hodler's tonal planes;
                                                       (3) chromatic clarity -- mid-tone
                                                       high-chroma pixels receive a
                                                       saturation push toward dominant hue.

  - atmospheric_depth_pass (horizon_glow_band=0.15)  -- SESSION 90 ARTISTIC IMPROVEMENT.
                                                       Horizon luminous band: a subtle
                                                       warm-ivory glow at the atmospheric
                                                       horizon (y_frac=0.54), simulating
                                                       the distinctive pale shimmer visible
                                                       behind the Mona Lisa's shoulders
                                                       where distant sky meets rocky
                                                       terrain.  Leonardo's landscape
                                                       horizon carries a bright warm-cool
                                                       atmospheric transition not replicated
                                                       by the earlier uniform haze; the
                                                       new horizon_glow_band parameter
                                                       adds a Gaussian-falloff luminance
                                                       lift centred on that horizon line.

  Sessions 89 and earlier (retained):
...
Session 90 artistic character:
  The random artist for this session is Ferdinand Hodler (1853-1918), the
  leading Swiss painter of his era and the originator of Parallelism — the
  doctrine that nature organises itself into rhythmic, symmetrical, repeated
  structures.  In his Alpine landscapes and figure paintings, Hodler reduced
  form to bold flat tonal zones separated by strong dark contour lines, with
  each colour area reading as its essential chromatic identity rather than a
  complex mixture.

  Hodler's contribution to the accumulated portrait is tonal clarification.
  After nineteen sessions, the composition has rich tonal complexity: Redon's
  jewel saturation, Spilliaert's shadow compression, Géricault's turbulence.
  What Hodler adds is structural clarity — a slight reduction in the number
  of distinct luminance levels toward clean distinct planes, a gentle darkening
  of the already-present edge contours, and a push of the cleaner mid-tone
  chromatic passages toward greater saturation identity.

  At opacity 0.30, the effect is subtle: the portrait does not become poster-
  like, but the tonal zones become more legible, edges slightly crisper, and
  the cobalt blue of the distant landscape slightly more decisive.

  The session 90 artistic improvement — horizon_glow_band in atmospheric_depth_pass()
  — corrects a gap in the existing pipeline: all prior sessions included the
  atmospheric_depth_pass but without a horizon glow band.  The Mona Lisa's
  background is notable for the bright pale zone at the horizon — the distant
  sky catching warm atmospheric light at the join between sky and rocky terrain.
  Leonardo's landscape horizon is never the darkest part of the canvas; it is
  specifically brighter than the mid-distance terrain, creating a warm-light
  effect as the eye recedes toward infinity.  The horizon_glow_band parameter
  adds a Gaussian-profile luminance lift centred at y_frac=0.54 that replicates
  this effect: a soft pale ivory-amber band brightening the middle-distance
  atmospheric haze while leaving the foreground figure and near-dark terrain
  untouched.
"""
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-90 painting pipeline."""
    print("Session 90 -- Mona Lisa portrait")
    print("Random artist: Ferdinand Hodler (Swiss Symbolism, 1853-1918)")
    print("New pass: hodler_parallelism_pass()")
    print("  -- tonal band simplification: reduce luminance to discrete planes")
    print("  -- contour darkening: strengthen bold outlines between tonal zones")
    print("  -- chromatic clarity: push clean mid-tone hues toward saturation identity")
    print()
    print("Session 90 artistic improvement: horizon_glow_band in atmospheric_depth_pass()")
    print("  Leonardo's landscape horizon carries a bright warm-ivory atmospheric glow")
    print("  where distant sky meets rocky terrain.  horizon_glow_band=0.15 adds a")
    print("  Gaussian-profile luminance lift centred at y_frac=0.54, replicating")
    print("  the distinctive pale shimmer visible behind the Mona Lisa's shoulders.")
    print()
    ...
    # ── SESSION 90: Atmospheric depth pass with horizon_glow_band (ARTISTIC IMPROVEMENT) ──
    # Leonardo's horizon carries a bright warm-ivory glow where distant sky meets terrain.
    # horizon_glow_band=0.15 adds a Gaussian luminance lift at y_frac=0.54.
    ...
    # ── SESSION 90: Hodler parallelism pass (NEW) ─────────────────────────────
    # Ferdinand Hodler (1853–1918) — Swiss Symbolist / Post-Impressionist.
    # Three operations: tonal band simplification, contour darkening, chromatic clarity.
    print("Hodler parallelism pass (session 90 -- NEW)...")
    p.hodler_parallelism_pass(
        n_bands              = 7,     # 7 tonal planes -- Hodler's moderate simplification
        band_hardness        = 0.45,  # partly hard steps -- visible but not poster-like
        contour_strength     = 0.22,  # modest darkening -- reinforce without harshness
        contour_sigma        = 2.0,
        contour_thresh       = 0.05,
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s90.png")
    p.save(out_path)

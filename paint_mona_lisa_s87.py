"""
paint_mona_lisa_s87.py -- Session 87 portrait painting.

Paints the Mona Lisa-inspired portrait using the full stroke-engine pipeline,
with session-87 additions:

  - whistler_tonal_harmony_pass()               -- NEW (session 87) James McNeill Whistler.
                                                   Three-stage tonal harmony:
                                                   (1) tonal key lock — compress all
                                                   luminance values toward a target
                                                   key center (0.48 = mid-key),
                                                   narrowing the tonal range into
                                                   Whistler's coherent 'keyed' register;
                                                   (2) cool silver monochromatic drift
                                                   — pull the palette toward Whistler's
                                                   dominant cool silver-grey hue,
                                                   reducing chromatic variety while
                                                   maintaining hue character; (3)
                                                   peripheral edge dissolution — apply
                                                   progressive Gaussian softening from
                                                   the compositional periphery inward,
                                                   dissolving edge detail into
                                                   atmospheric suggestion.

  - cool_atmospheric_recession_pass
    (lateral_horizon_asymmetry=+0.06)           -- SESSION 87 ARTISTIC IMPROVEMENT.
                                                   The Mona Lisa's background famously
                                                   has a left/right horizon mismatch —
                                                   the landscape on the left sits
                                                   visibly higher than the right.
                                                   lateral_horizon_asymmetry replicates
                                                   this by computing a per-column
                                                   effective horizon that varies
                                                   linearly across the canvas width:
                                                   left edge horizon is raised, right
                                                   edge is lowered.  When =0 the
                                                   behaviour is identical to prior
                                                   sessions.

  Sessions 86 and earlier (retained):
...
Session 87 artistic character:
  The random artist for this session is James McNeill Whistler (1834–1903),
  the American-British Tonalist whose philosophy of 'art for art's sake' —
  naming paintings as 'Arrangements', 'Harmonies', and 'Nocturnes' rather
  than describing their subjects — transformed the formal language of
  Western painting in the second half of the nineteenth century.

  Whistler stands in a highly specific relationship to the Mona Lisa's
  visual world.  The accumulated sessions have built extraordinary tonal
  depth through oil techniques: sfumato dissolution (Leonardo), sculptural
  contour clarity (Signorelli), warm golden sfumato (del Sarto), granular
  optical texture (Chardin), pearlescent pastel luminosity (Carriera).
  What the composition still lacks — despite all this layering — is the
  overarching tonal coherence that Whistler made the central purpose of
  his art: a unified 'key' into which every value is tuned before being
  placed on the canvas.

  The three-stage pass addresses this directly:

  1. Tonal key lock gently compresses the existing accumulated tonal range
     toward a mid-key center (0.48), preventing the extreme contrasts that
     accumulated chiaroscuro passes (Géricault, Rigaud, Signorelli) can
     introduce from dominating the composition.  The effect is subtle —
     the shadows remain deep, the highlights bright — but the sense of
     tonal excess is controlled into Whistler's considered equilibrium.

  2. Cool silver monochromatic drift nudges saturation inward toward
     Whistler's dominant cool silver-grey hue, harmonising the palette
     after the many warm-push sessions (del Sarto golden sfumato, Steen
     warm amber vitality, Géricault shadow warmth).  The accumulated
     warmth is not removed — just gently tuned toward Whistler's 'harmony'
     register where every colour feels related to a shared tonal theme.

  3. Peripheral edge dissolution applies graduated Gaussian softening from
     the canvas edges inward, dissolving the picture boundary into
     atmospheric suggestion in the manner of Whistler's Nocturnes and
     his Japanese woodblock compositional instincts.  The focal center
     (face, hands) retains full sharpness; the peripheral landscape and
     dress edges dissolve into a Whistlerian atmospheric ground.

  The session 87 artistic improvement — lateral_horizon_asymmetry in
  cool_atmospheric_recession_pass() — directly replicates the most
  famous compositional mystery of the actual Mona Lisa: the left side
  of the background sits measurably higher than the right, creating a
  subtle, disorienting perspective discontinuity that art historians have
  debated for centuries.  This was almost certainly intentional on
  Leonardo's part — a compositional device that subtly unsettles the
  viewer's spatial reading of the background and reinforces the portrait's
  enigmatic, slightly uncanny quality.  The per-column horizon calculation
  means every column of the atmospheric recession effect now has its own
  effective horizon height, producing a natural left/right asymmetry
...
def make_reference() -> Image.Image:
    ...
    def horizon_y(x: int) -> int:
        t = x / W
        # Left horizon at 50% canvas height, right at 56% — mirroring the
        # famous Mona Lisa background asymmetry where the left landscape is
...
def paint(out_dir: str = ".") -> str:
    """Run the full session-87 painting pipeline."""
    print("Session 87 -- Mona Lisa portrait")
    print("Random artist: James McNeill Whistler (American Tonalism, 1834–1903)")
    print("New pass: whistler_tonal_harmony_pass()")
    print("  -- tonal key lock: compress values toward mid-key register (0.48)")
    print("  -- cool silver monochromatic drift: palette harmony toward dominant grey")
    print("  -- peripheral edge dissolution: Japanese woodblock edge dissolution")
    print()
    print("Session 87 artistic improvement: lateral_horizon_asymmetry in")
    print("cool_atmospheric_recession_pass() — per-column horizon height replicates")
    print("the famous left/right background mismatch of the actual Mona Lisa.")
    print()
    ...
    # lateral_horizon_asymmetry=+0.06 raises the left horizon and lowers the right,
    # replicating the Mona Lisa's famous left/right background height mismatch.
    print("Cool atmospheric recession pass (session 74, IMPROVED session 87 -- lateral_horizon_asymmetry=+0.06)...")
    p.cool_atmospheric_recession_pass(
        ...
        feather                   = 0.12,
        opacity                   = 0.64,
        lateral_horizon_asymmetry = 0.06,   # left side higher, right side lower
    )
    ...
    # ── SESSION 87: Whistler tonal harmony pass (NEW) ─────────────────────────
    ...
    out_path = os.path.join(out_dir, "mona_lisa_s87.png")
    p.save(out_path)
